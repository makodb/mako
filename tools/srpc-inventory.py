#!/usr/bin/env python3
"""
Phase 0 inventory for the srpc → DSL migration plan
(see docs/TODO-rusty-rewrite.md).

Walks src/srpc/ (excluding tests), enumerates every top-level
...
docs/srpc-inventory.csv) plus a short Markdown summary at
--summary (default docs/srpc-inventory.md).
...
def write_summary(decls: list[Decl], out_path: Path) -> None:
    ...
    with out_path.open("w") as f:
        f.write("# srpc decl inventory (Phase 0)\n\n")
        f.write(
        ...
        for d in sorted(manual, key=lambda d: -d.loc)[:20]:
            relfile = d.file.split("src/srpc/", 1)[-1]
        ...
        for fname, loc in sorted(per_file.items(), key=lambda kv: -kv[1])[:15]:
            relfile = fname.split("src/srpc/", 1)[-1]
...
def main() -> int:
    ...
    p.add_argument("--out", default="docs/srpc-inventory.csv")
    p.add_argument("--summary", default="docs/srpc-inventory.md")
    args = p.parse_args()
    root = Path(args.root)
    if not root.exists():

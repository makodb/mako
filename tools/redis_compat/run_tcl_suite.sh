...
TEST_HELPER="${ROOT_DIR}/third-party/redis-tests/tests/test_helper.tcl"
...
FILES="${TCL_COMPAT_FILES:-unit/type/string unit/type/set unit/type/zset unit/expire unit/scan unit/multi unit/keyspace unit/networking unit/pubsub}"
passed=0
failed=0

for file in ${FILES}; do
    if tclsh "${TEST_HELPER}" --host "${HOST}" --port "${PORT}" --single "${file}" --singledb --ignore-encoding --ignore-digest; then
        passed=$((passed + 1))
    else
        failed=$((failed + 1))
    fi

#!/usr/bin/env python3
"""Smoke test for tools/srpc-inventory.py. Runs the script against a
small synthesized source tree under a tmp dir and checks that the
expected decls land in the expected buckets.

Invoke via `python3 -m unittest tools/test_srpc_inventory.py` from the
repo root, or just `python3 tools/test_srpc_inventory.py`.
"""
...
spec = importlib.util.spec_from_file_location("srpc_inventory", SCRIPT)
mod = importlib.util.module_from_spec(spec)
sys.modules["srpc_inventory"] = mod

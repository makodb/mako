...
- [ ] Mako, build a high-performance, reliable, transactional, datastore; GA release
  - repeated task
    ...
    - [ ] for every day, check docs/judge/commit_reviews.md to evaluate `Open Issues`. Evaluate each open issue, if you believe this issue is reasonable and can be fixed easily (e.g., changes <= 200 lines), add a task in TODO.md to fix this issue. For each added task, you should tag its corresponding Issue ID to avoid duplicated task created for the same issue. [last done: 2026-02-10, 18:54 - docs/testing/commit_reviews.md checked. 16 commits reviewed (a6bed72c..7a75d1af). Found 2 high-severity issues in commit 232ba3b0 (Raft disk persistence): ISSUE-232ba3b0-1 (detached threads use-after-free) and ISSUE-232ba3b0-2 (quorum double-counting). Added fix tasks to TODO.md.]
    - [ ] for every day, check the commits in the last 48 hours if they introdued any rusty-unsafe functions or blocks. If found any, please fix them, only use rusty safe coding. [last done: 2026-02-10, 18:57 - 12 commits in last 48 hours (aefdc2fa..7a75d1af). All are CI/test infrastructure changes (port allocation, timestamps). No rusty-unsafe code introduced - no std::unique_ptr, std::shared_ptr, std::weak_ptr, or @unsafe blocks added.]
    - [ ] for every day, run all the ci tests listed in github ci workflow, make sure no test fail. If failed tests found, investigate and fix. Repeat until no failures are detected. Don't cheat by removing or weakening tests. Also, double check the github ci test and the "ci all" have the same tests; if one misses something, add it. [last done: 2026-02-10, 19:33 - All 19 tests passed: rrrTests 66/66 (74.57 sec), simpleTransaction (passed), clientServer (passed), simplePaxos (passed), shardNoReplication (~8200 ops/sec), shardNoReplicationErpc (~140k ops/sec), shard1Replication (passed), shard2Replication (~9044 ops/sec), shard2ReplicationErpc (passed), shard1ReplicationSimple (passed), shard2ReplicationSimple (passed), shard1ReplicationRaft (passed), shard2ReplicationRaft (passed), shard1ReplicationSimpleRaft (passed), shard2ReplicationSimpleRaft (passed), rocksdbTests (passed), multiShardSingleProcess (~369k ops/sec), shard2SingleProcess (passed), shard2SingleProcessReplication (passed). GitHub CI workflow and ci.sh 'all' test lists verified identical. Borrow checking passed during build.]
  - [ ] *high* Fix use-after-free in Raft async persistence: detached threads capture `this` in doVote() and OnAppendEntries() async paths. If RaftServer is destroyed while thread runs, use-after-free occurs. Fix: use shared_from_this() or track threads for join in destructor. [ISSUE-232ba3b0-1]
  - [ ] *high* Fix quorum double-counting in Raft OnPeerRestart(): durableVoters_ already contains site_id_ (from ResetSpeculativeState), but OnPeerRestart() adds +1 for self, making quorum off-by-one (too lenient). Same issue for specVoters_. Fix: remove the +1 or verify set doesn't contain self. [ISSUE-232ba3b0-2]

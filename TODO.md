...
- [ ] Mako, build a high-performance, reliable, transactional, datastore; GA release
  - repeated task
    ...
    - [ ] for every day, run all the ci tests listed in github ci workflow, make sure no test fail. If failed tests found, investigate and fix. Repeat until no failures are detected. Don't cheat by removing or weakening tests. Also, double check the github ci test and the "ci all" have the same tests; if one misses something, add it. [last done: 2026-01-23, 04:00 - ALL TESTS PASSED. rrrTests 64/65 (1 flaky timeout on test_transport_integration, passed on retry), simpleTransaction, simplePaxos, clientServer, shardNoReplication, shardNoReplicationErpc, shard1Replication, shard2Replication, shard2ReplicationErpc, shard1ReplicationRaft, shard2ReplicationRaft, shard1ReplicationSimple, shard2ReplicationSimple, shard1ReplicationSimpleRaft, shard2ReplicationSimpleRaft, rocksdbTests, multiShardSingleProcess, shard2SingleProcess, shard2SingleProcessReplication. See logs/20260123_7051bafd_ci_daily.log.]
  - [ ] *high* Fix transaction ID collision risk in MakoClientService. [ISSUE-1886cab7-1, ISSUE-33b02756-1]
    - Problem: `HandleBeginTxn` uses `client_id` directly as `txn_id`. Multiple BeginTxn calls from same client will have same txn_id.
    - Evidence: `src/mako/client_service.cc:65-72` - `uint64_t txn_id = static_cast<uint64_t>(client_id);`
    - Fix: Implement proper txn_id generation using atomic counter in MakoClientService or delegate to ShardReceiver::begin_txn().
    - Also fix docs/client_server_architecture.md which claims txn_id = client_id (upper 32) + counter (lower 32) but that's not implemented.

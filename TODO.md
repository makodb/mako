...
- [ ] Mako, build a high-performance, reliable, transactional, datastore; GA release
  - repeated task
    - [ ] for every hour, check https://github.com/makodb/mako/actions/workflows/ci.yml, see if the most recent done ci test is a failure. If it fails, add a fix task to TODO.md (attach the git commit hash so we do not add duplicated TODO items). Please don’t commit this as a standalone change—it clutters the commit history. Instead, include this hourly update in your next commit along with other changes. [last checked: 2026-01-17, 15:15 - 472 runs total. Run #472 queued, #470 in progress. Runs #469-#465 completed successfully. No failures identified.]
    ...
    - [ ] for every day, run all the ci tests listed in github ci workflow, make sure no test fail. If failed tests found, investigate and fix. Repeat until no failures are detected. Don't cheat by removing or weakening tests. Also, double check the github ci test and the "ci all" have the same tests; if one misses something, add it. [last done: 2026-01-17, 14:47 - all CI steps passed: rrrTests 65/65, simpleTransaction, clientServer, simplePaxos, shard1Replication, shard2Replication, shard2ReplicationErpc, all Raft variants, rocksdbTests, shardFaultTolerance, multiShardSingleProcess, shard2SingleProcessReplication. Log: logs/20260117_144722_0a9b5250_daily_ci.log]

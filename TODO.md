...
- [ ] Mako, build a high-performance, reliable, transactional, datastore; GA release
  - repeated task
    - [ ] for every hour, check https://github.com/makodb/mako/actions/workflows/ci.yml, see if the most recent done ci test is a failure. If it fails, add a fix task to TODO.md (attach the git commit hash so we do not add duplicated TODO items). [last checked: 2026-01-17, 10:05 - 463 total runs. #463 in progress, #462-#454 all SUCCESS (20s to 1h41m). No failures detected.]
    - [ ] for every day, check if rusty-cpp checks all source files, if not, fix. Make sure rusty-cpp is not disabled. [last done: 2026-01-17, 00:10 - all borrow_check_all targets pass with no violations, including deptran_borrow, rrr_borrow, masstree, and new client_service files]
    - [ ] for every day, check the commits in the last 48 hours if they introdued any rusty-unsafe functions or blocks. If found any, please fix them, only use rusty safe coding. [last done: 2026-01-17, 00:20 - found std::unique_ptr usage in recent RRR RPC refactoring commits. Converted remote_db.hh: proxy_ and tables_ members from std::unique_ptr to rusty::Option<rusty::Box>. client_tcp_server.h worker_slots_ kept as std::unique_ptr (justified: WorkerSlot contains non-movable std::thread, rusty::Box::make requires move-constructible types).]
    - [ ] for every day, run all the ci tests listed in github ci workflow, make sure no test fail. If failed tests found, investigate and fix. Repeat until no failures are detected. Don't cheat by removing or weakening tests. Also, double check the github ci test and the "ci all" have the same tests; if one misses something, add it. [last done: 2026-01-16, 23:30 - all CI steps passed: rrrTests 65/65, simpleTransaction, clientServer (Test4 skipped - single-shard limitation), simplePaxos, shard1Replication, shard2Replication (8786 ops/sec). Fixed clientServer test to skip Test4 in single-shard mode since TCP client interface requires multi-shard setup with helper servers.]
  - [ ] *high* bug. shard2Replication still fails on ci server (run via ./ci/ci.sh shard2Replication) from time to time, please investigate and fix. verify fix by running it 10 times.

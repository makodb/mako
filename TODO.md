...
- [ ] Mako, build a high-performance, reliable, transactional, datastore; GA release
  - repeated task
    ...
    - [ ] for every day, check docs/judge/commit_reviews.md to evaluate `Open Issues`. Evaluate each open issue, if you believe this issue is reasonable and can be fixed easily (e.g., changes <= 200 lines), add a task in TODO.md to fix this issue. For each added task, you should tag its corresponding Issue ID to avoid duplicated task created for the same issue. [last done: 2026-03-16, 19:30 - No commit_reviews.md file exists. 4 commits in 48h window (cd4b90ee, cc8205c6, e664eb5a, 3eaef656). No open issues to evaluate.]
    - [ ] for every day, check the commits in the last 48 hours if they introdued any rusty-unsafe functions or blocks. If found any, please fix them, only use rusty safe coding. [last done: 2026-03-16, 19:30 - 4 commits in 48h window (cd4b90ee, cc8205c6, e664eb5a, 3eaef656). e664eb5a: tests/correctness/*.py (Python only) + suite_results.txt + TODO.md restore — no C++ changes. 3eaef656: CORRECTNESS_REPORT.md only. No new unique_ptr/shared_ptr/raw-new/detach in any C++ file. CLEAN.]

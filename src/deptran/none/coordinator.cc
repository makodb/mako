...
void CoordinatorNone::GotoNextPhase() {
  ...
  // Log_info("Enter GotoNextPhase CoroutineID %d %d phase_ = %d", Fiber::current_fiber()->id, Fiber::current_fiber()->global_id, phase_);
  int n_phase = 2;
  // int judgement_phase = phase_;
  switch (phase_++ % n_phase) {
    case Phase::INIT_END:
      // Log_info("Enter switch CoroutineID %d %d phase_ = %d", Fiber::current_fiber()->id, Fiber::current_fiber()->global_id, phase_);

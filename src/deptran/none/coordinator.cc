...
void CoordinatorNone::GotoNextPhase() {
  ...
  // uint64_t coroutine_id = Coroutine::current_coroutine()->id;
  // uint64_t coroutine_global_id = Coroutine::current_coroutine()->global_id;
  // Log_info("Enter GotoNextPhase CoroutineID %d %d phase_ = %d", Coroutine::current_coroutine()->id, Coroutine::current_coroutine()->global_id, phase_);
  int n_phase = 2;
  // int judgement_phase = phase_;
  switch (phase_++ % n_phase) {
    case Phase::INIT_END:
      // Log_info("Enter switch CoroutineID %d %d phase_ = %d", Coroutine::current_coroutine()->id, Coroutine::current_coroutine()->global_id, phase_);

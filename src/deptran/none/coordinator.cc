...
void CoordinatorNone::GotoNextPhase() {
  ...
  // uint64_t coroutine_id = Fiber::current_coroutine()->id;
  // uint64_t coroutine_global_id = Fiber::current_coroutine()->global_id;
  // Log_info("Enter GotoNextPhase CoroutineID %d %d phase_ = %d", Fiber::current_coroutine()->id, Fiber::current_coroutine()->global_id, phase_);
  int n_phase = 2;
  // int judgement_phase = phase_;
  switch (phase_++ % n_phase) {
    case Phase::INIT_END:
      // Log_info("Enter switch CoroutineID %d %d phase_ = %d", Fiber::current_coroutine()->id, Fiber::current_coroutine()->global_id, phase_);

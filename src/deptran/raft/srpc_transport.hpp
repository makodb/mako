 ...
 *        srpc::IntEvent until the reply lands.
 *
 * Rusty-safety:
 ...
 *    at the srpc boundary; `RaftCommo::Send*Cb` takes a std::function
...
class SrpcTransportAdapter : public TransportBase {
 public:
  // @unsafe { non-owning raw pointer; caller must ensure commo outlives this }
  SrpcTransportAdapter(RaftCommo* commo, siteid_t self, parid_t par)
  ...
  // @unsafe { std::function bridge at srpc boundary }
  ...
  // @unsafe { std::function + shared_ptr bridge at srpc boundary.
...
inline TransportProxy make_srpc_transport(RaftCommo* commo,
                                         siteid_t  self,
                                         parid_t   par) {
  return rusty::make_box<SrpcTransportAdapter>(commo, self, par);
}

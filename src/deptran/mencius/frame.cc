...
Communicator *MenciusFrame::CreateCommo(rusty::Option<rusty::Arc<PollThread>> poll) {
...
MenciusFrame::CreateRpcServices(uint32_t site_id,
  ...
  switch (config->replica_proto_) {
    case MODE_MENCIUS:result.push_back(rrr::make_service_proxy_from_typed_box(rusty::make_box<MenciusServiceImpl>(rep_sched)));
    default:break;
  }
  return result;
}

...
void MultiPaxosServiceImpl::ForwardToLearnerI(const MarshallDeputy& cmd, rrr::i32* ballot, rrr::i32* val, rrr::DeferredReply* defer) {
    verify(sched_ != nullptr);
    Log_info("received a message for the forwardToLearnerI");
    Coroutine::CreateRun([&] () {
      defer->reply();
    });
}

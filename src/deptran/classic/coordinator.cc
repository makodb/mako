...
void CoordinatorClassic::DoTxAsync(TxRequest& req) {
  ...
  if (config->forwarding_enabled_ && forward_status_ == FORWARD_TO_LEADER) {
    ...
    Coroutine::create_run([this]() {
        // Log_info("Start CoroutineID %d %d", Coroutine::current_coroutine()->id, Coroutine::current_coroutine()->global_id);
        GotoNextPhase();
      }, __FILE__, __LINE__
    );
  }
...
void CoordinatorClassic::DispatchAck(phase_t phase,
  ...
  if (res == REJECT) {
    ...
    // Log_info("DispatchAck Reject CoroutineID %d %d", Coroutine::current_coroutine()->id, Coroutine::current_coroutine()->global_id);
    GotoNextPhase();
    return;
  ...
  if (txn->HasMoreUnsentPiece()) {
    ...
    if (phase != phase_) {
      // Log_info("AllDispatchAcked Failed CoroutineID %d %d", Coroutine::current_coroutine()->id, Coroutine::current_coroutine()->global_id);
      return;
    }
    // Log_info("AllDispatchAcked Successed CoroutineID %d %d", Coroutine::current_coroutine()->id, Coroutine::current_coroutine()->global_id);
    GotoNextPhase();
  }
}

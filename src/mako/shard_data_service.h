#pragma once

// ShardDataService — the RPC transport for a shard's data plane, so a migration
// coordinator can drive a SOURCE or DESTINATION participant that lives in
// ANOTHER process.
//
//   ShardDataServiceImpl : server side, runs ON a shard, backed by that shard's
//                          ShardData (an OrderedIndexShardData over its real mbta
//                          index). Handler logic is factored into Do* methods
//                          (unit-testable with no socket); the RPC handlers just
//                          marshal, exactly like src/deptran/config_kv_service.h.
//   RemoteShardData      : client side -- a ShardData whose every op is one RPC
//                          to the remote shard's service. ShardMigrator takes it
//                          as a participant, so cross-process online migration
//                          reuses the SAME coordinator logic with zero changes.
//                          The range ops (checksum / verify_range / drop_range)
//                          are ONE RPC each -- the rows never cross the wire for
//                          those; only ScanRange (the bulk copy) ships pairs.

#include "rcc_rpc.h"        // ShardDataServiceService / ShardDataServiceProxy
#include "shard_data.h"     // ShardData

#include <cstdint>
#include <map>
#include <string>
#include <utility>
#include <vector>

namespace janus {

// @safe - thin handler over an injected ShardData (the shard's real data plane).
class ShardDataServiceImpl : public ShardDataServiceService {
public:
    explicit ShardDataServiceImpl(ShardData* shard) : shard_(shard) {}

    // ---- handler logic, factored out so it unit-tests without the socket ----
    std::map<std::string, std::string>
    DoScanRange(const std::string& lo, const std::string& hi, int limit) {
        std::map<std::string, std::string> rows;
        if (shard_ == nullptr) return rows;
        for (auto& kv : shard_->scan_range_limited(lo, hi, static_cast<size_t>(limit)))
            rows.emplace(std::move(kv.first), std::move(kv.second));
        return rows;
    }
    uint64_t DoChecksum(const std::string& lo, const std::string& hi) {
        return shard_ ? shard_->checksum(lo, hi) : 0;
    }
    bool DoVerifyRange(const std::string& lo, const std::string& hi, uint64_t expected) {
        return shard_ != nullptr && shard_->verify_range(lo, hi, expected);
    }
    void DoPut(const std::string& key, const std::string& value) {
        if (shard_) shard_->put(key, value);
    }
    void DoRemove(const std::string& key) { if (shard_) shard_->remove(key); }
    void DoDropRange(const std::string& lo, const std::string& hi) {
        if (shard_) shard_->drop_range(lo, hi);
    }
    bool DoGet(const std::string& key, std::string* out) {
        return shard_ != nullptr && out != nullptr && shard_->get(key, *out);
    }

    // ---- RPC handlers (generated signatures; delegate to Do*) ----
    void ScanRange(const RpcScanRangeRequest& req, RpcScanRangeResponse& resp,
                   rrr::DeferredReply defer) override {
        resp.rows = DoScanRange(req.lo, req.hi, req.limit);
        defer.reply();
    }
    void Checksum(const RpcChecksumRequest& req, RpcChecksumResponse& resp,
                  rrr::DeferredReply defer) override {
        resp.checksum = static_cast<rrr::i64>(DoChecksum(req.lo, req.hi));
        defer.reply();
    }
    void VerifyRange(const RpcVerifyRangeRequest& req, RpcVerifyRangeResponse& resp,
                     rrr::DeferredReply defer) override {
        resp.ok = DoVerifyRange(req.lo, req.hi,
                                static_cast<uint64_t>(req.expected)) ? 1 : 0;
        defer.reply();
    }
    void PutKey(const RpcPutKeyRequest& req, RpcPutKeyResponse& resp,
                rrr::DeferredReply defer) override {
        DoPut(req.key, req.value); resp.ok = 1; defer.reply();
    }
    void RemoveKey(const RpcRemoveKeyRequest& req, RpcRemoveKeyResponse& resp,
                   rrr::DeferredReply defer) override {
        DoRemove(req.key); resp.ok = 1; defer.reply();
    }
    void DropRange(const RpcDropRangeRequest& req, RpcDropRangeResponse& resp,
                   rrr::DeferredReply defer) override {
        DoDropRange(req.lo, req.hi); resp.ok = 1; defer.reply();
    }
    void GetKey(const RpcGetKeyRequest& req, RpcGetKeyResponse& resp,
                rrr::DeferredReply defer) override {
        std::string v;
        const bool found = DoGet(req.key, &v);
        resp.found = found ? 1 : 0;
        if (found) resp.value = std::move(v);
        defer.reply();
    }

private:
    ShardData* shard_;   // non-owning; the shard's real data plane.
};

// @unsafe - a ShardData whose ops are RPCs to a remote shard's ShardDataService.
// The proxy must outlive this. ShardMigrator uses it as a source/destination
// participant, so the coordinator drives a cross-process migration unchanged.
class RemoteShardData : public ShardData {
public:
    explicit RemoteShardData(ShardDataServiceProxy* proxy) : proxy_(proxy) {}

    void put(const std::string& key, const std::string& value) override {
        ShardDataServiceProxy::RpcPutKeyRequest req;
        req.key = key; req.value = value;
        (void)proxy_->PutKey(req);
    }
    bool get(const std::string& key, std::string& out) override {
        ShardDataServiceProxy::RpcGetKeyRequest req; req.key = key;
        auto r = proxy_->GetKey(req);
        if (r.is_err()) return false;
        auto resp = r.unwrap();
        if (!resp.found) return false;
        out = std::move(resp.value);
        return true;
    }
    void remove(const std::string& key) override {
        ShardDataServiceProxy::RpcRemoveKeyRequest req; req.key = key;
        (void)proxy_->RemoveKey(req);
    }
    std::vector<KvPair> scan_range_limited(const std::string& lo,
                                           const std::string& hi,
                                           size_t limit) override {
        std::vector<KvPair> out;
        ShardDataServiceProxy::RpcScanRangeRequest req;
        req.lo = lo; req.hi = hi; req.limit = static_cast<rrr::i32>(limit);
        auto r = proxy_->ScanRange(req);
        if (r.is_err()) return out;
        for (auto& kv : r.unwrap().rows) out.emplace_back(kv.first, kv.second);
        return out;   // rows arrive sorted (a std::map) -> ascending vector
    }
    std::vector<KvPair> scan_range(const std::string& lo,
                                   const std::string& hi) override {
        // A full scan = repeated bounded remote scans (keeps each message small).
        std::vector<KvPair> out;
        std::string cur = lo;
        while (cur < hi) {
            std::vector<KvPair> batch = scan_range_limited(cur, hi, 1024);
            if (batch.empty()) break;
            std::string last = batch.back().first;
            for (auto& kv : batch) out.push_back(std::move(kv));
            cur = last; cur.push_back('\0');
        }
        return out;
    }
    // Range ops as ONE RPC each -- the range's rows never cross the wire.
    uint64_t checksum(const std::string& lo, const std::string& hi) override {
        ShardDataServiceProxy::RpcChecksumRequest req; req.lo = lo; req.hi = hi;
        auto r = proxy_->Checksum(req);
        return r.is_err() ? 0 : static_cast<uint64_t>(r.unwrap().checksum);
    }
    bool verify_range(const std::string& lo, const std::string& hi,
                      uint64_t expected) override {
        ShardDataServiceProxy::RpcVerifyRangeRequest req;
        req.lo = lo; req.hi = hi; req.expected = static_cast<rrr::i64>(expected);
        auto r = proxy_->VerifyRange(req);
        return !r.is_err() && r.unwrap().ok != 0;
    }
    void drop_range(const std::string& lo, const std::string& hi) override {
        ShardDataServiceProxy::RpcDropRangeRequest req; req.lo = lo; req.hi = hi;
        (void)proxy_->DropRange(req);
    }

private:
    ShardDataServiceProxy* proxy_;   // non-owning; connected to the remote shard.
};

}  // namespace janus

...
protected:
  ...
  struct dbtuple_write_info {
    ...
    // Declaration only - implementation is outside class
...
// IMPLEMENTATION MOVED HERE TO FIX "compare" VIOLATIONS
// @unsafe: uses marked_ptr comparison which checker sees as "unknown"
inline ALWAYS_INLINE
bool transaction_base::dbtuple_write_info::operator<(const dbtuple_write_info &o) const
{
...
// @unsafe: uses function pointer call g_proto_version_str
      ...
      const dbtuple *tuple)
  {
    ...
    return std::binary_search(
        ...
        [](const dbtuple_write_info &lhs, const dbtuple_write_info &rhs)
          { return lhs < rhs; }); // Changed to use the operator we defined
  }

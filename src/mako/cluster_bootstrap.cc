...
void StartShard0Leader(abstract_db* db, uint32_t nshards) {
    ...
    if (idx == nullptr) {
        rrr::Log_warn("BootstrapClusterConfig: could not open __mako_config__ index");
        return;
    }
    ...
    if (g_cfg_server->start(reinterpret_cast<const int8_t*>(bind_addr.c_str())) != 0) {
        rrr::Log_warn("BootstrapClusterConfig: config server failed to bind {}",
                 bind_addr.c_str());
        return;
    }
    rrr::Log_info("BootstrapClusterConfig: shard-0 config service listening on {}",
             bind_addr.c_str());
...
void StartRemoteWatcher() {
    ...
    if (g_cfg_client.as_ref().unwrap()->connect(reinterpret_cast<const int8_t*>(addr.c_str()), false) != 0) {
        rrr::Log_warn("BootstrapClusterConfig: could not connect to shard-0 config at {}",
                 addr.c_str());
        return;
    }
    ...
    g_cfg_watcher.as_ref().unwrap()->start();
    rrr::Log_info("BootstrapClusterConfig: watching shard-0 config at {}", addr.c_str());
}

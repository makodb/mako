  ...
  void load_simple() {
    ...
    try {
      for (uint i = 1; i <= NumWarehouses(); i++) {
        ...
        warehouse::value v;
        tbl_warehouse(i)->tx_insert(txn, Encode(k), Encode(obj_buf, v));
      }
      ...
      for (uint w_id = 1; w_id <= NumWarehouses(); w_id++)
        for (uint c_id = 1; c_id <= 10; c_id++) {
          ...
          v.c_delivery_cnt = 0;
          tbl_customer(w_id)->tx_insert(txn_1, Encode(k), Encode(obj_buf, v));
    ...
    for (size_t batchid = 0; batchid < nbatches;) {
      ...
      try {
        ...
        for (size_t i = batchid * batchsize + keystart; i < rend; i++) {
          ...
          tbl_item(1)->tx_insert(txn, EncodeK(k), Encode(obj_buf, v));
        }
  ...
  load()
  {
    ...
    try {
      ...
      for (uint i = 1; i <= NumWarehouses(); i++) {
        ...
        ALWAYS_ERROR(tbl_warehouse(i)->tx_get(txn, Encode(k), warehouse_v));
  ...
  load()
  {
    ...
    try {
      for (uint i = 1; i <= NumItems(); i++) {
        ...
        const size_t sz = Size(v);
        total_sz += sz;
        tbl_item(1)->tx_insert(txn, EncodeK(k), Encode(obj_buf, v)); // this table is shared, so any partition is OK
      ...
      for (uint b = 0; b < nbatches;) {
        ...
        try {
          ...
          for (uint i = (b * batchsize + 1); i <= iend; i++) {
            ...
            stock_total_sz += sz;
            n_stocks++;
            tbl_stock(w)->tx_insert(txn, EncodeK(k), Encode(obj_buf, v));
            tbl_stock_data(w)->tx_insert(txn, EncodeK(k_data), Encode(obj_buf1, v_data));
          }
          if (db->commit_txn(txn)) {
            b++;
          } else {
    ...
    try {
      ...
      for (uint w = 1; w <= NumWarehouses(); w++) {
        ...
        for (uint d = 1; d <= NumDistrictsPerWarehouse(); d++, cnt++) {
          ...
          n_districts++;
          tbl_district(w)->tx_insert(txn, Encode(k), Encode(obj_buf, v));
        ...
        for (uint batch = 0; batch < nbatches;) {
          ...
          try {
            for (uint cidx0 = cstart; cidx0 < cend; cidx0++) {
              ...
              const size_t sz = Size(v);
              total_sz += sz;
              tbl_customer(w)->tx_insert(txn, EncodeK(k), Encode(obj_buf, v));
              tbl_customer(w)->tx_insert(txn, EncodeK(k_balance), Encode(obj_buf, v_balance));
              ...
              tbl_customer(w)->tx_insert(txn, EncodeK(k_data), Encode(obj_buf, v_data));
              ...
              tbl_customer_name_idx(w)->tx_insert(txn, Encode(k_idx), Encode(obj_buf, v_idx));
              ...
              tbl_history(w)->tx_insert(txn, EncodeK(k_hist), Encode(obj_buf, v_hist));
            }
            if (db->commit_txn(txn)) {
              batch++;
            } else {
      ...
      for (uint d = 1; d <= NumDistrictsPerWarehouse(); d++) {
        ...
        for (uint c = 1; c <= NumCustomersPerDistrict();) {
          ...
          try {
            ...
            oorder_total_sz += sz;
            n_oorders++;
            tbl_oorder(w)->tx_insert(txn, EncodeK(k_oo), Encode(obj_buf, v_oo));

            const oorder_c_id_idx::key k_oo_idx(k_oo.o_w_id, k_oo.o_d_id, v_oo.o_c_id, k_oo.o_id);
            const oorder_c_id_idx::value v_oo_idx(0);

            tbl_oorder_c_id_idx(w)->tx_insert(txn, Encode(k_oo_idx), Encode(obj_buf, v_oo_idx));
            ...
            if (c >= 2101) {
              ...
              n_new_orders++;
              tbl_new_order(w)->tx_insert(txn, Encode(k_no), Encode(obj_buf, v_no));
            }
            ...
            for (uint l = 1; l <= uint(v_oo.o_ol_cnt); l++) {
              ...
              n_order_lines++;
              tbl_order_line(w)->tx_insert(txn, Encode(k_ol), Encode(obj_buf, v_ol));
            }
            if (db->commit_txn(txn)) {
              c++;
            } else {
...
void tpcc_worker::scan_entire_warehouses(int w_id) {
    ...
    // tbl_warehouse(w_id)->tx_scan(txn_0, startKey, &endKey, calloc_0);
    ...
    tbl_customer(w_id)->tx_scan(txn_1, startKey_1, &endKey_1, calloc_1);
    ...
    tbl_oorder(w_id)->tx_scan(txn_1, startKey_2, &endKey_2, calloc_2);
    ALWAYS_ERROR(db->commit_txn(txn_1));
...
tpcc_worker::txn_new_order_simple() {
    ...
    for (int c_id=1; c_id<=10; c_id++) {
      ...
      try {
        // Tranasction-1
        {
          ...
          ALWAYS_ERROR(tbl_customer(warehouse_id)->tx_get(txn, EncodeK(obj_key0, k_c), obj_v));
          if(TThread::transget_without_stable){TThread::transget_without_stable=false;}
          ...
          tbl_customer(warehouse_id)->tx_put(txn, Encode(str(), k_c), Encode(str(), v_c_new));
          const oorder::key k_oo(warehouse_id, 1, oorder_id);
          oorder::value v_oo_new;
          v_oo_new.o_c_id = 10;
          v_oo_new.o_carrier_id = c_id;
          tbl_oorder(warehouse_id)->tx_insert(txn, Encode(k_oo), Encode(str(), v_oo_new));
          ...
          if (remote_warehouse_id > 0 && !WarehouseInShard(remote_warehouse_id, BenchmarkConfig::getInstance().getShardIndex()) && (c_id == 4||c_id == 5)) {
            ...
            ALWAYS_ERROR(remote_tbl_customer(remote_warehouse_id)->tx_get(txn, Encode(obj_key0, r_k_c), obj_v));
            ...
            remote_tbl_customer(remote_warehouse_id)->tx_put(txn, EncodeK(str(), r_k_c), Encode(str(), r_v_c_new));
            is_remote=true;
          }
        }
      ...
      //Transaction-2
      {
        ...
        try {
          ...
          const oorder::key k_oo(warehouse_id, 1, oorder_id);
          tbl_oorder(warehouse_id)->tx_remove(txn, Encode(str(), k_oo));
  ...
  for (int i=0; i<ids.size(); i++) {
    ...
    bool tag = tbl_oorder(warehouse_id)->tx_get(txn, EncodeK(obj_key0, k_oo), obj_v);
...
tpcc_worker::txn_new_order_simple() {
  ...
  // scan the range: [3,7)
  {
    ...
    tbl_oorder(warehouse_id)->tx_scan(txn, Encode(obj_key0, k_no_0), &Encode(obj_key1, k_no_1), calloc, s_arena.get());
    ALWAYS_ERROR(db->commit_txn(txn));
...
tpcc_worker::txn_new_order_micro_drtm() {
  ...
  try {
    ...
    for (int i=0; i<RMW_count; i++) {
      ...
      if (isRemote&&!first) {
        first=true;
        ALWAYS_ERROR(remote_tbl_item(remote_warehouse_id)->tx_get(txn, EncodeK(k), obj_v));
        {  // optimistic replication
          ...
          remote_tbl_item(remote_warehouse_id)->tx_put(txn, Encode(str(), k), Encode(str(), v_c_new));
        ...
        ALWAYS_ERROR(tbl_item(1)->tx_get(txn, EncodeK(k), obj_v));
        if(TThread::transget_without_stable){TThread::transget_without_stable=false;}
        if(TThread::transget_without_throw){TThread::transget_without_throw=false;db->abort_txn_local(txn);return txn_result(isRemote,0);}
        {
          ...
          string tt=intToString2(getCurrentTimeMillis2()%100000000);
          v_c_new.i_name.assign(tt.c_str());
          tbl_item(1)->tx_put(txn, Encode(str(), k), Encode(str(), v_c_new));
        }
      }
    }
  ...
  try {
    ...
    for (int i=0; i<RMW_count; i++) {
      ...
      if (isRemote&&!first) {
        first=true;
        ALWAYS_ERROR(remote_tbl_item(remote_warehouse_id)->tx_get(txn, EncodeK(k), obj_v));
        remote_tbl_item(remote_warehouse_id)->tx_put(txn, Encode(str(), k), Encode(str(), v));
      } else {
        ALWAYS_ERROR(tbl_item(1)->tx_get(txn, EncodeK(k), obj_v));
        if(TThread::transget_without_stable){TThread::transget_without_stable=false;}
        if(TThread::transget_without_throw){TThread::transget_without_throw=false;db->abort_txn_local(txn);return txn_result(isRemote,0);}
        tbl_item(1)->tx_put(txn, Encode(str(), k), Encode(str(), v));
      }
    }
    if (likely(db->commit_txn(txn)))
        return txn_result(true, 0 + (isRemote?1:0));
    ...
    for (int i=0; i<RMW_count; i++) {
      ...
      if (isRemote&&!first) {
        ...
        for (int i=0; i<batch_size; i++) {
          ...
          item_micro::value bv;
          ALWAYS_ERROR(tbl_item(1)->tx_get(txn, EncodeK(bk), obj_v));
          if(TThread::transget_without_stable){TThread::transget_without_stable=false;}
          if(TThread::transget_without_throw){TThread::transget_without_throw=false;db->abort_txn_local(txn);return txn_result(isRemote,0);}
          tbl_item(1)->tx_put(txn, Encode(str(), bk), Encode(str(), bv));
        }
      }
    }
    if (likely(db->commit_txn(txn)))
...
tpcc_worker::txn_new_order_mega()
{
  ...
  try {
    ...
    ALWAYS_ERROR(tbl_warehouse(warehouse_id)->tx_get(txn, Encode(obj_key0, k_w), obj_v));
    if(TThread::transget_without_stable){TThread::transget_without_stable=false;}
    if(TThread::transget_without_throw){TThread::transget_without_throw=false;db->abort_txn_local(txn);return txn_result(false,isRemote?1:0);}


    const district::key k_d(warehouse_id, districtID);
    ALWAYS_ERROR(tbl_district(warehouse_id)->tx_get(txn, Encode(obj_key0, k_d), obj_v));
    if(TThread::transget_without_stable){TThread::transget_without_stable=false;}
    if(TThread::transget_without_throw){TThread::transget_without_throw=false;db->abort_txn_local(txn);return txn_result(false,isRemote?1:0);}
    ...
    for (int i=0;i<batch_size;i++){
        ...
        tbl_new_order(warehouse_id)->tx_insert(txn, Encode(str(), k_no), Encode(str(), v_no));
        ret += new_order_sz;
        ...
        tbl_oorder(warehouse_id)->tx_insert(txn, EncodeK(str(), k_oo), Encode(str(), v_oo));
        ret += oorder_sz;

        const oorder_c_id_idx::key k_oo_idx(warehouse_id, districtID, customerID, k_no.no_o_id);
        const oorder_c_id_idx::value v_oo_idx(0);

        tbl_oorder_c_id_idx(warehouse_id)->tx_insert(txn, Encode(str(), k_oo_idx), Encode(str(), v_oo_idx));
    }
    ...
    for (uint ol_number = 1; ol_number <= numItems; ol_number++) {
      ...
      for (int i=0;i<batch_size;i++){
        ...
        ALWAYS_ERROR(tbl_item(1)->tx_get(txn, EncodeK(obj_key0, k_i), obj_v));
        if(TThread::transget_without_stable){TThread::transget_without_stable=false;}
        if(TThread::transget_without_throw){TThread::transget_without_throw=false;db->abort_txn_local(txn);return txn_result(false,isRemote?1:0);}
        item::value v_i_temp;
        ...
        if (WarehouseInShard(ol_supply_w_id, BenchmarkConfig::getInstance().getShardIndex())){
          ...
          ALWAYS_ERROR(tbl_stock(WarehouseGlobal2Local(ol_supply_w_id))->tx_get(txn, EncodeK(obj_key0, k_s), obj_v));
          if(TThread::transget_without_stable){TThread::transget_without_stable=false;}
          if(TThread::transget_without_throw){TThread::transget_without_throw=false;db->abort_txn_local(txn);return txn_result(false,isRemote?1:0);}
          ...
          tbl_order_line(warehouse_id)->tx_insert(txn, Encode(str(), k_ol), Encode(str(), v_ol));
          ret += order_line_sz;
      }
      ...
      if (!WarehouseInShard(ol_supply_w_id, BenchmarkConfig::getInstance().getShardIndex())) {
        ...
        remote_tbl_stock(ol_supply_w_id)->tx_get(txn, EncodeK(obj_key0, k_s), obj_v);
        ...
        // batch-write
        remote_tbl_stock(ol_supply_w_id)->tx_put(txn, EncodeK(str(), k_s), Encode(str(), v_s_new)); 
      }
...
tpcc_worker::txn_new_order()
{
  ...
  try {
    ...
    if (!g_new_order_fast_id_gen) {
      ...
      v_d_new.d_next_o_id++;
      tbl_district(warehouse_id)->tx_put(txn, Encode(str(), k_d), Encode(str(), v_d_new));
    }
    ...
    for (uint ol_number = 1; ol_number <= numItems; ol_number++) {
      ...
      if (WarehouseInShard(ol_supply_w_id, BenchmarkConfig::getInstance().getShardIndex())) {
        ...
        bool ret=remote_tbl_stock(ol_supply_w_id)->tx_get(txn, EncodeK(obj_key0, k_s), obj_v);
  ...
  try {
    ...
    for (uint d = 1; d <= NumDistrictsPerWarehouse(); d++) {
      ...
      new_order_scan_callback new_order_c;
      {
        ...
        tbl_new_order(warehouse_id)->tx_scan(txn, Encode(obj_key0, k_no_0), &Encode(obj_key1, k_no_1), new_order_c, s_arena.get());
      }
      ...
      if (unlikely(!tbl_oorder(warehouse_id)->tx_get(txn, EncodeK(obj_key0, k_oo), obj_v))) {
        if(TThread::transget_without_stable){TThread::transget_without_stable=false;}
      ...
      tbl_order_line(warehouse_id)->tx_scan(txn, Encode(obj_key0, k_oo_0), &Encode(obj_key1, k_oo_1), c, s_arena.get());
      float sum = 0.0;
      for (size_t i = 0; i < c.size(); i++) {
        ...
        tbl_order_line(warehouse_id)->tx_put(txn, *c.values[i].first, Encode(str(), v_ol_new));
      }

      // delete new order
      tbl_new_order(warehouse_id)->tx_remove(txn, Encode(str(), *k_no));
      ret -= 0 /*new_order_c.get_value_size()*/;

      // update oorder
      oorder::value v_oo_new(*v_oo);
      v_oo_new.o_carrier_id = o_carrier_id;
      tbl_oorder(warehouse_id)->tx_put(txn, EncodeK(str(), k_oo), Encode(str(), v_oo_new));
      ...
      // tbl_customer(warehouse_id)->tx_put(txn, EncodeK(str(), k_c), Encode(str(), v_c_new));

      const customer::key k_c_new(warehouse_id, d + 200, c_id);
      ALWAYS_ASSERT(tbl_customer(warehouse_id)->tx_get(txn, EncodeK(obj_key0, k_c_new), obj_v));
      if(TThread::transget_without_stable){TThread::transget_without_stable=false;}
      if(TThread::transget_without_throw){TThread::transget_without_throw=false;db->abort_txn_local(txn);return txn_result(false,0);}
      ...
      tbl_customer(warehouse_id)->tx_put(txn, EncodeK(str(), k_c_new), Encode(str(), v_c_b_new));
    }
...
if (TThread::get_is_micro()) { 
  ...
  try {
    ...
    tbl_warehouse(warehouse_id)->tx_put(txn, Encode(str(), k_w), Encode(str(), v_w_new));
    ...
    if (RandomNumber(r, 1, 100) <= 60) {
      ...
      if (WarehouseInShard(customerWarehouseID, BenchmarkConfig::getInstance().getShardIndex())) {
        ALWAYS_ERROR(tbl_customer(WarehouseGlobal2Local(customerWarehouseID))->tx_get(txn, EncodeK(obj_key0, k_c), obj_v));
        if(TThread::transget_without_stable){TThread::transget_without_stable=false;counter_payment_failed+=1;}
        if(TThread::transget_without_throw){TThread::transget_without_throw=false;db->abort_txn_local(txn);return txn_result(false,isRemote?1:0);}
      } else {
        ALWAYS_ERROR(remote_tbl_customer(customerWarehouseID)->tx_get(txn, EncodeK(obj_key0, k_c), obj_v));
      }
    ...
    if (WarehouseInShard(customerWarehouseID, BenchmarkConfig::getInstance().getShardIndex())) {
      ...
      remote_tbl_customer(customerWarehouseID)->tx_put(txn, EncodeK(str(), k_c), Encode(str(), v_c_new));
    }
    ...
    tbl_history(warehouse_id)->tx_insert(txn, EncodeK(str(), k_h), Encode(str(), v_h));
    ret += history_sz;
    ...
    if (strncmp(v_c.c_credit.data(), "BC", 2) == 0) {
      ...
      if (WarehouseInShard(customerWarehouseID, BenchmarkConfig::getInstance().getShardIndex())) {
        ...
        remote_tbl_customer(customerWarehouseID)->tx_put(txn, EncodeK(str(), k_c), Encode(str(), v_c_data_new));
      }
    }
...
tpcc_worker::txn_order_status()
{
  ...
  try {
    ...
    if (RandomNumber(r, 1, 100) <= 60) {
      ...
      static_limit_callback<NMaxCustomerIdxScanElems> c(s_arena.get(), true); // probably a safe bet for now
      tbl_customer_name_idx(warehouse_id)->tx_scan(txn, Encode(obj_key0, k_c_idx_0), &Encode(obj_key1, k_c_idx_1), c, s_arena.get());
      ALWAYS_ERROR(c.size() > 0);
      INVARIANT(c.size() < NMaxCustomerIdxScanElems); // we should detect this
      int index = c.size() / 2;
      if (c.size() % 2 == 0)
    ...
    if (g_order_status_scan_hack) {
      ...
      const oorder_c_id_idx::key k_oo_idx_1(warehouse_id, districtID, k_c.c_id, numeric_limits<int32_t>::max());
      {
        ...
        tbl_oorder_c_id_idx(warehouse_id)->tx_scan(txn, Encode(obj_key0, k_oo_idx_0), &Encode(obj_key1, k_oo_idx_1), c_oorder, s_arena.get());
      }
      ...
      tbl_oorder_c_id_idx(warehouse_id)->tx_rscan(txn, Encode(obj_key0, k_oo_idx_hi), nullptr, c_oorder, s_arena.get());
    ...
    tbl_order_line(warehouse_id)->tx_scan(txn, Encode(obj_key0, k_ol_0), &Encode(obj_key1, k_ol_1), c_order_line, s_arena.get());
...
tpcc_worker::txn_stock_level()
{
  ...
  //    auto ret=tbl_stock(warehouse_id)->tx_get(txn, EncodeK(obj_key0, k_s), obj_v, nbytesread);
  //    if (!ret){
  //      cnt++;
  ...
  try {
    ...
    const order_line::key k_ol_1(warehouse_id, districtID, cur_next_o_id, 0);
    {
      ...
      tbl_order_line(warehouse_id)->tx_scan(txn, Encode(obj_key0, k_ol_0), &Encode(obj_key1, k_ol_1), c, s_arena.get());
    }
    {

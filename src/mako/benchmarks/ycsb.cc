...
public:
  ...
  txn_read()
  {
    ...
    try {
      ...
      ALWAYS_ASSERT(tbl->tx_get(txn, u64_varkey(k).str(obj_key0), obj_v));
  ...
  txn_rmw()
  {
    ...
    try {
      ...
      ALWAYS_ASSERT(tbl->tx_get(txn, u64_varkey(key).str(obj_key0), obj_v));
      computation_n += obj_v.size();
      tbl->tx_put(txn, obj_key0, str().assign(YCSBRecordSize, 'c'));

...
// Variadic `Log_*` wrappers — deliberately OUTSIDE src/rrr.
//
// These five templates are the last parameter-pack construct that used
// to live inside the srpc/rrr tree, and a pack is the one blocker no
...
// comment, not code). So rather than pin un-DSL-able C++ inside rrr,
// the wrapper lives on the CONSUMER's side of the boundary, exactly like
// `janus::Command` and `janus::QuorumEventBase`.
//
// What stayed in rrr is the whole logging IMPLEMENTATION, which is DSL:
// `rrr::log_line` (level filter, tag, basename, timestamp, decoration,
// sink routing) and `rrr::log_level_tag`. These wrappers add nothing but

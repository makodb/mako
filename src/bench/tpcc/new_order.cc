...
//         Log_debug("TPCC_NEW_ORDER, piece: {}", TPCC_NEW_ORDER_1);
...
//         Log_debug("TPCC_NEW_ORDER, piece: {}", TPCC_NEW_ORDER_2);
...
//         Log_debug("TPCC_NEW_ORDER, piece: {}", TPCC_NEW_ORDER_3);
         int32_t oi = 0;
...
//       PROC {
//         verify(cmd.input.size() >= 3);
//         Log_debug("TPCC_NEW_ORDER, piece: {}", TPCC_NEW_ORDER_4);
...
//                 cmd.input.at(TPCC_VAR_O_ID),   // o_id
  ...
  for (int i = (0); i < (1000); i++) {
    ...
    RegP(TPCC_NEW_ORDER, TPCC_NEW_ORDER_RI(i),
         ...
         LPROC {
           verify(cmd.input.size() >= 1);
           Log_debug("TPCC_NEW_ORDER, piece: {}",
                     TPCC_NEW_ORDER_RI(i));
    ...
    RegP(TPCC_NEW_ORDER, TPCC_NEW_ORDER_WS(i),
         ...
         LPROC {
           ...
           Log_debug("TPCC_NEW_ORDER, piece: {}", TPCC_NEW_ORDER_WS(i));
           mdb::Row *r = NULL;
           mdb::MultiBlob mb(2);
           mb[0] = cmd.input[TPCC_VAR_I_ID(i)].get_blob();
           mb[1] = cmd.input[TPCC_VAR_S_W_ID(i)].get_blob();
    ...
    RegP(TPCC_NEW_ORDER, TPCC_NEW_ORDER_WOL(i),
         ...
         LPROC {
           ...
           Log_debug("TPCC_NEW_ORDER, piece: {}", TPCC_NEW_ORDER_WOL(i));

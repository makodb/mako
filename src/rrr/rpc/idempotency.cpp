...
bool IdempotencyCache::remove(const IdempotencyKey& key) const {
    ...
    while (rusty::detail::deref_if_pointer_like(i) < rusty::detail::deref_if_pointer_like(n)) {
        if ((rusty::detail::deref_if_pointer_like([&](auto&& __r) -> decltype(auto) { if constexpr (requires { (__r.client_id); }) { return (__r.client_id); } else if constexpr (requires { (__r.client_id_field); }) { return (__r.client_id_field); } else if constexpr (requires { ((*__r).client_id); }) { return ((*__r).client_id); } else { return ((*__r).client_id_field); } }([&](auto&& __r) -> decltype(auto) { if constexpr (requires { (__r.key); }) { return (__r.key); } else if constexpr (requires { (__r.key_field); }) { return (__r.key_field); } else if constexpr (requires { ((*__r).key); }) { return ((*__r).key); } else { return ((*__r).key_field); } }(guard[i]))) == rusty::detail::deref_if_pointer_like(key.client_id)) && (rusty::detail::deref_if_pointer_like([&](auto&& __r) -> decltype(auto) { if constexpr (requires { (__r.sequence); }) { return (__r.sequence); } else if constexpr (requires { (__r.sequence_field); }) { return (__r.sequence_field); } else if constexpr (requires { ((*__r).sequence); }) { return ((*__r).sequence); } else { return ((*__r).sequence_field); } }([&](auto&& __r) -> decltype(auto) { if constexpr (requires { (__r.key); }) { return (__r.key); } else if constexpr (requires { (__r.key_field); }) { return (__r.key_field); } else if constexpr (requires { ((*__r).key); }) { return ((*__r).key); } else { return ((*__r).key_field); } }(guard[i]))) == rusty::detail::deref_if_pointer_like(key.sequence))) {
            (*guard).remove(i);
            return true;
        }

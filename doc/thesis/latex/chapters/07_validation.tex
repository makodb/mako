...
replicas on one machine and communicate over local TCP/IP. That choice isolates
Mako's replication backend, which is the system layer being evaluated here.
...
The tests target ordinary crash-stop and rejoin behavior, not Byzantine
behavior, malicious replicas, disk corruption, or arbitrary network partitions.
The persistence work in Chapter 6 validates that modeled persistence work is
counted and reported; physical-media restart testing is a separate storage
recovery campaign.
...
rules. The replicated log therefore remains available at the Raft layer.
Serving new transaction workload from that backup leader is a
client/workload-routing question because the benchmark workers normally submit
through the leader-side submission role. This thesis validates the Raft
placement and recovery mechanism that the Mako integration needs.
...
Those tests establish that the Raft layer is a real consensus backend rather
than a benchmark shortcut.
...
The final replay-lag validation adds a more direct accounting check. It counts
records submitted by the leader and records replayed by the slower follower at
the end of the run. This is stronger than a throughput-rate comparison alone:
it checks whether leader-accepted records accumulated unreplayed, and whether the
ReplayPool queue drained after the workload stopped submitting new records.

\section{Persistence and Measurement}

Persistence validation focuses on measurement integrity. It checks that
simulated-disk instrumentation is present and interpretable. Runs with
...
relying on throughput alone. The evidence is the accounting and fairness check
needed for the controlled storage model.
...
\addlinespace
Replay lag and drain accounting &
Leader-submitted records are compared with records replayed by the slower follower, and ReplayPool queue state is checked after the run. &
ReplayPool prevents leader-accepted records from accumulating unreplayed in the measured Single-Raft run. \\
\addlinespace
...
\addlinespace
Replay-lag proof & End-of-run leader-submitted records, follower-replayed records, replay gap, queue peak, and drain time are recorded. & Direct evidence that 1:1 ReplayPool drains the follower work counted by the benchmark's replay accounting path. \\
\addlinespace
...
produced the corresponding graph. Appendix A summarizes the evaluation metrics
and maps each major claim to its supporting evidence.

...
public:
    ...
    void test_single_key_contention() {
        ...
        for (size_t i = 0; i < 10; i++) {
            ...
            try {
                tx_put(table, txn, shared_key, value);
                db->commit_txn(txn);
                commits++;
        ...
        if (original_worker_id_ == 0) {
            ...
            std::string value;
            try {
                bool exists = tx_get(table, txn, shared_key, value);
                db->commit_txn(txn);
                if (exists) {
        ...
        if (original_worker_id_ == 0) {
            ...
            for (int group = 0; group < 10; group++) {
                for (size_t i = 0; i < 5; i++) {
                    ...
                    std::string value;
                    try {
                        bool exists = tx_get(table, txn, key, value);
                        db->commit_txn(txn);
                        if (exists) {
                            total_existing_keys++;
                        }
    ...
    void test_cross_shard_contention() {
        ...
        for (size_t i = 0; i < 10; i++) {
            ...
            try {
                tx_put(table, txn, shared_local_key, value);
                tx_put(table, txn, shared_remote_key, value);
                db->commit_txn(txn);
                commits++;
        ...
        if (original_worker_id_ == 0) {
            ...
            try {
                bool local_exists = tx_get(table, txn, local_key, local_value);
                db->commit_txn(txn);
                if (local_exists) {
...

            {
                ...
                try {
                    bool remote_exists = tx_get(table, txn2, remote_key, remote_value);
                    db->commit_txn(txn2);
                    if (remote_exists) {

    ...
    try {
        ...
        for (size_t i = 0; i < request->num_ops; i++) {
            ...
            if (op.op == TXN_OP_GET) {
                // GET operation
                tl_val_buf.clear();
                mako::Status s = mbta_sharded_Get(g_table, txn, tl_key_buf, tl_val_buf);
                if (s.ok()) {
                    result.success = true;
                    if (!tl_val_buf.empty()) {
                ...
                mako::Status s = mbta_sharded_Put(g_table, txn, tl_key_buf, encoded_vals[i]);
                result.success = s.ok();
                if (!s.ok()) {
                    all_success = false;
                }
                ...
                mako::Status s = mbta_sharded_Delete(g_table, txn, tl_key_buf);
                result.success = s.ok();

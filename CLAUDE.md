...
The `rrr` Rust package is rooted at `src/rrr/Cargo.toml`. The sixteen modules
...
The other 22 named modules still own 380 inline `RUSTYCPP_RUST` blocks, so a
successful Cargo build proves only the current sixteen-module coverage, not full
...
sixteen canonical `rrr` modules, edit its `src/rrr/src/*.rs` source directly.

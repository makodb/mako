...
The `rrr` Rust package is rooted at `src/rrr/Cargo.toml`. The nine modules
...
The other 29 named modules still own 419 inline `RUSTYCPP_RUST` blocks, so a
successful Cargo build proves only the current nine-module coverage, not full
...
nine canonical `rrr` modules, edit its `src/rrr/src/*.rs` source directly.

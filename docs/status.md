...
The former standalone Janus, Mencius, SNOW/RO6, Extern-C, 2PL, Rule, and TAPIR
protocol implementations are retired and are not supported configuration
options. The internal memdb `Txn2PL` base remains because OCC and nested
transactions share its storage and query machinery. Generic Jetpack recovery
code remains as a legacy subsystem pending a separate audit; it is not a
supported replacement for the retired Rule protocol.

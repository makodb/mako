...
**Routing (the mixed strategy).** The **partition table is the single routing authority** for governed tables. TPC-C does not keep a parallel policy plane: at workload init it *expresses* its warehouse sharding as partition-table entries — per-table split points at warehouse-key prefixes, each segment owned by that warehouse's shard. `compute_shard_for_key_value` consults the partition table for governed tables (building the prefix key from the extracted warehouse id); the legacy `ShardingPolicyCache` remains only as the fallback for ungoverned tables. A warehouse migration is then an ordinary range migration — same fence, drain, copy, checksum, `split_and_reassign` cutover — and routing follows automatically on config reload.

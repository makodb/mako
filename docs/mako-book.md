...
- **Master-owned freeze.** `lock_range()` itself freezes `[lo,hi)` on the SOURCE participant — locally into the process-global `MigrationGuard` (which the non-txn write handler enforces with `SERVER_BUSY`), remotely via one `FreezeRange` RPC. Abort unfreezes (source resumes). **Commit deliberately does not unfreeze**: the source no longer owns the range, and the standing fence keeps rejecting stale-routed writers (they retry, reload config, and land on the destination) — closing the cutover race without an ownership check on the server. The destination is never frozen, so it serves immediately. (Transactional migration — coordinating with in-flight Silo transactions — and durable/recoverable migration state remain deliberately out of scope.)

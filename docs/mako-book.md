...
The live wiring is BUILT (`src/mako/cluster_bootstrap.cc`, gated by `MAKO_CLUSTER_CONFIG=1`, verified end-to-end by `ci/ci.sh shardMigrationAdmin` on the replicated multi-process bed):

- **Data plane over RPC.** Every shard *leader* serves a `ShardDataService` at its mako shard port `+ kShardDataPortDelta` — addresses derive from the shared mako transport config, which names every shard (the per-shard paxos config cannot address peers). Every RPC names its **table**; the service resolves it through the shard's `ShardDataCatalog` (`src/mako/shard_data_plane.cc`), which creates named tables on demand as *standalone* fixed-id engine indexes (ids 9100+ — system tables must never consume `open_index`'s per-process sequential ids, which would skew the workload's cross-shard table-id arithmetic), registers each in the table registry (so the non-txn write handler resolves an op's table name for the per-table freeze), and gates every op with engine thread-registration (rrr handler threads are not otherwise registered). One service per shard covers all migratable tables. *(Migrating the workload's own `open_tables` (txn tables) remains gated on transactional migration.)*
- **The standing master.** Shard 0's leader hosts the ONE long-lived `ShardMaster` (`g_shard_master`), created over the authoritative `ConfigManager` and the routing cache. Its own data plane attaches as participant 0 at bootstrap; remote participants attach lazily (a `RemoteShardData` dialed on the first `Migrate` that names them).
- **Admin trigger.** `mako_admin migrate <addr> <table> <lo> <hi> <src> <dst>` fires the `MigrationAdmin.Migrate` RPC — served on its **own** rrr server on shard 0 (`+kMigrationAdminPortDelta`), never on the data-plane server: a blocking `Migrate` must not occupy the worker that serves this shard's scans (a destination's pull connects *back* to shard 0 mid-migration). The handler drives the standing master: begin → background copy (source live) → `lock_range` → catch-up copy → checksum-gated `final_sync` → `commit_migration` (which calls `split_and_reassign` and version-bumps).
- **Destination-driven copy.** The bulk copy never transits the master: `copy_range_from` on a remote destination sends **one `PullRange` control RPC**, and the destination shard pulls `[lo,hi)` directly from the source's `ShardDataService` in chunked `ScanRange` batches (participants self-identify as pull endpoints via `service_addr`/`service_table`). Only control flows through the coordinator: freeze/unfreeze, checksums, the 2PC votes, `DropRange`, and the config publish. A failed pull latches the participant's `faulted()` bit — the same gate that makes a dead participant abort the migration instead of vacuously committing — and the admin evicts faulted cached handles so retries reconnect fresh.
- **Master-owned freeze.** `lock_range()` itself freezes `[lo,hi)` on the SOURCE participant — locally into the process-global `MigrationGuard` (which the non-txn write handler enforces with `SERVER_BUSY`), remotely via one `FreezeRange` RPC. Abort unfreezes (source resumes). **Commit deliberately does not unfreeze**: the source no longer owns the range, and the standing fence keeps rejecting stale-routed writers (they retry, reload config, and land on the destination) — closing the cutover race without an ownership check on the server. The destination is never frozen, so it serves immediately. (Durable/recoverable migration state remains deliberately out of scope until the config store itself is persistent.)

#### Transactional migration (design of record, v1)

**The motivating race — which the non-txn path shares.** The freeze check is check-then-act: a writer can pass `is_frozen` *before* the coordinator installs the fence and commit its (one-op or multi-op) transaction *after* the catch-up copy and checksum scans — an acknowledged write destroyed by `drop_range` at cutover. The checksum gate catches only the interleavings where the late write lands before the source's checksum scan; it is a copy-integrity check, **not** a concurrency fence. The fix below is therefore not a "transactional feature": it is the correct freeze for *every* migration, and a non-txn op is just a one-op Silo transaction that gets covered for free.

**Fence (at write staging).** The authoritative freeze check moves *inside* the transaction, at the point where a write enters the write-set — the mbta txn write verbs (which have `(table, key)` in hand; the commit loop's `TransItem`s do not carry string keys) and the server-side participant lock/stage path (`BatchLock` carries `table_id` + keys). A hit aborts the transaction with a retryable status; workers already retry aborted transactions, and the retry lands on the destination once routing reloads. Hot-path cost: one relaxed atomic load (`fences_active == 0` → no per-key work) when no migration is running. `RunNontxnOp`'s early check remains as a fast-path courtesy; correctness no longer depends on it.

**Drain (a staged-writer counter — NOT Silo's epochs).** The staging check still races the fence install; the drain closes it. The first design reused `global_epochs.active_epoch` — and the racing-writer test disproved it immediately: the **idle-participant invariant** (2PC helper threads keep an `in_progress`-but-empty txn between requests) pins their `tinfo` epochs forever, `active_epoch` never passes the fence capture, and with the drain inert the test caught a genuine lost acknowledged write. The shipped drain instead counts **staged writers**: entering any staging kernel registers the txn (one atomic, once per txn via a thread-local flag) *before* checking the fence — rolling back if the fence rejects its first write — and `Transaction::stop` (commit *and* abort) closes the registration. The L3 one-op kernels (non-txn put/insert/remove) need a **different registration shape**: the engine call retries OCC aborts *internally*, so a per-txn registration closes at the first attempt's `stop` and the internal retry runs uncounted and unfenced — it can commit on the source after the drain returned and the catch-up copy scanned (a lost acknowledged write, caught live by the racing-writer test *twice*: first against the epoch drain, then against the per-attempt counting). Those kernels instead hold an RAII registration (`MigrationStagedWriter`) spanning the whole logical op — every internal retry included — with the fence still checked per outer attempt. And the wait itself must be a **generation watermark, not quiescence**: "wait for zero staged writers" never terminates on a busy shard (live bed: TPC-C at ~5k txn/s kept the counter nonzero across the whole 3s window; every migration aborted on drain timeout). Registrations land in the bucket of the current fence generation; the drain bumps the generation and waits for **every bucket except its own bump's** to empty — post-bump writers can't stage into the fenced range (their fence check follows registration), and writers to other ranges never block a drain they have nothing to do with. Three refinements, each pinned live: (1) **16 generation buckets, not two parities** — sequential drains alternate parities, so a still-open txn from two bumps ago sat in the *not-waited* parity and its install leaked past the drain (single-payment-lag divergence on every table after the first); with 16 buckets a straggler aliases into the unwaited bucket only after 16 bumps while its txn stays open. This also makes *retrying* the drain-begin RPC safe — an extra bump just burns a bucket — where the parity scheme forbade it. (2) **Exact per-table counter identity** — hash-mod slots collided `customer_1` with `order_line_8`, and a hot never-migrating table's churn kept the fenced table's drain from ever emptying; slots now claim their full 64-bit name hash (open addressing, permanent claims; a genuine collision degrades to conservative sharing). (3) **The participant txn boundary** — 2PC helper txns end via `shard_reset` (a bare `start_transaction`), never `Transaction::stop`, so their registrations leaked forever (four gen-0 holders stalled every later drain at any budget); `shard_reset` now runs the same txn-done hook, on the helper thread that staged (installs precede `shard_reset`, so the drain still covers them). Ordering gives the proof: a writer is counted before it can stage, and a post-fence writer aborts before staging, so once the fence is up the counter only drains — **zero ⟺ every pre-fence staged-write txn has finished**. Idle empty participants never count (they cannot hold pre-fence staged writes), so the pinning problem vanishes. The raw-write `remove` asymmetry (no txn, no `stop`) keeps the plain fence check — it has no staged window to wait on. The master's sequence: `lock_range` → `drain_writes` on the source (local counter wait, or a `DrainWrites` begin/poll loop; timeout → abort) → catch-up copy → `final_sync` → commit.

**Copies are mirrors; folds are jobs; control RPCs retry to deadlines.** Three more shapes the live bed forced. *Mirror copy*: a put-only copy never converges a table that takes deletes (delivery consumes `new_order` rows between the phase-1 copy and the fence; the stale destination rows survive every re-copy and the checksum gate refuses forever) — `copy_range_from` walks both sides chunked and deletes in-range destination keys the source no longer has, safe in every phase because the destination never serves the range before commit (a short source batch with `faulted()` set stops the mirror rather than wiping the tail). *Fold jobs*: `Checksum` and `VerifyRange` fold 100k+ rows and outrun rrr's ~1s client timeout under load, and a retried synchronous fold stacks another full scan each time — a self-inflicted saturation spiral that stretches 2PC tails past the drain budget; both are BEGIN/POLL jobs on one server-side compute thread (`VerifyRange` compares `expected` at reply time). *Deadline retries*: a bulk copy saturates the participant's rrr workers for 10s+ stretches and every control RPC times out; begin/poll clients retry to their deadline and latch `faulted` only on total silence for the window. And the freeze must land under the **physical** table name — the staging fence's lookup key — not the wire name: `DoFreezeRange` delegates to the catalog-resolved participant (which installs the physical, widened entry) and keeps the raw wire-name entry for the RPC contract; freezing only the wire name (`wh:6:customer` vs `customer_1` queries) fenced *nothing*, which hid behind small tables' millisecond verify windows until the big tables made it deterministic.

**The full warehouse move (as built).** All 11 governed tables of a warehouse migrate live (~645k rows mid-TPC-C), with two workload-facing rules. *Scans*: point ops self-dispatch on the resolved handle's `is_remote`, but multi-row range scans have no remote leg — a remote proxy index stages point ops and holds no rows, so a `tx_scan` against one reads a clean empty and the workload panics on a must-find miss. Scans got the moved-read gate (retryable abort, like point gets since T0); payment's by-name lookup branches on the resolved handle itself (the group oracle `WarehouseInShard` keys off the *customer* segment and can disagree with a per-table resolve mid-migration); and the scan-bearing home transactions (delivery, order_status, stock_level) *skip a departed home* — the bench's stand-in for client rerouting, while payment/new_order keep hammering the moved warehouse through the remote point-op legs and the server-side one-pick scan. *Ordering*: the bed migrates **customer first** on the way out (the group signal flips before any scanned table moves) and **customer last** on the way back — the mirror image, so the returning home's scan users resume only when everything scannable is already home. The return leg is also the remote-destination path from the master's seat: the copy runs as PullRange (the destination pulls; `DoPullRange` delegates to the destination participant's `copy_range_from`, so wh-spec widening and mirroring apply, and the pull identity is the catalog-resolvable *spec*, not the per-process physical name), the prepare vote is the remote `VerifyRange` job, and `commit_migration` clears the returning owner's stale MOVED fence so it serves immediately.

**Moved-range ownership guard.** Post-cutover, a stale-routed *read* on the source must not return a clean miss ("key absent") for data that moved. Shedding the range upgrades the source's fence entry from *frozen* (writes rejected, reads served — the mid-migration contract) to *moved* (reads **and** writes rejected with the retryable status) — clients retry, reload config, and land on the new owner. The upgrade rides `DropRange` itself (losing the data *is* losing ownership), so no new RPC.

**Routing (the mixed strategy, as built).** The **partition table is the single routing authority** for governed tables. TPC-C does not keep a parallel policy plane: at workload init it *expresses* its warehouse sharding as partition-table entries. The wrinkle discovered during build: TPC-C's physical row keys carry the shard-**local** warehouse id (`WarehouseGlobal2Local` at every remote access), so key bytes cannot identify the global warehouse — the **physical index identity** does ("customer_0", "customer_remote_5": one index per warehouse per table). Routing therefore goes through a **routing alias** registered at `open_index` time: physical `table_id` → (logical table `"customer"`, fixed routing key = 4-byte big-endian **global** warehouse id, `warehouse_route_key`). `compute_shard_for_key` consults the alias first — when the logical table is governed (map mode + partition present), the op routes by the index's alias key and *ignores its key bytes*; ungoverned tables keep the legacy `ShardingPolicyCache`/table-id arithmetic. The workload's local-vs-remote decision (`WarehouseInShard`) asks the same oracle (`route_shard_for_warehouse`, −1 = ungoverned → static layout). Seeding: `seed_warehouse_partitions` installs per-table segments with split points at `warehouse_route_key` boundaries reproducing the legacy layout, invoked by the workload on the shard-0 master (`tpcc_seed_warehouse_partitions_if_master`, dormant unless `MAKO_SHARDING_MODE=map`); the 11 warehouse-partitioned tables are governed, `item` (read-only, replicated per shard) deliberately is not. A warehouse migration is then an ordinary range migration — same fence, drain, copy, checksum, `split_and_reassign` cutover over `[warehouse_route_key(w), warehouse_route_key(w+1))` — and both remote-op routing and the workload's warehouse oracle flip on config reload. Routing granularity equals physical-index granularity by construction (an index spanning several warehouses gets its first warehouse's key).

**Verify-before-building checklist.** (1) The L3 one-op commit on a replicated leader appends to the log (destination's pulled rows and the source's drops must replicate). (2) Follower replay lands in the same tables — true for workload tables (symmetric `open_index` ids), which is why v1 migrates those, not catalog tables (per-process 9100+ ids). (3) Every write-capable path (mode-0 one-op, workers, mode-1 participants) runs on epoch-registered threads, or the drain has a blind spot.

**Phasing.** T0 moved-range guard + the three verifications → T1 staging fence → T2 drain + a racing-writer test (a client hammering the migrating range through the real server path *while* the fence installs; every acknowledged write must be present on exactly the final owner) → T3 partition-governed txn routing + TPC-C warehouse seeding (equivalence check: seeded partitions reproduce the policy placement, so the existing bed's abort ratios must not move) → T4 workload-table data plane + a live mid-TPC-C warehouse migration. Master-restart recovery stays deferred until the config store persists.

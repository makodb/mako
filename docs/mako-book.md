...
- **Data plane over RPC.** Every shard *leader* serves a `ShardDataService` over a real engine-backed index (`__mako_kv__`, opened at bootstrap) at its mako shard port `+ kShardDataPortDelta` — addresses derive from the shared mako transport config, which names every shard (the per-shard paxos config cannot address peers). The `ShardData` handed to the service self-registers each calling thread with the storage engine (`src/mako/shard_data_plane.cc`), because rrr handler threads are not otherwise engine-registered. *(Current scope: one designated migratable index per shard; per-`table_id` service coverage of `open_tables` is the follow-up.)*

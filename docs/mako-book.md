...
**Drain (read-only reuse of Silo's epochs).** The staging check still races the fence install; the drain closes it. After freezing, capture `fence_epoch = Transaction::global_epochs.global_epoch`; wait until `global_epochs.active_epoch > fence_epoch` — the field's own comment is the guarantee ("no thread is before this epoch"; maintained by the existing `epoch_advancer`, 100ms cadence, so the drain costs one-to-two ticks). Case analysis: a txn beginning **after** the capture sees the fence at staging (mutex ordering) and aborts; a txn beginning **at/before** the capture has begin-epoch ≤ `fence_epoch` and is waited out. Every write is one of the two, so after freeze → capture → drain the range is provably quiescent; then catch-up copy → checksum → cutover. No new epoch machinery: two reads of existing counters. (This is Silo's *transaction* epoch, not the replication term that shares the name.) The master's sequence gains one step: `lock_range` → `drain_writes` on the source (local wait, or one `DrainWrites` RPC) → catch-up copy → `final_sync` → commit.

...
srpc source (excluding tests), ~25.3k non-blank C++ lines:
...
Inventory generated by `tools/srpc-inventory.py`; outputs at `docs/srpc-inventory.csv` (one row per top-level class/struct/enum/union) and `docs/srpc-inventory.md` (bucket summary + top-N tables).

Headline buckets for top-level decls (post-DragonBall deletion,
2026-06-11; see `docs/srpc-inventory.md` for the live numbers):

| Bucket            | Decls | LOC | % of LOC |
|-------------------|------:|----:|---------:|
| trivial           |    1  |  20 |    0.2%  |
| trivial-blocked   |    8  | 482 |    5.5%  |
| refactor-then-dsl |   29  |2,204|   25.2%  |
| needs-transpiler  |   44  |4,420|   50.5%  |
| boundary          |    3  | 117 |    1.3%  |
| already-dsl       |  171  |1,498|   17.1%  |
...
in `src/srpc/src/reconnect_policy.rs`; its explicit factory preserves the
...
3. Build, run srpc unit tests + `rpcbench` smoke, commit.
...
Rough floor: **~2,000 manual C++ lines** (~8% of current srpc). Anything below that needs DSL-grammar extensions, which is a different project.
...
1. **Phase 0 inventory** — one CSV listing every top-level decl in srpc and its triage bucket. ~half a day.

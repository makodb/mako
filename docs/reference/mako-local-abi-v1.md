# Mako local C ABI revision 1 contract

This document is the normative caller contract for the revision-1
`mako_local_*` C ABI declared in the
[public header](../../src/mako/storage/mako_local_abi.h). The implementation
reports `MAKO_LOCAL_ABI_VERSION == 1`. New semantics require a later ABI
revision or a negotiated feature bit; the numeric status assignments listed
here are reserved and must not be renumbered.
...
optional surface. Revision 1 defines these feature bits:

| Feature | Guarantee when present |
| --- | --- |
| `MAKO_LOCAL_FEATURE_POINT_TRANSACTIONS` (bit 0) | The point-transaction surface is available. |
| `MAKO_LOCAL_FEATURE_READ_MY_WRITES` (bit 1) | Point reads and repeated point mutations observe the transaction's staged state. |
| `MAKO_LOCAL_FEATURE_OPACITY` (bit 2) | The linked engine was built with opacity. Absence does not weaken committed-transaction serializability. |
| `MAKO_LOCAL_FEATURE_TRANSACTIONAL_SCANS` (bit 3) | Forward and reverse scans participate in the transaction. |
| `MAKO_LOCAL_FEATURE_SCAN_READ_MY_WRITES` (bit 4) | Scans merge the transaction's staged point mutations. |
| `MAKO_LOCAL_FEATURE_TEST_COMMIT_OBSERVER` (bit 5) | The test-only commit-observer functions are available. |
| `MAKO_LOCAL_FEATURE_TEST_CLEANUP_FAILURES` (bit 6) | The test-only cleanup-failure arm and clear functions are available. |
| `MAKO_LOCAL_FEATURE_TEST_TIMESTAMP_CLOCK` (bit 7) | The test-only process-global physical-clock set and clear functions are available. |
...
The commit-observer, cleanup-failure, and timestamp-clock functions are usable
only when their respective feature bit is present. In a build without the
corresponding bit, the functions return `MAKO_LOCAL_FEATURE_UNAVAILABLE` before
validating their arguments. Test-only symbols remain exported in production
profiles so the exported-symbol set and link compatibility do not depend on
whether any test feature was compiled in. The content fingerprint still
distinguishes those profiles.

## Revision-1 timestamp contract

Revision 1 adds the stable `mako_timestamp_v1` value from
[`mako_timestamp.h`](../../src/mako/storage/mako_timestamp.h). Its native C
layout is exactly 16 bytes: `physical_us` at byte 0, `logical` at byte 8, and
`origin` at byte 12. Compare timestamps as the unsigned tuple
`(physical_us, logical, origin)`. Persistent and wire formats encode those
fields in big-endian tuple order; callers must not persist the native struct by
copying its host-endian bytes.

The first single-machine allocator emits a 44-bit millisecond physical reading
multiplied by 1,000 into the microsecond field, uses a 19-bit logical counter,
and fixes `origin` to 1. A revision-1 timestamp is valid when `origin` is
nonzero; `physical_us` and `logical` may otherwise use their full public field
widths. The recovery-floor operation normalizes values wider or finer than the
local allocator can mint, or returns `TIMESTAMP_EXHAUSTED` when no greater
local 44/19 value exists.
`mako_local_timestamp_origin()` reports that process-wide origin. The fixed
origin is not a distributed lease and is valid only for this milestone's one
allocator and one recovered cache namespace per process.

For a write transaction, `mako_local_txn_commit_with_hook()` acquires the
complete STO write set, completes local read and predicate validation,
allocates the timestamp, and then invokes the hook before installing a write.
The hook sees the exact
nonzero timestamp while the write locks remain held. Returning zero definitely
aborts. A successful hook transfers the caller's prepared cache-log obligation
into the irrevocable native install path; the caller must therefore make the
hook bounded, allocation-free, and free of I/O or unwinding.

Recovery calls `mako_local_advance_mako_timestamp_past()` before admitting
workers. A successful call guarantees that every later timestamp is greater
than the supplied nonzero value. Smaller observations never move the clock
backward, and exhaustion returns `TIMESTAMP_EXHAUSTED`.
...
| 14 | `MAKO_LOCAL_VALUE_TOO_LARGE` | A table name, key, value, or scan bound exceeded its revision-1 limit. |
...
An unknown status returned while a transaction might be active is
terminal-uncertain. Do not attempt commit. Apply the same one-shot owner-thread
destroy procedure as for `WORKER_POISONED`, and retire the worker if cleanup
cannot be proved. Any future ABI revision must set the authoritative TLS
quarantine before returning whenever cleanup is not proved complete. The safe
Rust table-open entry point repeats
...
Revision 1 retains revision 0's conditional all-output-pointer rule rather
than initializing the non-null
...
`mako_local_db_options` is an append-only input structure. A revision-1 caller
...
are ignored, so a newer caller can pass its larger structure to this
revision-1 implementation when the common flags remain supported.
...
## Binary slices and revision-1 limits
...
| `mako_local_abi_version` | Returns `MAKO_LOCAL_ABI_VERSION` (currently 1). | Scalar result; no ownership or transaction effect. |
| `mako_local_feature_bits` | Returns the linked build's feature mask. | Scalar result; no ownership or transaction effect. |
| `mako_local_timestamp_origin` | Returns the process-wide timestamp-authority origin (currently 1). | Nonzero scalar result with process lifetime; no ownership or transaction effect. It is a single-machine identity, not a distributed lease. |
...
| `mako_local_advance_mako_timestamp_past` | `OK`, `INVALID_ARGUMENT`, `TIMESTAMP_EXHAUSTED`. | A null timestamp or any timestamp with origin zero is invalid. Any valid revision-1 timestamp is accepted; its origin, physical precision, and logical width need not match a locally minted value. A successful call monotonically advances the process clock past the supplied tuple; smaller observations do not move it backward. No transaction effect. |
| `mako_local_test_set_timestamp_physical_ms` | With `MAKO_LOCAL_FEATURE_TEST_TIMESTAMP_CLOCK`: `OK`, `INVALID_ARGUMENT`. Without it: `FEATURE_UNAVAILABLE`. | Freezes the process HLC's physical candidate at a representable Unix-millisecond value, including zero. Test-only process-global state; no transaction transition. |
| `mako_local_test_clear_timestamp_physical_ms` | With `MAKO_LOCAL_FEATURE_TEST_TIMESTAMP_CLOCK`: `OK`. Without it: `FEATURE_UNAVAILABLE`. | Clears the process-global physical-clock override and restores the production clock source. Test-only; no transaction transition. |
...
| `mako_local_db_close` | `OK`, `BUSY`, `INTERNAL`; null is `OK`. | `OK` consumes the facade. `BUSY` retains it and is retryable after healthy active transactions finish. On `INTERNAL`, revision 1 cannot make destruction retry-safe: abandon the pointer without dereferencing or retrying it. No worker-health transition. |
...
documented here. The HLC work then promoted the interface to revision 1.
Phase 1F and final Milestone 1 acceptance are recorded separately.

## Quarantine diagnostics and cleanup-failure tests

Revision 1 represents **Q** explicitly. A caught native cleanup failure marks
...
These choices, the HLC timestamp seam, and the recovery-floor operation are
the reported revision-1 contract. Revision 0 is superseded and is not accepted
by current required-native callers.

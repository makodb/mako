...
- [ ] *high* repeated task — daily Raft CI
  - [ ] for every day, run all Raft CI tests (`./docker_build.sh ci shard1ReplicationRaft`, `./docker_build.sh ci shard2ReplicationRaft`, `./docker_build.sh ci shard1ReplicationSimpleRaft`, `./docker_build.sh ci shard2ReplicationSimpleRaft`) and verify all pass. If any fail, investigate root cause and add a fix task above. [last done: 26:04:25, 11:00 — all 4 suites green on a clean-from-origin rebuild (fresh worktree, fresh `rm -rf build_docker`). shard1ReplicationRaft 60812 ops/sec; shard2ReplicationRaft 8579/8595 ops/sec; both Simple suites pass. Confirms the dfe47ee31 fix is stable across full rebuilds. Previous run at 10:25 was on the same commit but reused the build I had compiled during ASan diagnosis; this run rebuilds from scratch and matches.]

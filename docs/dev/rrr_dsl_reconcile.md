...
`scripts/rrr_dsl_check.sh` reports two blocks failing their
...
"reconciling" means regenerating `src/rrr` with the current transpiler.

## Which is blocked, because the rename would break deptran

`pause()` has real callers: `src/deptran/communicator.cc:456`,
`src/rrr/tests/rpc_chaos_test.cc:266` and `:330`, plus internal uses in
...
3. Regenerate `src/rrr`, confirm deptran and the chaos tests still

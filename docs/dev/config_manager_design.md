...
## Speculation Recovery and the Configuration Manager

A critical responsibility of the Configuration Manager is supporting **Mako's speculation recovery protocol (Jetpack)**. When a leader crashes after speculatively committing transactions but before replication completes, the system must recover those speculative transactions on the new leader. The Configuration Manager is involved because recovery requires knowledge of **which replicas were in the old view and the new view**.

### The Witness Mechanism

Each shard maintains a **witness** — a per-node data structure that tracks all speculatively executed commands that have not yet been durably replicated. The witness is essentially a buffer of recovery candidates:

```
Leader speculatively executes transaction T:
  1. T is added to the local witness (fast hash table insert)
  2. T is returned to the client as "committed" (speculative)
  3. T is submitted to Paxos/Raft for replication (async)

When T is durably replicated:
  4. T is removed from the witness (garbage collected)

If the leader crashes before step 4:
  5. T remains in the witness on surviving replicas
  6. Jetpack recovery protocol recovers T on the new leader
```

The witness tracks commands per-key with conflict detection. If two commands touch the same key, the witness detects the conflict and can reject the second (preventing inconsistent speculative state).

### Jetpack Recovery Protocol (8 Steps)

When a leader fails, the new leader runs Jetpack recovery to ensure no speculative transactions are lost. The protocol uses **view changes** — `old_view` (the membership before the failure) and `new_view` (the membership after) — which are provided by the Configuration Manager.

```
New Leader                  Replicas (old_view + new_view)
    |                              |
    |  Step 1: BeginRecovery       |
    |--- broadcast(old_view,       |
    |    new_view, epoch) -------->|  All replicas enter RECOVERY mode
    |                              |
    |  Step 2: PullIdSet           |
    |--- broadcast --------------->|  Collect witness command IDs from
    |<-- witness ID sets ----------|  f+1 replicas, merge into recovery set
    |                              |
    |  Step 3: PullCmd             |
    |--- broadcast(missing IDs) -->|  Fetch full command objects for
    |<-- command data -------------|  any IDs not already held locally
    |                              |
    |  Steps 4-6: Paxos Consensus  |
    |--- Prepare(recovery_set) --->|  Achieve consensus on which commands
    |<-- Promise ------------------|  to recover (prevents divergence if
    |--- Accept(recovery_set) ---->|  multiple replicas try recovery
    |<-- Accepted -----------------|  simultaneously)
    |--- Commit ------------------>|
    |                              |
    |  Step 7: Resubmit            |
    |  Re-execute all recovered    |
    |  commands on the new leader  |
    |  (marked is_recovery_command_|
    |   = true)                    |
    |                              |
    |  Step 8: FinishRecovery      |
    |--- broadcast --------------->|  Clear witnesses, update epoch,
    |                              |  resume normal operation
```

### Why the Configuration Manager Matters for Recovery

The Jetpack protocol depends on the Configuration Manager for:

1. **View membership**: `old_view` and `new_view` determine which replicas to contact during PullIdSet and PullCmd. The config manager stores `shard/<id>/replicas` which defines the replica set.

2. **Leader identity**: The new leader must know it is the leader (from `shard/<id>/leader`) before initiating recovery. The config manager records leader changes.

3. **Epoch tracking**: Each recovery increments the epoch (`jepoch_`, `oepoch_`). The config manager's `__version__` provides a global ordering of configuration changes, ensuring recovery uses the correct view.

4. **Partition awareness**: For multi-shard transactions, recovery may need to coordinate across shards. The config manager's shard map determines which shards are involved.

### Configuration

The witness mechanism is tied to the concurrency control mode:
- **Enabled**: When `cc: rule` in YAML config (the Mako speculative protocol)
- **Disabled**: When `cc: none`, `cc: occ`, or `cc: tpl_ww`

Jetpack recovery can be disabled for testing:
```bash
export MAKO_DISABLE_JETPACK=1  # Skip recovery (for Raft-only testing)
```
...
| File | Content |
|------|---------|
| `src/deptran/scheduler.h` | `Witness` class, `RecoveryCandidates`, recovery method signatures |
| `src/deptran/scheduler.cc` | Complete 8-step Jetpack recovery implementation (~340 LOC) |
| `src/deptran/communicator.h` | `JetpackBroadcast*` RPC methods for recovery phases |
| `src/mako/mako.hh` | Paxos callback integration with watermarks and witness GC |

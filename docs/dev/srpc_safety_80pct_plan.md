# srpc safety annotation push — plan to reach 80% @safe LOC
...
> `src/srpc/src/basetypes.rs` owns that module today.

## Goal

Grow the share of srpc LOC that is explicitly `// @safe` (function-level
...
2. Extending rusty-cpp with external annotations for trusted unsafe
   helpers, or
3. Moving Marshal to a separate `srpc.marshal_unsafe` module that we
...
3. [ ] `rusty::sys::*` syscall wrappers for the top syscalls used:
   ...
   `// @unsafe` annotations across srpc.
...
1. [ ] Marshal byte ops — choose one of:
   ...
   - quarantine `marshal.cpp` byte-ops into a separate
     `srpc.marshal_unsafe` submodule; the rest of marshal becomes
     @safe.
...
4. **Verify**: run `borrow_check_srpc` and confirm it stays clean.
   Run `cmake --build build_clang21 --target srpc` to confirm compile.
...
- Edit source files in `src/srpc/` and `third-party/rusty-cpp/include/`.
...
### Phase 0
- [x] Fix LOC script + relocate into repo as `scripts/srpc_safety_loc.py`.
...
- [x] Server (rpc/server.cpp) — class-level `// @safe` with method-level
  ...
  borrow_check_srpc 45/45 clean; ratio 6.3% → 7.2% (after script fix).
- [x] Reactor (reactor/reactor.cpp) — class-level `// @safe` added.
  Commit af6db929; borrow_check_srpc 45/45 clean; ratio 7.2% → 7.4%.
...
- [x] misc/alarm.cpp — class `Alarm` `// @safe`. Methods use
  `rusty::BTreeMap` + `rusty::Function` + `srpc::Time::now()`. The raw
  `srpc::PollThread *holder` field is never dereferenced and
  `set_holder` is a no-op stub. No per-method overrides needed.
  Commit a64a2a96; ratio 20.5% → **20.8%** (+32 LOC).
- [x] base/basetypes.cpp — both `export namespace srpc` and the impl
  `namespace srpc` `// @safe`. Per-method `// @unsafe` overrides on
  ...
  **22.7%** (+237 LOC).
- [x] base/debugging.cpp — both `export namespace srpc` and the impl
  `namespace srpc` `// @safe`. Per-method `// @unsafe` on both
...
- [x] base/strop.cpp — `namespace srpc` `// @safe`. Per-method
...
- [x] base/callback_wrapper.cpp — both `export namespace srpc` and the
...
- [x] base/misc.cpp — both `export namespace srpc` and the impl
  `namespace srpc` `// @safe`. Per-method `// @unsafe` on `rdtsc`
  (inline asm), `FrequentJob::Ready` (srpc::Time::now), `make_int`
  ...
  23.8% → **24.2%** (+42 LOC).
- [x] base/unittest.cpp — both `export namespace srpc` and the impl
  `namespace srpc` `// @safe`. Per-method `// @unsafe` on `TestMgr::
  ...
  (+83 LOC).
- [x] reactor/epoll_wrapper.cc — both `export namespace srpc` and the
  impl `namespace srpc` `// @safe`. Per-method `// @unsafe` on every
...
- [x] inmemory_channel.cpp retry — both `export namespace srpc` and the
  impl `namespace srpc` `// @safe`. Per-method `// @unsafe` on every
...
- [x] rpc/utils.cpp retry — both `export namespace srpc` and the impl
  `namespace srpc` `// @safe`. AddrInfo's per-method `// @unsafe` on
...
- [x] SP-1: `rusty::sys::fs` wrapper — added new
  ...
  (no srpc files modified — adoption comes next).
- [x] netinfo.cpp retry — `export namespace srpc` and `class NetInfo`
...
- [x] cpuinfo.cpp retry — `export namespace srpc` and `class CPUInfo`
...
- [x] frame_codec.cpp retry — `export namespace srpc` and the impl
  `namespace srpc` `// @safe`. Per-method `// @unsafe` on every
...
- [x] misc/serializable_envelope.cpp retry — `export namespace srpc`
...
- [x] ChannelConnectionProxy / ChannelListenerProxy / ChannelFactoryProxy
  ...
  borrow_check_srpc 45/45 clean; srpc library + downstream rpcbench/dbtest
...
- [x] rusty::sys::* syscall wrappers — cross-repo library-design task.
   ...
   * epoll/kqueue — epoll_wrapper.cc already abstracts the platform
     ...
     into rusty::sys would be net-zero on srpc safety.
   ...
   * full file I/O surface — rusty::sys::fs::read_to_string (SP-1) plus
     rusty::os::fd::OwnedFd (Phase A) cover the entries srpc actually
...
- [x] Pthread_* wrappers — namespace `// @safe` umbrellas added on
  both `export namespace srpc` (line 23) and the impl `namespace srpc`
...
- [x] Marshal byte ops decision — initially **chose labeling (option 3
  ...
  on both `export namespace srpc` and the impl `namespace srpc`; class
...
- [x] Marshal byte ops rewrite (Cursor-style) — picked option 1 of
  ...
  internals of `srpc::Marshal` with a `rusty::Vec<uint8_t>` +
...
- [x] client.cpp — namespace `// @safe` comments added to all 3
  `export namespace srpc` blocks + impl `namespace srpc`. No code
...
- [x] rcc_rpc.h codegen rewrite — done in prior commits; this
  ...
  `src/srpc/pylib/simplerpcgen/lang_cpp.py:193` emits
  `rusty::Arc<srpc::Future>` (not `std::shared_ptr<srpc::Future>`) for
  ...
  describing the implicit-conversion shim; the srpc-side wire boundary
...
srpc whose rationales were tied to operations that have since become
...
**73.3%**), borrow_check_srpc 45/45 clean throughout. The
remaining `// @unsafe` markers in srpc are mostly legitimate
...
Survey-driven prune of code in srpc that has zero production callers:

- [x] `ThreadPool` / `RunLater` / `SpinCondVar` / `Queue` from
  `src/srpc/base/threading.cpp` — never constructed in production
  (the deptran workers wired `ThreadPool` into fields but never
  enqueued work). Commit `f6be7df9`. ~520 LOC.
- [x] `NetInfo` class (`src/srpc/misc/netinfo.cpp`) — public API is
  `NetInfo::net_stat()`, never invoked anywhere. Removed file plus
  `import srpc.netinfo;` from srpc.hpp and the matching entries in

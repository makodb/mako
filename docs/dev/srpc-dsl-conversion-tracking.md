# srpc → Inline-Rust DSL: Conversion Progress Tracker
...
Total hand-written C++ **code** lines in `src/srpc/` (non-blank, non-comment, **outside**
...
★ yapps blocker RESOLVED — vendored yapps 2.2.0 runtime into `src/srpc/pylib/yapps/` (`ac99fb06`).
...
`make_source_proxy(&req->m)` direct + `reply(..., srpc::ServerReplyFn{})`). Verified: fails at BASELINE.
...
The vendored `src/srpc/pylib/yapps` here was untracked stale `.pyc`-only bytecode (removed); `yapps` is NOT
...
`src/srpc/pylib/simplerpcgen/lang_cpp.py` — flip these CALL-SITE emits (`<<`→`srpc::Serialize_::serialize(x, m)`,
`>>`→`srpc::Deserialize_::deserialize(x, ar)`); the generic catch-all (Phase 3) makes them resolve for any type:
...
- 2026-07-18 — **Marshal-deprecation steps 1-3 LANDED** (3 commits): Request retyped to
  ...
  client scratch writers (serialize-once-for-retry, heartbeat probe) → BufferSink. No srpc/rpc
...
- 2026-07-18 — **★ PHASE 8 COMPLETE — operator layer DELETED**: endgame pt1 (Archive decoy-ADL,
  ...
  forwarders dropped from srpc.serializable/srpc.marshal; final stragglers (log_storage save/load
...
- 2026-07-18 — **Phase 8 batch 2 LANDED — the "visibility puzzle" was PURE DYNDEP STALENESS**:
  ...
  *.modmap (clean-slate 452s, test_marshal 22/22). The misleading symptoms (candidates only
  from srpc.serializable, error lines on comment lines) were stale-BMI artifacts from earlier
...
- 2026-07-18 — **Phase 8 batch 1 LANDED on retry (`0b407a14`)**: brace-counting transformer
  ...
  candidate lists containing ONLY srpc.serializable's Serialize_ members — NONE from marshal.cpp —
  ...
  seeing srpc.marshal's namespace additions?) or dyndep/BMI staleness confusing the diagnosis
  ...
  namespaces across module boundaries (marshal.hpp is just `import srpc.marshal;`) — fallback
  design: put the Marshal container overloads in srpc.serializable (where the trait lives) with
...
- 2026-07-18 — **Build-time campaign CLOSED (all levers measured)**: local mirror infra landed
  ...
  srpc.hpp 30→22). Every remaining lever measured DEAD: reduced-BMI (pcm literally identical —
  ...
  mechanical chain PROVEN via srpc.utils probe (auto-glob, scanner, dyndep, 3s no-cascade leaf)
...
- 2026-07-17 — **Phase 7b IN PROGRESS**: chose Path A (add serialize-over-Marshal, flip call sites;
  ...
  marshal.cpp (`srpc::Serialize_::serialize(const T&, Marshal&){ m << t; }` + Deserialize_ mirror;
  ...
  `srpc::deserialize_from(fu->get_reply(), a, b)` — a variadic helper that binds the RefMut<Marshal>
...
- 2026-07-17 — **Phase 7a DONE (`fcf3b2ae`)**: flipped all 201 hand-written ARCHIVE call sites
  ...
  ANALYSIS: the trait is Archive-only; Marshal (module srpc.marshal) imports serializable (not vice
  versa), so a Marshal catch-all `serialize(const T&, Marshal&){ m << t; }` must live in marshal.cpp
  (reopening srpc::Serialize_ — visible at m-sites since they include marshal). The m-sites HAVE chains
...
**Traits** (in serializable.cpp, module srpc.serializable), TWO because write is const-self, read is mut-self:
```rust

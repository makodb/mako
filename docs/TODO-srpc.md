  ...
  - Sub-leaves (decomposed because `client.{hpp,cpp}` totals ~3,900 LOC with deeply intertwined fd-using code paths; the refactor is too large to commit safely in one shot, and the RPC test suite is the critical regression surface — each sub-leaf must keep it green before the next one can land):
    ...
    - [ ] *high* Sub-leaf 4g — Make channel mode the default; delete the legacy fd path.
      ...
      - [ ] Sub-leaf 4g1b — Fix the `MultiThreadedStressTest` 100-thread wedge in channel mode (poll-thread wake-up tried; was *not* the root cause).
        ...
        - **Two more hypotheses ruled out (2026-04-28):**
          ...
          - Plus from the prior iteration: ❌ poll-thread wake-up latency (busy-loop epoll_wait still wedges).
        - **Remaining hypotheses (narrowed):**
          1. **`outbound_` SpinMutex contention or deadlock**: 100 user threads contending on per-connection SpinMutexes are fine in isolation, but if a poll-thread `handle_write` and a user-thread `send_frame` deadlock-equivalent on different connections via shared scheduling resources (fiber pool, reactor lock, etc.), the drain stalls.
          ...
          3. **`update_mode(R|W)` racing with `update_mode(R)` for the same fd**: with many connections sending in a tight loop, two threads (user + poll) can post update_mode commands for the same fd. mpsc preserves per-producer order but inter-producer order is undefined; the *last* enqueued command wins. If a poll-thread handle_write completes (posts `update_mode(R)`) and a user thread's send_frame for the *same fd* posts `update_mode(R|W)` slightly earlier in mpsc ordering, the poll thread processes `update_mode(R|W)` first then `update_mode(R)` last, leaving `mode_=R` and EPOLLOUT removed despite having pending bytes. **This is the most likely remaining cause.** Fix shape: protect the mode transition with a per-connection sequence number, OR rely on `pending_write_update_` flag at the bottom of poll_loop to recover lost EPOLLOUT.
          4. **PollThread fiber-allocation thrash**: 100 fibers parked on IntEvents + 1000+ events going through Reactor::loop's `waiting_events_` Vec each iteration. The "event status ready, triggered?" log floods strongly during the wedge; investigate whether fiber recycling / event extraction has an O(N²) path.

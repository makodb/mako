...
- [ ] **Phase 1 — New abstractions (parallel, no caller migration)**.
  ...
  - [ ] **Phase 1c — FdSink / FdSource**. `FdSink::write(const void*, size_t)` does `::write` (full-write loop, propagating partial writes / EINTR). `FdSource::read(void*, size_t)` does `::read` (full-read loop). Tests: pipe-based round-trip, EOF handling, large-payload chunked write.

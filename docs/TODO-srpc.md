...
- [x] *high* Sub-leaf L1b — `std::unique_ptr<T>` → `rusty::Box<T>` in rrr test files. ✅ **LANDED 2026-04-28** across two commits (initial 4 files + L1b-tail 7 files).
  - **Survey correction (2026-04-28)**: the original Workstream L survey miscounted these as "11 prod sites" — all 27 occurrences were in `src/rrr/tests/` test files. There are zero `std::unique_ptr` uses in rrr production code (verified via `grep -rE 'std::unique_ptr|std::make_unique' src/rrr | grep -v tests/` returning empty).
...
- [ ] *high* Sub-leaf L2 — `std::list<T>` → rusty equivalents (76 prod sites total). **PARTIAL — L2a-base done (2026-04-28)**.
  ...
  - Decomposed:
    ...
    - [x] **L2b-request_queue — `src/rrr/rpc/request_queue.hpp`** (19 sites). ✅ **LANDED 2026-04-28**. Migrated `std::list<QueuedRequest>` → `rusty::VecDeque<QueuedRequest>`.
    ...
    - [ ] **L2c-alock — `src/rrr/misc/alock.{hpp,cpp}`** (22 sites). **DEFERRED — non-trivial migration**. Survey (2026-04-28) shows alock uses std::list as the lock-waiter queue with patterns that don't translate cleanly to `rusty::VecDeque`:
      - **Reverse iteration** (`std::list::reverse_iterator rit = requests_.rbegin();` at alock.cpp:309, 329): `rusty::VecDeque` does not expose `rbegin/rend`. Migration would either rewrite as manual reverse-index loops (`for (size_t i = vec.size(); i-- > 0;)`) or add a custom reverse-iterator wrapper.
      - **Iterator-stable erase-during-iterate** (`std::list<...>::iterator next_it = requests_.erase(it);` at alock.cpp:230, 257, 270 etc.): `VecDeque::erase(iterator)` does not exist; alternatives are `extract_if(predicate)` (separate pass — predicate is `Function<bool(const T&)>` so can't move out) or manual index-based shifting.
      - **Long-lived pointer/reference stability**: callbacks captured via `to_calls` reference `req->yes_callback` and `req->id` after a separate iteration; in std::list those references stay valid until the node is erased. In VecDeque, push_back can grow the buffer and invalidate all element references.
      - **Recommendation**: this is a real migration with semantic implications, not just a 1:1 type swap. Either keep `std::list` as a documented exception (similar to LRU caches in idempotency.hpp / completion_tracker.hpp) or refactor the lock-waiter queue's storage and iteration patterns. ~80-200 LOC change either way; needs careful design review.
      - Pulling out of the L2 quick-win batch; revisit as a focused leaf with explicit design decisions.
    - [x] **L2d-server-cleanup — `src/rrr/rpc/server.{hpp,cpp}` + `client.hpp`** (~18 stale-annotation sites). ✅ **LANDED 2026-04-28**.
...
- [ ] *medium* Sub-leaf L5 — `std::function<F>` → `rusty::Function<F>` (143 prod sites, 24 test sites). **PARTIAL — L5a (heartbeat + alarm) done (2026-04-28)**.
  ...
  - **Decomposed**:
    ...
    - [ ] **L5-callbacks (DEFERRED)** — `callbacks.hpp` `ConnectionCallbacks` uses `Vec<std::function<...>>::clone()` to drain callbacks under lock before invoking. Migrating the inner `std::function` to `rusty::Function` breaks the clone() (rusty::Function is move-only; Vec<T>::clone() requires T copyable). Options: wrap as `Vec<Arc<Function<...>>>` (cheap clone via Arc — but changes the API surface for callers who currently pass plain lambdas) or keep as carve-out (similar to marshal API — std::function as the callable wrapper boundary type). Defer to a focused leaf with explicit design decision.
    - [ ] **L5-request_queue (DEFERRED)** — `RequestQueue::peek(QueuedRequest&)` does `out = queue_.front();` which copy-assigns the QueuedRequest. With move-only callback inside, the copy fails. Either drop `peek()` (only used in tests) or wrap callback as `Arc<Function<...>>`. Defer.
    - [ ] **L5-channel (DEFERRED)** — `channel.hpp` callback type aliases (`OnFrameCallback`, `OnClosedCallback`, etc.) flow through `pro::proxy` facade conventions (`add_convention<..., void(OnFrameCallback)>`). Need to verify pro::proxy supports move-only convention argument types under proxy facade dispatch. Investigation needed before migration.
    - [ ] **L5-client + L5-server (DEFERRED)** — 12 + 7 sites in `client.{hpp,cpp}` and `server.{hpp,cpp}`; mostly callbacks stored on per-connection state. Each needs ownership review (single-shot vs repeated; copy-in-storage required?).
    - [ ] **L5-alock (DEFERRED)** — alock.hpp/cpp has 43+18 sites; the largest concentration. Needs decomposition by sub-method; complex iterator/callback interactions.
    - [ ] **L5-cleanup (deferred)** — recorder.hpp/cpp (5+3 sites; raw-pointer pattern), threading.hpp/cpp (4+3 sites; ThreadPool job queue), connection_state.hpp (5 sites), reactor.h/cc (2+2 sites), marshal.hpp/cpp (1+1 sites), base/threading + dball/misc (small files).
...
- [ ] *medium* Sub-leaf L6 — `std::shared_ptr<T>` → `rusty::Arc<T>` (multi-thread) or `rusty::Rc<T>` (single-thread). **DEFERRED with documented findings (2026-04-28)**: the 87 prod sites split into two distinct islands, each blocked.
  ...
  - **Recommendation**: treat both islands as **carve-outs** rather than migration targets, on the same rationale as the existing `std::string` and `std::map`-marshal-API carve-outs:
    - **Island A (marshal API)**: The boundary type for the wire-format/serialization framework. Changing it is a breaking change for every user of the marshal API; resolving the non-owning shared_ptr in scheduler.cc requires API redesign. Mark `std::shared_ptr<Marshallable>` as a documented carve-out alongside `std::string` (rrr framework wire boundary) and `std::map<...>`/`std::set<...>` marshal serialization overloads.
    - **Island B (Event subsystem)**: Promote to its own follow-up — **"Workstream M: Event subsystem rusty migration"** — atomic refactor of `std::shared_ptr<Event>` + `std::weak_ptr<Event>` + `enable_shared_from_this` patterns, scoped at ~200-400 LOC inside rrr plus an unknown impact in deptran callers that construct events via `make_shared<MyEvent>()`. Reactor's RPC test suite must remain green throughout. Defer until there's a clear motivation (e.g. a borrow-check win or a measurable safety regression).
  - Goal: documented design rationale rather than code change. Reopen as a real migration leaf only when (a) the marshal API can accept a breaking change AND the non-owning-shared_ptr pattern in scheduler.cc has a chosen replacement, OR (b) Workstream M is explicitly green-lit.
- [ ] *medium* Sub-leaf L7 — `std::mutex` + `std::lock_guard` → `rrr::SpinMutex<T>` / `rusty::Mutex<T>` ownership-style. **PARTIAL — L7-request_queue done (2026-04-28)**.
  ...
  - **Decomposed**:
    ...
    - [x] **L7-marshal — `marshal.cpp` global registry mutex**. ✅ **LANDED 2026-04-28**.
      - Replaced the prior `std::mutex md_mutex_g` + `static MarContainer mc_` (Construct-On-First-Use static inside `get_initializers()`) pair with a single rusty-style `rrr::SpinMutex<MarContainer>` Construct-On-First-Use static, accessed via a file-local `md_registry_locked()` helper.
...
- [ ] **Phase 3 — RPC framework boundary**. Decomposed into:
  ...
  - [x] **Phase 3c — `rpcgen` archive emission flag** ✅ **LANDED 2026-04-28**. Added `--archive` flag to `bin/rpcgen` (threaded through `rpcgen.py`, `lang_cpp.py`'s `emit_rpc_source_cpp` / `emit_struct` / `emit_marshaled_typed_struct` / `emit_service_and_proxy`). When set, the generator emits BinaryWriteArchive / BinaryReadArchive operator<<>> overloads ALONGSIDE the existing Marshal& ones (additive, not replacing) for both user structs in the .rpc and the auto-generated typed Rpc*Request/Response bundles. Default off — `rcc_rpc.h` regeneration is unchanged because it uses `MarshallDeputy` and `Value` types that don't yet have archive operators (Phase 3f / Phase 4 work). Also added `marshal_archive.hpp` to the `rrr.hpp` umbrella so generated headers see the archive types via the existing `#include "rrr/rrr.hpp"` line. `rpcgen_compile_test.py` extended with a `RPC_SOURCES_ARCHIVE` list covering the small fixtures (`helloworld.rpc`, `network.rpc`, `benchmark_service.rpc`); both modes compile cleanly. Full RPC suite green: 14 binaries, 282 tests, no regressions.
  - [ ] **Phase 3d — Reactor TX/RX path consumes BinaryWriteArchive/BinaryReadArchive directly**. Sketch `tcp_channel`'s out buffer and in buffer to expose Sink/Source views directly (rather than going through `Marshal`).
  - [ ] **Phase 3e — Switch `rpcgen` default to `--archive`**. Regenerate `rcc_rpc.h` under the new default; `Marshal&` operator emission goes away.
  - [ ] **Phase 3f — `MarshallDeputy` internals on `SerializableProxy`**. Replace `sp_data_` + `inner_sp_data_` duplication with a single `SerializableProxy serializable_`. Public surface unchanged. Decomposed:
    - [x] **Phase 3f-1 — Eliminate `sp_data_` field; derive `MarshallableProxy` on demand** ✅ **LANDED 2026-04-28**. Removed `std::shared_ptr<rrr::MarshallableProxy> sp_data_` from MarshallDeputy. The MarshallableProxy is constructed on demand inside `data_proxy()` from `inner_sp_data_` via `make_marshallable_proxy`. `data_proxy()` returns by value (the temp lives until end-of-full-expression for `data_proxy()->method(...)` calls). `set_marshallable_state` no longer consumes `state.proxy` (the field stays populated for backward compat with external `MarInitializerFn`s; the value is harmless and ignored). `inner()` retains both `shared_ptr&` overloads — call sites pass `md.inner()` to functions taking non-const ref, which would break with return-by-value semantics; the inner() reference shape stays. `has_marshallable()` checks `inner_sp_data_`. Public API unchanged. Net: one less heap allocation per MarshallDeputy. Full RPC suite green: 14 binaries, 291 tests, no regressions.
    - [ ] **Phase 3f-2 — Add SerializableProxy storage alongside `inner_sp_data_`**. Add a `SerializableProxy serializable_` field. Populate via `as_serializable(inner_sp_data_)` for legacy Marshallable types or directly when constructed from a SerializableProxy. Wire format unchanged.
    - [ ] **Phase 3f-3 — Single-field storage** (depends on Phase 4 complete). Once all production types are Serializable, `inner_sp_data_` derives from `serializable_` via `as_marshallable`. Whether to actually drop `inner_sp_data_` from the storage depends on `inner()` callers — many take by reference and expect stable identity; likely we keep `inner_sp_data_` as the primary field and `serializable_` as a lazy cache (Phase 5 then evaluates the swap).
...
- [ ] **Phase 4 — Per-command-type migrations** (decompose by directory):
  ...
  - 4a — `deptran/classic/` (TPC commands)
    ...
    - [ ] **Phase 4a-3 — `TpcPrepareCommand`**, `TpcCommitCommand`, `TpcBatchCommand`. Unblocked by Phase 3f-prep. Decomposed:
      - [x] **Phase 4a-3a — `TpcPrepareCommand`** ✅ **LANDED 2026-04-28**. Fields: tx_id (i64), ret (i32), shared_ptr<Marshallable> cmd_. Migrated to Serializable: dropped to_marshal/from_marshal, added save/load/kind methods that use `ar << md` / `ar >> md` for the nested MarshallDeputy field via the Phase 3f-prep archive operators. Removed `TypedMarshallableAdapterTraits<TpcPrepareCommand>`, switched registration to `reg_serializable_in_deputy`. Updated construction site at `deptran/classic/scheduler.cc:171` (`wrap_typed_marshallable` → `rrr::wrap_serializable`) and cast site at `deptran/classic/scheduler.cc:387` (`marshallable_cast` → `serializable_cast`). Wire format byte-for-byte identical (kind | tx_id | ret | nested_md). Full RPC suite green: 14 binaries, 294 tests, no regressions.
      - [ ] **Phase 4a-3b — `TpcCommitCommand`**. Same shape as `TpcPrepareCommand` plus `term` (ballot_t) and an optional `sp_view_data_` (encoded with `bool_t has_view_data` prefix + nested MarshallDeputy<ViewData>). Has a serialization round-trip test in `rpc_marshallable_proxy_test.cc:420` that also needs updating.
      - [ ] **Phase 4a-3c — `TpcBatchCommand`**. Holds `vector<shared_ptr<TpcCommitCommand>>`. Currently calls `(*it)->to_marshal(m)` directly on each commit; after migration, batched-element save/load uses TpcCommitCommand's new save/load (so 4a-3b must land first).

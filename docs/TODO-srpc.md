...
- [ ] *medium* Sub-leaf L7 — `std::mutex` + `std::lock_guard` → `rrr::SpinMutex<T>` / `rusty::Mutex<T>` ownership-style. **PARTIAL — L7-request_queue done (2026-04-28)**.
  ...
  - **Decomposed**:
    ...
    - [ ] **L7-inmemory_connstate (DEFERRED)** — the OTHER mutex in `inmemory_channel.{hpp,cpp}` is `InMemoryConnectionState::mu` (line 209 of hpp) which protects ~30 internal fields of the per-channel-pair state (a_closed, b_closed, callbacks a_on_frame/b_on_frame/a_on_closed/b_on_closed/a_on_error/b_on_error, fault-injection counters, peer_address). Migration would wrap as `SpinMutex<InternalConnState>` and refactor the state struct, touching ~12 method-body lock sites in send_frame/close/is_closed/peer_address/set_on_*/inject_*. Doable but more invasive; separate leaf with full method-by-method walk-through.

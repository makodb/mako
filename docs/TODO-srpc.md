...
- [ ] Migrate in-tree generated RPC headers from `.rpc` sources (`helloworld`, `network`, `rcc_rpc`) to typed mode and update callsites.
  ...
  - [x] Leaf 2 (`network`): regenerate `src/deptran/network.h` in typed mode and update `network_client`/`nc_main` callsites for typed request/response usage.
    ...
    - Closed on 2026-04-12 after leaf 2a/2b/2c completion and full RPC-focused suite pass.
    ...
    - [x] Leaf 2b (`network` remaining paths): migrate remaining `network_client`/benchmark callsites (non-active transaction categories) to typed request/response overloads where compiled paths exist.
      - Analysis on 2026-04-12: no additional compiled non-active benchmark callsites remain in-tree beyond the already migrated active YCSB path; historical TPCC-like benchmark callsites in `src/nc_main.cc` are currently commented-out (non-compiled).
      - Implemented on 2026-04-12 by extending `test/rpcgen_in_tree_network_typed_test.py` compile coverage to typed and legacy sync/async proxy callsites for all non-active methods (`txn_new_order`, `txn_payment`, `txn_delivery`, `txn_order_status`, `txn_stock_level`) plus `txn_rmw`, ensuring compiled-path migration guard coverage across the full network proxy surface.
  ...
  - [x] Leaf 3b (`rcc_rpc` migration part 1): migrate high-traffic `ClassicService`/`ClassicProxy` callsites to typed request/response APIs while keeping compatibility wrappers.
    ...
    - [x] Leaf 3b.2 (`ClassicProxy` high-traffic callsites): migrate communicator/rcc classic high-traffic proxy callsites (`RccDispatch`/`RccPreAccept`/`RccAccept`/`RccCommit` and related fast paths) to typed request/response overloads while retaining wrapper compatibility.
      - Implemented on 2026-04-12 in `src/deptran/troad/commo.cc` for high-traffic classic paths by switching `async_RccPreAccept`/`async_RccAccept`/`async_RccCommit` from legacy positional-argument wrappers to typed request overloads (`RpcRccPreAcceptRequest`/`RpcRccAcceptRequest`/`RpcRccCommitRequest`) while preserving existing callback/reply behavior and wrapper compatibility.
      - Migration guard added on 2026-04-12 as `test/rcc_rpc_classic_proxy_typed_callsites_test.py` with CTest wiring (`test_rpc_rcc_rpc_classic_proxy_typed_callsites`); it verifies typed high-traffic callsite usage in `troad/commo.cc`, rejects fallback to the previous legacy argument patterns, and compiles typed proxy request overload usage for `RccDispatch`/`RccPreAccept`/`RccAccept`/`RccCommit`.
      - Validation on 2026-04-12: focused typed migration guard set passed (`test_rpc_rpcgen_in_tree_rcc_rpc_typed_prep`, `test_rpc_rpcgen_in_tree_rcc_rpc_typed_sync`, `test_rpc_rcc_rpc_classic_proxy_typed_callsites`) and full RPC-focused suite regex passed (`ctest --test-dir build_rpc --output-on-failure -j8 -R '^(test_rpc.*|test_load_balancer|test_idempotency|test_completion_tracker|rpc_chaos_test|test_erpc_integration)$'`, 41/41 passed).
    - [x] Leaf 3b.3 (`ClassicService` bridge alignment): migrate corresponding high-traffic `ClassicServiceImpl` typed bridge callsites and remove remaining pointer-assumption drift in part-1 surfaces.
      - Implemented on 2026-04-12 in `src/deptran/service.h` and `src/deptran/service.cc` by adding typed `ClassicServiceImpl` overrides for `RccDispatch`/`RccPreAccept`/`RccAccept`/`RccCommit` and routing legacy pointer-style handlers through those typed paths so request/response behavior is defined once.
      - Added migration guard on 2026-04-12 as `test/rcc_rpc_classic_service_typed_bridge_test.py` with CTest wiring (`test_rpc_rcc_rpc_classic_service_typed_bridge`), verifying typed service bridge symbols + compile-time typed signature compatibility while retaining existing proxy migration guards.
      - Validation on 2026-04-12: focused typed guard set passed (`test_rpc_rpcgen_in_tree_rcc_rpc_typed_prep`, `test_rpc_rpcgen_in_tree_rcc_rpc_typed_sync`, `test_rpc_rcc_rpc_classic_proxy_typed_callsites`, `test_rpc_rcc_rpc_classic_service_typed_bridge`) before full RPC-focused regression run.

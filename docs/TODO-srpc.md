...
- [ ] **Phase 3 — RPC framework boundary**. Decomposed into:
  ...
  - [ ] **Phase 3c — `rpcgen` archive emission flag**. Add a `--archive` flag to `rpcgen.py` / `lang_cpp.py` that emits `BinaryWriteArchive`/`BinaryReadArchive` operator<<>> instead of `Marshal&`. Default off — generator generates the same code as today unless flag is passed.

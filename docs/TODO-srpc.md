...
- [ ] **Phase 4 — Per-command-type migrations** (decompose by directory):
  - 4a — `deptran/classic/` (TPC commands)
    ...
    - [ ] **Phase 4a-3 — `TpcPrepareCommand`**, `TpcCommitCommand`, `TpcBatchCommand`. Each has a `MarshallDeputy cmd_` field (or `vector<TpcCommitCommand>` for batch). Unblocked by Phase 3f-prep — `BinaryWriteArchive::operator<<(MarshallDeputy)` / `BinaryReadArchive::operator>>(MarshallDeputy)` are now available. Migration follows the Phase 4a-2 template: add save/load/kind, drop to_marshal/from_marshal + TypedMarshallableAdapterTraits, switch registration, update construction + cast sites.

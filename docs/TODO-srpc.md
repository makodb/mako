...
- [ ] **Phase 3 — RPC framework boundary**. Decomposed into:
  ...
  - [ ] **Phase 3b — `MarshallDeputy::serializable()` accessor + transitional bridge**. Introduce a `SerializableProxy MarshallDeputy::serializable() const` that wraps the existing `inner()` Marshallable as a `Serializable` (via a transitional `MarshallableSerializableAdapter` that calls `to_marshal`/`from_marshal` from `save`/`load`). This unblocks Phase 4 — per-command-type migrations can flip individual types from `Marshallable` to `Serializable` without breaking `MarshallDeputy` callers. Estimated +120 LOC.

...
- [ ] *high* Phase 1: Migrate `Pollable` to proxy. Defined in `reactor/epoll_wrapper.h`, 10 virtual methods, 3 implementations (ServerListener, ServerConnection, ClientConnection). Stored as `Arc<Pollable>`. Define `PollableFacade` using `PRO_DEF_MEM_DISPATCH`. Remove `: public Pollable` from impls. Change `Arc<Pollable>` to `pro::proxy<PollableFacade>`. ~200 LOC.
  ...
  - [x] Leaf 4: remove `: public Pollable` from `ServerListener`, `ServerConnection`, and `ClientConnection`; update construction/callers to produce `pro::proxy<PollableFacade>` directly.
    ...
    - Poll-thread integration updates in `src/rrr/reactor/reactor.h` and `src/rrr/reactor/reactor.cc`: added explicit proxy ingress API (`add_proxy(PollableProxy)`), retained legacy `add(Arc<Pollable>)` compatibility path, and added fd-based `update_mode(int fd, int new_mode)` overload so non-`Pollable` classes can request mode transitions without pointer/interface coupling.

...
- [ ] *high* Refactor RPC server to depend on `ChannelListener`/`ChannelConnection`.
  ...
  - Sub-leaves:
    - [ ] *high* Sub-leaf 5a — Channel-binding scaffolding on `Server`.
      - Add an optional `ChannelFactoryProxy server_channel_factory_` member (default null) and a `set_channel_factory(ChannelFactoryProxy)` setter that records the factory for use by `start()` in subsequent leaves.
      - Add an `is_channel_factory_bound()` accessor for routing logic.
      - **No behavior change**: legacy `socket(2)`+`bind(2)`+`listen(2)`+`accept(2)`+epoll-driven path stays the only active code path; `start()` doesn't read the new field yet.
      - Tests: a small unit test (analogous to `rpc_client_channel_binding_test.cc`) verifying the new accessors exist with the expected signatures and the latch flips on a non-null factory; existing RPC tests continue to pass unchanged.
      - Goal: lock down the type signature so subsequent leaves can wire behavior without touching it.
    - [ ] *high* Sub-leaf 5b — Route outbound replies through `ChannelConnection` when bound.
      - Add an optional `ChannelConnectionProxy channel_proxy_` member to `ServerConnection` plus a `bind_channel(ChannelConnectionProxy)` setter that records the proxy and flips a `channel_mode_` latch.
      - Modify `ServerConnection::reply(...)` to encode the response body into a temporary `Marshal`, extract bytes, and dispatch via `channel_proxy_->send_frame(...)` when in channel mode. The legacy `out_` Marshal-as-syscall-buffer path stays for `is_channel_mode() == false`.
      - Tests: a new test that constructs a `ServerConnection` with a fake `ChannelConnectionProxy`, drives a synthesized request through dispatch, and verifies the reply frame arrives at the fake's `send_frame` capture.
    - [ ] *high* Sub-leaf 5c — Drive inbound demux from on_frame callback when channel-bound.
      - When `ServerConnection` is in channel mode, the `ChannelConnectionProxy::set_on_frame(...)` callback should call a new `decode_request_and_dispatch(bytes, size)` helper that parses `[v64 xid][i32 rpc_id][user-args]` (mirrors `ServerConnection::handle_read`'s per-request path) and routes to the service's `__dispatch__`.
      - Legacy `handle_read` path stays for `is_channel_mode() == false`.
      - Tests: extend the channel-bound `ServerConnection` test to push a synthesized request frame through `on_frame` and verify the registered service handler runs.
    - [ ] *high* Sub-leaf 5d — Map `on_closed` / `on_error` to existing close/error paths when channel-bound.
      - Wire `set_on_closed` to call the existing `ServerConnection::close()` path (state transition + Server-side cleanup).
      - Wire `set_on_error` to log + close.
      - Tests: synthesize an `on_closed(ChannelError::ConnectionReset)` and verify the server-side connection map drops the entry.
    - [ ] *high* Sub-leaf 5e — Listen via `ChannelFactoryProxy::make_listener()`.
      - Modify `Server::start(addr)` to: when a channel factory is bound (`is_channel_factory_bound()`), call `factory->make_listener()`, install an `on_accept(ChannelConnectionProxy)` callback that constructs a `ServerConnection` bound to the new proxy via 5b's `bind_channel(...)` and registers it in the `Server`'s connection map, then call `listener->listen(addr)`. Otherwise fall back to the legacy `ServerListener` path.
      - Tests: end-to-end channel-mode server using `TcpFactory::make_listener()` against a `Client` (also in channel mode) — verify request/response round-trip, multiple concurrent connections, and `Server::stop_accepting()` propagation.
    - [ ] *high* Sub-leaf 5f — Auto-install default `TcpFactory` in `Server::start` when none bound.
      - Mirror the client-side post-4g4 pattern: if no factory has been installed via `set_channel_factory(...)`, `Server::start(addr)` constructs a default `TcpFactory(poll_thread_)` and binds it. Channel mode becomes the only path; the legacy `ServerListener` socket path is bypassed.
      - **Behavior change**: channel mode becomes default for `Server::start`. Existing tests should continue to pass (the channel layer's `TcpListener` is functionally equivalent).
      - Tests: full RPC suite green; verify no test relies on the legacy `ServerListener`'s internals leaking out.

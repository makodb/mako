...
- [ ] **Phase 4 — Per-command-type migrations** (decompose by directory):
  - 4a — `deptran/classic/` (TPC commands)
    ...
    - [ ] **Phase 4a-3 — `TpcPrepareCommand`**, `TpcCommitCommand`, `TpcBatchCommand`. Each has a `MarshallDeputy cmd_` field (or `vector<TpcCommitCommand>` for batch). Migration depends on completing Phase 3f (MarshallDeputy archive operators) and reviewing nested-MarshallDeputy save/load semantics.

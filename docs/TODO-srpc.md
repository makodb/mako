  ...
  - Sub-leaves (decomposed because `client.{hpp,cpp}` totals ~3,900 LOC with deeply intertwined fd-using code paths; the refactor is too large to commit safely in one shot, and the RPC test suite is the critical regression surface — each sub-leaf must keep it green before the next one can land):
    ...
    - [ ] *high* Sub-leaf 4g — Make channel mode the default; delete the legacy fd path.
      ...
      - [ ] Sub-leaf 4g1b — Fix the `MultiThreadedStressTest` 100-thread wedge in channel mode (poll-thread wake-up tried; was *not* the root cause).
        ...
        - **Remaining hypotheses for the *actual* wedge** (post-eliminating wake-up):
          1. **Edge-triggered EPOLL re-arm bug**: under `EPOLLET | EPOLLOUT`, the kernel fires `EPOLLOUT` only on transitions from "non-writable" to "writable". With many connections rapidly cycling READ/WRITE mode via `update_mode` (each `send_frame` calls `update_mode(fd, R|W)`, then `handle_write` returns READ-only and `update_mode(fd, R)` removes EPOLLOUT, then next `send_frame` re-adds it), if the kernel decides the fd is "writable" but we've toggled EPOLLOUT off and on, the level-vs-edge interpretation can leave the fd permanently un-fired despite having pending bytes. Audit `Epoll::Update`'s `epoll_ctl(MOD)` semantics and consider switching the channel-mode TcpConnection to `EPOLLLT` (level-triggered).
          2. **`outbound_` SpinMutex contention or deadlock**: 100 user threads contending on a per-connection SpinMutex are fine in isolation, but the poll-thread `handle_write` *also* takes the same lock to drain. If the poll thread is starved of the lock for any reason (e.g., a user thread is mid-encode but yielded the CPU under high load), the drain stalls — and with kernel send buffers full, the user thread eventually returns `WouldBlock` and the future never resolves.
          3. **Server-side wedge**: while client logs show server "got new client" for all 100 fds, we never confirmed the server actually *received* and *replied to* the missing 60 first-frames. A targeted instrumentation pass on `ServerConnection::handle_read` (count `complete_requests` per call, count replies sent) would tell us whether the bytes left the wire at all — distinguishing a client send-side wedge from a server processing-side wedge.
          4. **Cross-thread `Future::notify_ready` ⇆ `Future::wait` deadlock**: notify_ready acquires `state_.lock()` (rusty::Mutex) and signals condvar; wait_while releases it for waiting and re-acquires on signal. If `record_request_completed` or any callback inside `notify_ready` tries to take a *different* lock that the user thread holds via `fu_result` post-processing, deadlock. The 40-vs-60 split is suspicious.

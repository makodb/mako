...
- [ ] **Phase 4 — Per-command-type migrations** (decompose by directory):
  - 4a — `deptran/classic/` (TPC commands)
    ...
    - [ ] **Phase 4a-2 — `TpcEmptyCommand`**. Has `event` member (`shared_ptr<BoxEvent<bool>>`) for sender↔apply synchronization on the leader. Needs `wrap_serializable_aliased<T>(shared_ptr<T>)` variant that holds the typed `shared_ptr` inside the adapter (preserves `*caller_ptr == *receiver_ptr` aliasing on the leader-local apply path).

...
| `mbta_ordered_index` | Silo/STO | hand-written C++ (`mbta_wrapper.hh`) | the exception boundary (`STD_OP` catch of `Transaction::Abort`, non-txn retry loops) is C++'s to own; full-DSL conversion with per-verb `try_op` kernels is unblocked and planned. |

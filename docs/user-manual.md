...
- `./docker_build.sh compose-up`/`compose-down` warn when additional running `mako-*-dev-1` containers are bound to the same checkout via different (legacy/manual) compose project IDs, so stale compose sessions are visible and can be stopped explicitly.

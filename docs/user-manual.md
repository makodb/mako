...
- If multiple stale compose projects are running for the same checkout, `./docker_build.sh enter` refuses to start another duplicate and asks you to choose one explicitly (via `MAKO_COMPOSE_PROJECT=...`) with both interactive and non-interactive command forms, or stop stale projects first.

...
- `./docker_build.sh compose-up`/`compose-down`/`enter` and non-interactive `shell` guidance warn when running `mako-*-dev-1` containers for the same checkout are bound to non-current (legacy/manual) compose project IDs, print the stale project IDs, and include `MAKO_COMPOSE_PROJECT=... docker compose down` cleanup commands.

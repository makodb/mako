...
- In non-interactive mode, `./docker_build.sh shell` prints guidance instead of trying to open an interactive shell (running standalone `mako-dev` guidance when available, explicit `docker start` guidance when standalone exists but is stopped, direct compose-exec guidance when compose `dev` is already running, and compose-start guidance otherwise). For stopped standalone containers (including when compose `dev` is running), guidance now also points to `./docker_build.sh enter`, which auto-recovers legacy immediate-exit containers.
...
- `./docker_build.sh create` and `./docker_build.sh enter` also auto-recover legacy standalone containers that exit immediately after `docker start` (for example old `/bin/bash` command setups), recreating them with a persistent keepalive command before suggesting `docker exec`.

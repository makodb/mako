...
- Non-interactive `./docker_build.sh shell` now also prints compose access commands plus project-scoped teardown commands (`... docker compose down`) for matching compose sessions (single project) or explicit selection guidance (multiple projects), even when standalone is already running.

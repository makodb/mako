...
- `./docker_build.sh shell` now requires standalone `mako-dev` to stay running for a short stability window and use a persistent keepalive command before advertising direct `docker exec` commands, so transient or non-keepalive legacy containers are redirected to recovery-safe guidance.
...
- Docker bind mounts in `docker_build.sh` are normalized to the checkout's canonical path (`pwd -P`), so symlinked working directories do not trigger false workspace mismatch checks or mount creation failures.

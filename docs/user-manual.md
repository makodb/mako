...
- In non-interactive mode, `./docker_build.sh shell` prints guidance instead of trying to open an interactive shell (running standalone `mako-dev` guidance when available, explicit `docker start` guidance when standalone exists but is stopped, direct compose-exec guidance when compose `dev` is already running, and compose-start guidance otherwise). For stopped standalone containers, guidance now also points to `./docker_build.sh enter`, which auto-recovers legacy immediate-exit containers.

...
- In non-interactive mode, `./docker_build.sh shell` prints guidance instead of trying to open an interactive shell (standalone `mako-dev` guidance when present, compose `dev` guidance otherwise).

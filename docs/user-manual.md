...
- `./docker_build.sh shell` now re-checks standalone liveness before advertising direct `docker exec` commands, so transient legacy containers that exit immediately are redirected to recovery-safe guidance.

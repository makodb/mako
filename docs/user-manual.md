...
- If compose `dev` is already running for this checkout (current project or stale project), `./docker_build.sh create` warns that it is starting/recovering standalone in parallel; once standalone is running, `./docker_build.sh enter` will prefer standalone.
...
- `./docker_build.sh compose-up`/`compose-down`/`enter` and non-interactive `shell` guidance warn when additional running `mako-*-dev-1` containers are bound to the same checkout via different (legacy/manual) compose project IDs, print the stale project IDs, and include `MAKO_COMPOSE_PROJECT=... docker compose down` cleanup commands.
- Non-interactive `./docker_build.sh shell` now also prints compose access commands for matching stale compose sessions (single project) or explicit selection guidance (multiple projects), even when standalone is already running.
...
- When `compose-up` reuses a stale compose project, it prints the exact teardown command (`MAKO_COMPOSE_PROJECT=... docker compose down`) because plain `./docker_build.sh compose-down` still targets the current project ID.
- When `enter` reuses a stale compose project (for example standalone is missing/stopped), it also prints the same teardown command so users can stop the reused project without guessing project IDs.
- `./docker_build.sh compose-down` now distinguishes full vs partial shutdown: if stale same-checkout compose projects are still running, it reports that only the current compose project was stopped and exits non-zero.

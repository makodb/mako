...
- `./docker_build.sh shell` now requires standalone `mako-dev` to stay running for a short stability window and pass image/workspace/keepalive checks before advertising direct `docker exec` commands, so transient or mismatched legacy containers are redirected to recovery-safe guidance.

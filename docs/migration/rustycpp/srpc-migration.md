# SRPC/RPC RustyCpp Memory Safety Migration Plan
...
> `src/srpc/RUST_CANARY.md`. The former `basetypes.hpp/cpp` provider has been
> replaced by canonical `src/srpc/src/basetypes.rs` and generated
> `srpc.basetypes` C++ module output.

## Overview
This document outlines the plan to make the SRPC/RPC library memory-safe using rusty-cpp borrow checking. The goal is to mark all functions as safe and only use `unsafe` when absolutely necessary.

## Phase 1: Assessment and Preparation (Week 1)

### 1.1 Inventory Current State
- [ ] Count total files in `src/srpc/`
...
- [ ] Enable borrow checking for SRPC in CMakeLists.txt
...
### 2.2 Marshaling (`src/srpc/misc/marshal.hpp/cpp`)
...
### 2.3 Memory Management (`src/srpc/misc/`)
Priority: **High**
...
- [ ] Entire SRPC library passes borrow checking
- [ ] All tests pass
...
### Primary Goals
- [ ] 100% of SRPC files pass borrow checking

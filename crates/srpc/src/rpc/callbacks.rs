...
// Native Rust uses `String`. The Mako consumer profile maps this private alias
...
pub type ErrorCallback =
    Arc<Box<dyn Fn(crate::rpc::errors::RpcError, &LegacyStdString) + Send + Sync>>;
...
fn invoke_error_callback_safely(
    ...
    error: crate::rpc::errors::RpcError,
...
impl CallbackManager {
    ...
    pub fn add_on_error(
        &self,
        callback: Box<dyn Fn(crate::rpc::errors::RpcError, &LegacyStdString) + Send + Sync>,
    ) {
    ...
    pub fn invoke_on_error(&self, error: crate::rpc::errors::RpcError, message: &LegacyStdString) {
        self.inflight_enter();
        let callbacks = {
            let guard = self.callbacks_field.lock().unwrap();

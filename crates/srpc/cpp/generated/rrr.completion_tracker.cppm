...
// srpc-cpp-source: crates/srpc/src/rpc/completion_tracker.rs
// srpc-cpp-source-sha256: 0d3f46546c5b1e20f6b2d2ff3735780b10984981cf4d543fc0ce2fed0de9203e
...
// srpc-cpp-unit-id: rrr.completion_tracker.interface
// srpc-cpp-module: rrr.completion_tracker
...
// srpc-cpp-output-sha256: 3ccc5e90a16a1caed823b72c6bf6e095f1c2bd7f7499c64c044d3ea6b1ecc3ba
...
export enum class CompletionStatus : uint8_t;
export constexpr CompletionStatus CompletionStatus_NOT_FOUND();
export constexpr CompletionStatus CompletionStatus_COMPLETED();
export constexpr CompletionStatus CompletionStatus_COMPLETED_WITH_ERROR();
export constexpr CompletionStatus CompletionStatus_EXPIRED();
...
export struct CompletionTrackerConfig {
    ...
    CompletionTrackerConfig clone() const;
...
export struct CompletionTracker {
    ...
    CompletionTracker();
    CompletionTracker(::rrr::CompletionTrackerConfig config);
    bool enabled() const;
    CompletionTrackerConfig config() const;
    void set_config(::rrr::CompletionTrackerConfig config);
...
export std::string_view completion_status_to_string(::rrr::CompletionStatus status) {
    return [&]() -> std::string_view { auto&& _m = status; if (_m == CompletionStatus::NOT_FOUND) return std::string_view("NOT_FOUND");
if (_m == CompletionStatus::COMPLETED) return std::string_view("COMPLETED");
if (_m == CompletionStatus::COMPLETED_WITH_ERROR) return std::string_view("COMPLETED_WITH_ERROR");
if (_m == CompletionStatus::EXPIRED) return std::string_view("EXPIRED");
return std::string_view("UNKNOWN"); }();
}


CompletionTrackerConfig CompletionTrackerConfig::clone() const {
    return CompletionTrackerConfig{.ttl_ms = this->ttl_ms, .max_entries = this->max_entries, .enabled = this->enabled};
}
...
CompletionTracker::CompletionTracker(::rrr::CompletionTrackerConfig config)
...
void CompletionTracker::set_config(::rrr::CompletionTrackerConfig config) {
...
void CompletionTracker::mark_completed(int64_t xid, uint64_t current_time_ms) {
    ...
    if (rusty::detail::rust_not([&](auto&& __r) -> decltype(auto) { if constexpr (requires { (__r.enabled); }) { return (__r.enabled); } else if constexpr (requires { (__r.enabled_field); }) { return (__r.enabled_field); } else if constexpr (requires { ((*__r).enabled); }) { return ((*__r).enabled); } else { return ((*__r).enabled_field); } }(config))) {
        return;
    }
    auto completed = this->completed_set_.lock().unwrap();
    if (rusty::contains(completed, &xid)) {
        return;
    }
    auto lru = this->lru_list_.lock().unwrap();
    while ((rusty::len(lru) >= rusty::detail::deref_if_pointer_like([&](auto&& __r) -> decltype(auto) { if constexpr (requires { (__r.max_entries); }) { return (__r.max_entries); } else if constexpr (requires { (__r.max_entries_field); }) { return (__r.max_entries_field); } else if constexpr (requires { ((*__r).max_entries); }) { return ((*__r).max_entries); } else { return ((*__r).max_entries_field); } }(config))) && rusty::detail::rust_not(rusty::is_empty(lru))) {
        auto oldest_xid = (*lru).back().unwrap().xid;
        (*completed).remove(oldest_xid);
        (*lru).pop_back();
        this->evictions_.set(rusty::wrapping_add(this->evictions_.get(), static_cast<std::remove_cvref_t<decltype(this->evictions_.get())>>(1)));
    }
    (*lru).push_front(CompletedEntry::new_(std::move(xid), std::move(current_time_ms)));
    (*completed).insert(std::move(xid));
    this->total_tracked_.set(rusty::wrapping_add(this->total_tracked_.get(), static_cast<std::remove_cvref_t<decltype(this->total_tracked_.get())>>(1)));
}

bool CompletionTracker::is_completed(int64_t xid, uint64_t current_time_ms) {
    const auto config = this->config_.get();
    this->queries_.set(rusty::wrapping_add(this->queries_.get(), static_cast<std::remove_cvref_t<decltype(this->queries_.get())>>(1)));
    if (rusty::detail::rust_not([&](auto&& __r) -> decltype(auto) { if constexpr (requires { (__r.enabled); }) { return (__r.enabled); } else if constexpr (requires { (__r.enabled_field); }) { return (__r.enabled_field); } else if constexpr (requires { ((*__r).enabled); }) { return ((*__r).enabled); } else { return ((*__r).enabled_field); } }(config))) {
        return false;
    }
    auto completed = this->completed_set_.lock().unwrap();
    if (rusty::detail::rust_not(rusty::contains(completed, &xid))) {
        return false;
    }
    auto lru = this->lru_list_.lock().unwrap();
    size_t index = static_cast<size_t>(0);
    while (rusty::detail::deref_if_pointer_like(index) < rusty::len(lru)) {
        if (rusty::detail::deref_if_pointer_like([&](auto&& __r) -> decltype(auto) { if constexpr (requires { (__r.xid); }) { return (__r.xid); } else if constexpr (requires { (__r.xid_field); }) { return (__r.xid_field); } else if constexpr (requires { ((*__r).xid); }) { return ((*__r).xid); } else { return ((*__r).xid_field); } }(lru[index])) == rusty::detail::deref_if_pointer_like(xid)) {
            if (lru[index].is_expired(std::move(current_time_ms), std::move([&](auto&& __r) -> decltype(auto) { if constexpr (requires { (__r.ttl_ms); }) { return (__r.ttl_ms); } else if constexpr (requires { (__r.ttl_ms_field); }) { return (__r.ttl_ms_field); } else if constexpr (requires { ((*__r).ttl_ms); }) { return ((*__r).ttl_ms); } else { return ((*__r).ttl_ms_field); } }(config)))) {
                (*completed).remove(xid);
                (*lru).remove(std::move(index));
                return false;
            }
            this->query_hits_.set(rusty::wrapping_add(this->query_hits_.get(), static_cast<std::remove_cvref_t<decltype(this->query_hits_.get())>>(1)));
            return true;
        }
        index += 1;
    }
...
size_t CompletionTracker::size() const {
    return rusty::len(this->completed_set_.lock().unwrap());
}
...
double CompletionTracker::hit_rate() const {
    const auto queries = this->queries_.get();
    if (rusty::detail::deref_if_pointer_like(queries) == 0) {
        return 0.0;
    }
    return ((static_cast<double>(this->query_hits_.get()))) / ((static_cast<double>(queries)));
}
...
size_t CompletionTracker::evict_expired(uint64_t current_time_ms) {
    const auto config = this->config_.get();
    if (rusty::detail::rust_not([&](auto&& __r) -> decltype(auto) { if constexpr (requires { (__r.enabled); }) { return (__r.enabled); } else if constexpr (requires { (__r.enabled_field); }) { return (__r.enabled_field); } else if constexpr (requires { ((*__r).enabled); }) { return ((*__r).enabled); } else { return ((*__r).enabled_field); } }(config)) || (rusty::detail::deref_if_pointer_like([&](auto&& __r) -> decltype(auto) { if constexpr (requires { (__r.ttl_ms); }) { return (__r.ttl_ms); } else if constexpr (requires { (__r.ttl_ms_field); }) { return (__r.ttl_ms_field); } else if constexpr (requires { ((*__r).ttl_ms); }) { return ((*__r).ttl_ms); } else { return ((*__r).ttl_ms_field); } }(config)) == 0)) {
    ...
    while (rusty::detail::deref_if_pointer_like(index) < rusty::len(lru)) {
        if (lru[index].is_expired(std::move(current_time_ms), std::move([&](auto&& __r) -> decltype(auto) { if constexpr (requires { (__r.ttl_ms); }) { return (__r.ttl_ms); } else if constexpr (requires { (__r.ttl_ms_field); }) { return (__r.ttl_ms_field); } else if constexpr (requires { ((*__r).ttl_ms); }) { return ((*__r).ttl_ms); } else { return ((*__r).ttl_ms_field); } }(config)))) {
            auto xid = [&](auto&& __r) -> decltype(auto) { if constexpr (requires { (__r.xid); }) { return (__r.xid); } else if constexpr (requires { (__r.xid_field); }) { return (__r.xid_field); } else if constexpr (requires { ((*__r).xid); }) { return ((*__r).xid); } else { return ((*__r).xid_field); } }(lru[index]);
            (*completed).remove(xid);
            (*lru).remove(std::move(index));
            evicted += 1;
        } else {
...
CompletionQueryResult CompletionQueryResult::completed(int32_t err_code, bool has_response) {
    auto status = (rusty::detail::deref_if_pointer_like(err_code) == static_cast<int32_t>(0) ? rusty::clone(CompletionStatus_COMPLETED()) : rusty::clone(CompletionStatus_COMPLETED_WITH_ERROR()));
    return CompletionQueryResult{.status = std::move(status), .error_code = std::move(err_code), .has_cached_response = std::move(has_response)};
}

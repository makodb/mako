...
// srpc-cpp-source: crates/srpc/src/base/legacy_logging.rs
// srpc-cpp-source-sha256: 15e27ab66784723cb7130c7c36999fd58d24ede4583c893f7460b6cb93c6d009
...
// srpc-cpp-unit-id: rrr.logging.interface
// srpc-cpp-module: rrr.logging
// srpc-cpp-unit-kind: interface
// srpc-cpp-type-map-sha256: 472dd8ffa2aeb359dc4fda492a05c262de3c5e64556595c32c0fb51e65d9c123
// srpc-cpp-legacy-dependencies-sha256: f202a40f8e4d0bdfc79ff4740ad6c5bd0b671f6b964498eefe5ace4f36b1c67e
// srpc-cpp-cpp-module-index-source-sha256: 453f5196ac6468d6fd63f7812856992d2f1043f4f71c2b80f2af41e4669637a6
// srpc-cpp-cpp-module-index-sha256: af53d6a2897c5f825802248202d68fe8c0eb15f82eac8a36642f970fe71352a9
// srpc-cpp-output-sha256: ff9842d259fbde90da61c9b12ed924c1869dc7b1b79b797e226e07bab3830e41
...
inline rusty::sync::atomic::AtomicI32 LOG_LEVEL_S = AtomicI32::new_(static_cast<int32_t>(4));
...
namespace logging_ffi {
    ...
    using ::rrr::LegacyCChar;

    extern "C" {
        const std::string::value_type* srpc_path_basename(const std::string::value_type* path);
        void srpc_time_now_str(std::string::value_type* now);
    }
...
export std::string_view log_level_tag(int32_t level) {
    return [&]() -> std::string_view { auto&& _m = level; if (_m == 0) return std::string_view("F ");
if (_m == 1) return std::string_view("E ");
if (_m == 2) return std::string_view("W ");
if (_m == 3) return std::string_view("I ");
if (_m == 4) return std::string_view("D ");
return std::string_view("? "); }();
}

/// Filter, decorate, and synchronously emit one preformatted message.
export void log_line(int32_t level, int32_t line, const int8_t* file, const std::string& msg) {
    if (rusty::detail::deref_if_pointer_like(level) > Log::DEBUG) {
        // @unsafe
        {
            ::rrr::verify(false);
        }
    ...
    if (rusty::detail::deref_if_pointer_like(level) <= Log::level_now()) {
        ::std::string out = rusty::default_like<::std::string>();
        out.append(::rrr::log_level_tag(std::move(level)));
        out.append("[");
        out.append(::rrr::log_basename(file));
        out.append(":");
        out.append(rusty::to_string(line));
        out.append("] ");
        out.append(::rrr::log_time_now());
        out.append(" | ");
        out.append(msg);
        ::rrr::log_sink_write(out);
    }
}
...
export void log_sink_write(const ::std::string& line) {
    // @unsafe
    {
        ...
        ::std::cout.put(static_cast<std::string::value_type>(static_cast<uint8_t>(10)));
        ::std::cout.flush();
    }
}
...
export ::std::string log_basename(const int8_t* fpath) {
    ::std::string out = rusty::default_like<::std::string>();
    const auto base = logging_ffi::srpc_path_basename(reinterpret_cast<const std::string::value_type*>(fpath));
    if ((base == nullptr)) {
        out.append("<unknown>");
        return std::move(out);
    }
    size_t index = static_cast<size_t>(0);
    while (*rusty::ptr::add(base, std::move(index)) != (static_cast<std::string::value_type>(0))) {
        out.push_back(*rusty::ptr::add(base, std::move(index)));
...
export ::std::string log_time_now() {
    ::std::string now = rusty::default_like<::std::string>();
    now.resize(static_cast<size_t>(24));
    // @unsafe
    {
        logging_ffi::srpc_time_now_str(now.data());
    }
    now.resize(static_cast<size_t>(23));
    return std::move(now);
}


void Log::set_level(int32_t level) {
    LOG_LEVEL_S.store(std::move(level), Ordering::Relaxed);
}

int32_t Log::level_now() {
    return LOG_LEVEL_S.load(Ordering::Relaxed);
}

...
// srpc-cpp-source: crates/srpc/src/rpc/idempotency.rs
// srpc-cpp-source-sha256: fae6bba29f3afe962486f103818d44c5a6eb994c02d9034e4332f45881722f3d
...
// srpc-cpp-unit-id: rrr.idempotency.interface
// srpc-cpp-module: rrr.idempotency
// srpc-cpp-unit-kind: interface
// srpc-cpp-type-map-sha256: e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855
// srpc-cpp-legacy-dependencies-sha256: dcb7a5ecbb37272863ac83e01ea13738dabc80756f0b434a47f92e084dfd4133
// srpc-cpp-cpp-module-index-source-sha256: af4519a480e7a8f564e89a4cd1ca87f1aed77315ca91cda297f7ed403decac70
// srpc-cpp-cpp-module-index-sha256: e5a3fa3124f6f38fbddda88c15536dea7ea691d4fdccf31501fc226e9d23608a
// srpc-cpp-output-sha256: dc0b81f178ca37731993581b7c4b27f1d661b0814758f30c0696c19c917a74c9
...
// C++ module import (reserved cpp::): rrr::serializable
...
export struct IdempotencyCache {
    ...
    IdempotencyCache();
    IdempotencyCache(::rrr::IdempotencyConfig config);
    bool enabled() const;
...
export void serialize(const IdempotencyKey& key, ::rrr::BinaryWriteArchive& archive) {
    ::rrr::Serialize_::serialize(key.client_id, archive);
    ::rrr::Serialize_::serialize(key.sequence, archive);
}
...
export void deserialize(IdempotencyKey& key, ::rrr::BinaryReadArchive& archive) {
    ::rrr::Deserialize_::deserialize(key.client_id, archive);
    ::rrr::Deserialize_::deserialize(key.sequence, archive);
}
...
IdempotencyKey IdempotencyKeyGenerator::next() const {
    uint64_t sequence = this->sequence_field.get();
    this->sequence_field.set(rusty::wrapping_add(sequence, static_cast<std::remove_cvref_t<decltype(sequence)>>(static_cast<uint64_t>(1))));
    return IdempotencyKey(this->client_id_field.get(), std::move(sequence));
}
...
IdempotencyCache::IdempotencyCache(::rrr::IdempotencyConfig config)
...
bool IdempotencyCache::lookup(const IdempotencyKey& key, uint64_t current_time_ms, int32_t& out_error_code, rusty::Vec<uint8_t>& out_response) const {
    ...
    const auto config = this->config_.get();
    if (rusty::detail::rust_not([&](auto&& __r) -> decltype(auto) { if constexpr (requires { (__r.enabled); }) { return (__r.enabled); } else if constexpr (requires { (__r.enabled_field); }) { return (__r.enabled_field); } else if constexpr (requires { ((*__r).enabled); }) { return ((*__r).enabled); } else { return ((*__r).enabled_field); } }(config)) || !key.is_valid()) {
        this->misses_.set(rusty::wrapping_add(this->misses_.get(), static_cast<std::remove_cvref_t<decltype(this->misses_.get())>>(static_cast<uint64_t>(1))));
        return false;
    }
    auto guard = this->cache_.lock().unwrap();
    size_t index = static_cast<size_t>(0);
    while (rusty::detail::deref_if_pointer_like(index) < rusty::len(guard)) {
        if (rusty::detail::deref_if_pointer_like([&](auto&& __r) -> decltype(auto) { if constexpr (requires { (__r.key); }) { return (__r.key); } else if constexpr (requires { (__r.key_field); }) { return (__r.key_field); } else if constexpr (requires { ((*__r).key); }) { return ((*__r).key); } else { return ((*__r).key_field); } }(guard[index])) == key) {
            if (guard[index].is_expired(std::move(current_time_ms), std::move([&](auto&& __r) -> decltype(auto) { if constexpr (requires { (__r.ttl_ms); }) { return (__r.ttl_ms); } else if constexpr (requires { (__r.ttl_ms_field); }) { return (__r.ttl_ms_field); } else if constexpr (requires { ((*__r).ttl_ms); }) { return ((*__r).ttl_ms); } else { return ((*__r).ttl_ms_field); } }(config)))) {
                (*guard).remove(index);
                this->misses_.set(rusty::wrapping_add(this->misses_.get(), static_cast<std::remove_cvref_t<decltype(this->misses_.get())>>(static_cast<uint64_t>(1))));
                return false;
            }
            *out_error_code_shadow1 = [&](auto&& __r) -> decltype(auto) { if constexpr (requires { (__r.error_code); }) { return (__r.error_code); } else if constexpr (requires { (__r.error_code_field); }) { return (__r.error_code_field); } else if constexpr (requires { ((*__r).error_code); }) { return ((*__r).error_code); } else { return ((*__r).error_code_field); } }(guard[index]);
            ::rrr::cached_response_get(guard[index], out_response);
            auto entry = (*guard).remove(index).unwrap();
            (*guard).push_front(std::move(entry));
            this->hits_.set(rusty::wrapping_add(this->hits_.get(), static_cast<std::remove_cvref_t<decltype(this->hits_.get())>>(static_cast<uint64_t>(1))));
            return true;
        }
...
void IdempotencyCache::store(const IdempotencyKey& key, int32_t error_code, const rusty::Vec<uint8_t>& response, uint64_t current_time_ms) const {
    ...
    while (rusty::detail::deref_if_pointer_like(index) < rusty::len(guard)) {
        if (rusty::detail::deref_if_pointer_like([&](auto&& __r) -> decltype(auto) { if constexpr (requires { (__r.key); }) { return (__r.key); } else if constexpr (requires { (__r.key_field); }) { return (__r.key_field); } else if constexpr (requires { ((*__r).key); }) { return ((*__r).key); } else { return ((*__r).key_field); } }(guard[index])) == key) {
            [&](auto&& __r) -> decltype(auto) { if constexpr (requires { (__r.error_code); }) { return (__r.error_code); } else if constexpr (requires { (__r.error_code_field); }) { return (__r.error_code_field); } else if constexpr (requires { ((*__r).error_code); }) { return ((*__r).error_code); } else { return ((*__r).error_code_field); } }(guard[index]) = std::move(error_code);
            ::rrr::cached_response_set(guard[index], response);
            [&](auto&& __r) -> decltype(auto) { if constexpr (requires { (__r.timestamp_ms); }) { return (__r.timestamp_ms); } else if constexpr (requires { (__r.timestamp_ms_field); }) { return (__r.timestamp_ms_field); } else if constexpr (requires { ((*__r).timestamp_ms); }) { return ((*__r).timestamp_ms); } else { return ((*__r).timestamp_ms_field); } }(guard[index]) = std::move(current_time_ms);
            auto entry = (*guard).remove(index).unwrap();
            (*guard).push_front(std::move(entry));
            return;
        }
    ...
    while ((rusty::len(guard) >= rusty::detail::deref_if_pointer_like([&](auto&& __r) -> decltype(auto) { if constexpr (requires { (__r.max_entries); }) { return (__r.max_entries); } else if constexpr (requires { (__r.max_entries_field); }) { return (__r.max_entries_field); } else if constexpr (requires { ((*__r).max_entries); }) { return ((*__r).max_entries); } else { return ((*__r).max_entries_field); } }(config))) && rusty::detail::rust_not(rusty::is_empty(guard))) {
        (*guard).pop_back();
        this->evictions_.set(rusty::wrapping_add(this->evictions_.get(), static_cast<std::remove_cvref_t<decltype(this->evictions_.get())>>(static_cast<uint64_t>(1))));
    }
    ...
    ::rrr::cached_response_set(entry, response);
    (*guard).push_front(std::move(entry));
}

bool IdempotencyCache::remove(const IdempotencyKey& key) const {
    auto guard = this->cache_.lock().unwrap();
    const auto size = rusty::len(guard);
    size_t index = static_cast<size_t>(0);
    while (rusty::detail::deref_if_pointer_like(index) < rusty::detail::deref_if_pointer_like(size)) {
        if ((rusty::detail::deref_if_pointer_like([&](auto&& __r) -> decltype(auto) { if constexpr (requires { (__r.key); }) { return (__r.key); } else if constexpr (requires { (__r.key_field); }) { return (__r.key_field); } else if constexpr (requires { ((*__r).key); }) { return ((*__r).key); } else { return ((*__r).key_field); } }(guard[index]).client_id) == rusty::detail::deref_if_pointer_like(key.client_id)) && (rusty::detail::deref_if_pointer_like([&](auto&& __r) -> decltype(auto) { if constexpr (requires { (__r.key); }) { return (__r.key); } else if constexpr (requires { (__r.key_field); }) { return (__r.key_field); } else if constexpr (requires { ((*__r).key); }) { return ((*__r).key); } else { return ((*__r).key_field); } }(guard[index]).sequence) == rusty::detail::deref_if_pointer_like(key.sequence))) {
            (*guard).remove(index);
            return true;
        }
        index += static_cast<size_t>(1);
...
double IdempotencyCache::hit_rate() const {
    ...
    const auto total = rusty::wrapping_add(h, static_cast<std::remove_cvref_t<decltype(h)>>(std::move(m)));
...
size_t IdempotencyCache::evict_expired(uint64_t current_time_ms) const {
    const auto config = this->config_.get();
    if (rusty::detail::rust_not([&](auto&& __r) -> decltype(auto) { if constexpr (requires { (__r.enabled); }) { return (__r.enabled); } else if constexpr (requires { (__r.enabled_field); }) { return (__r.enabled_field); } else if constexpr (requires { ((*__r).enabled); }) { return ((*__r).enabled); } else { return ((*__r).enabled_field); } }(config)) || (rusty::detail::deref_if_pointer_like([&](auto&& __r) -> decltype(auto) { if constexpr (requires { (__r.ttl_ms); }) { return (__r.ttl_ms); } else if constexpr (requires { (__r.ttl_ms_field); }) { return (__r.ttl_ms_field); } else if constexpr (requires { ((*__r).ttl_ms); }) { return ((*__r).ttl_ms); } else { return ((*__r).ttl_ms_field); } }(config)) == static_cast<uint64_t>(0))) {
    ...
    this->evictions_.set(rusty::wrapping_add(this->evictions_.get(), static_cast<std::remove_cvref_t<decltype(this->evictions_.get())>>(static_cast<uint64_t>(evicted))));

...
// srpc-cpp-source: crates/srpc/src/rpc/fiber_channel.rs
// srpc-cpp-source-sha256: 5c4e3bb8cd1919b226343ac288aad1a27c9a538b7fa973fec045589a2cd0e0da
...
// srpc-cpp-unit-id: rrr.fiber_channel.interface
// srpc-cpp-module: rrr.fiber_channel
// srpc-cpp-unit-kind: interface
// srpc-cpp-type-map-sha256: b141e3c5718ba4bb161d2224c9647c86b262aed5bffb6fe7dafbcec616755914
// srpc-cpp-legacy-dependencies-sha256: 302323385ca4ca5427bac7f830ebcafd363760ffcff3a8bfc55fc66cd029e26e
// srpc-cpp-cpp-module-index-source-sha256: c5df5e5cf2dcca46594ff8b5a10702ef2f08ccae3047855d967d686f0cd29638
// srpc-cpp-cpp-module-index-sha256: 904fc9330b07063dbd5b1bc872b0b4f5c84a0b01f23032c0ada42b907471756c
// srpc-cpp-output-sha256: 615edb5cff0dc54001f9d33be5b2cdc03b242d33c1743dd063f050d51ebc43cf
...
template<typename T>
using LegacyStdDeque = rusty::VecDeque<T>;
using LegacyChannelConnectionBase = void*;
...
// Rust-only: using std::marker::PhantomPinned;
...
export struct OwnedFrame {
    ...
    static OwnedFrame default_() { return {}; }
...
export struct FiberChannel {
    ::rrr::ChannelConnectionProxy ch_;
    rusty::Mutex<std::deque<OwnedFrame>> queue_;
    rusty::Mutex<rusty::Option<rusty::Arc<::rrr::IntEvent>>> pending_recv_event_;
    ...
    void rusty_mark_forgotten() const noexcept { _rusty_forgotten = true; rusty::detail::mark_forgotten_if_supported(this->ch_); rusty::detail::mark_forgotten_if_supported(this->queue_); rusty::detail::mark_forgotten_if_supported(this->pending_recv_event_); rusty::detail::mark_forgotten_if_supported(this->closed_); rusty::detail::mark_forgotten_if_supported(this->_pin); }
    ...
    void on_inbound_frame(const ::rrr::ChannelFrame& frame);
    ...
    ::rrr::ChannelConnectionProxy& channel_for_test();
    ~FiberChannel() noexcept(true);
};

OwnedFrame fiberchannel_owned_copy(const ::rrr::ChannelFrame& frame) {
    OwnedFrame owned = rusty::default_like<OwnedFrame>();
    if ((rusty::detail::deref_if_pointer_like(frame.size) > static_cast<size_t>(0)) && rusty::detail::rust_not((frame.payload == nullptr))) {
        owned.bytes.resize(frame.size, static_cast<uint8_t>(0));
        // @unsafe
        {
            rusty::ptr::copy_nonoverlapping(frame.payload, rusty::as_mut_ptr(owned.bytes), frame.size);
        }
    }
...
FiberChannel::FiberChannel(::rrr::ChannelConnectionProxy ch)
    : ch_(std::move(ch))
    , queue_(rusty::Mutex<std::deque<OwnedFrame>>::new_(rusty::default_like<std::deque<OwnedFrame>>()))
    , pending_recv_event_(rusty::Mutex<rusty::Option<rusty::Arc<::rrr::IntEvent>>>::new_(rusty::Option<rusty::Arc<::rrr::IntEvent>>{rusty::None}))
...
void FiberChannel::bind_callbacks() {
    ...
    FiberChannel* frame_self = self_ptr;
    rusty::Function<void(const ::rrr::ChannelFrame&) const> frame_callback = rusty::Box<std::function<void(const ::rrr::ChannelFrame&)>>::new_([=, frame_self = std::move(frame_self)](auto&& frame) {
// @unsafe
{
    ((*frame_self)).on_inbound_frame(rusty::detail::deref_if_pointer_like(frame));
}
});
    rusty::Box<rrr::ChannelConnectionBase>& ch = this->ch_;
    ch->set_on_frame(::rrr::OnFrameCallback::from_callable(std::move(frame_callback)));
    FiberChannel* closed_self = self_ptr;
    rusty::Function<void(::rrr::ChannelError) const> closed_callback = rusty::Box<std::function<void(::rrr::ChannelError)>>::new_([=, closed_self = std::move(closed_self)](auto&& _reason) {
// @unsafe
{
    ((*closed_self)).on_inbound_closed();
}
});
    ch->set_on_closed(::rrr::OnClosedCallback::from_callable(std::move(closed_callback)));
    rusty::Function<void(::rrr::ChannelError, std::string_view) const> error_callback = rusty::Box<std::function<void(::rrr::ChannelError, std::string_view)>>::new_([=](auto&& _error, auto&& _message) {
});
    ch->set_on_error(::rrr::OnErrorCallback::from_callable(std::move(error_callback)));
}

rusty::Option<OwnedFrame> FiberChannel::try_pop() {
    auto guard = this->queue_.lock().unwrap();
    if (rusty::is_empty(guard)) {
        return rusty::Option<OwnedFrame>{rusty::None};
    }
    OwnedFrame frame = rusty::mem::take(guard[static_cast<size_t>(0)]);
    (*guard).pop_front();
    return rusty::Option<OwnedFrame>(std::move(frame));
}

rusty::Option<OwnedFrame> FiberChannel::recv_frame() {
    while (true) {
        if (auto&& _iflet_scrutinee = this->try_pop(); _iflet_scrutinee.is_some()) {
            decltype(auto) frame = _iflet_scrutinee.unwrap();
            return rusty::Option<OwnedFrame>(std::move(frame));
        }
        if (this->closed_.get()) {
            return rusty::Option<OwnedFrame>{rusty::None};
        ...
        {
            auto guard = this->queue_.lock().unwrap();
            if (rusty::detail::rust_not(rusty::is_empty(guard)) || this->closed_.get()) {
                should_wait = false;
            }
        }
        if (should_wait) {
        ...
        auto event_guard = this->pending_recv_event_.lock().unwrap();
        *event_guard = rusty::None;
    }
}

void FiberChannel::on_inbound_frame(const ::rrr::ChannelFrame& frame) {
    OwnedFrame copy = ::rrr::fiberchannel_owned_copy(frame);
    {
        auto guard = this->queue_.lock().unwrap();
        (*guard).push_back(std::move(copy));
    }
    this->signal_pending_recv();
}
...
void FiberChannel::signal_pending_recv() {
    rusty::Option<rusty::Arc<::rrr::IntEvent>> held = [&]() -> rusty::Option<rusty::Arc<::rrr::IntEvent>> { auto guard = this->pending_recv_event_.lock().unwrap();
return rusty::clone(((*guard))); }();
    if (held.is_some()) {
        decltype(auto) event = held.unwrap();
        // @unsafe
        {
            rusty::detail::deref_if_pointer_like(event).set(static_cast<int32_t>(1));
        }
    }
}

void FiberChannel::arm_waiter() {
    rusty::Arc<::rrr::IntEvent> event = ::rrr::create_sp_int_event(static_cast<int32_t>(1));
    auto guard = this->pending_recv_event_.lock().unwrap();
    *guard = rusty::Option<rusty::Arc<::rrr::IntEvent>>(std::move(event));
}

void FiberChannel::wait_for_signal() {
    rusty::Option<rusty::Arc<::rrr::IntEvent>> held = [&]() -> rusty::Option<rusty::Arc<::rrr::IntEvent>> { auto guard = this->pending_recv_event_.lock().unwrap();
return rusty::clone(((*guard))); }();
    if (held.is_some()) {
        decltype(auto) event = held.unwrap();
        // @unsafe
        {
            rusty::detail::deref_if_pointer_like(event).wait();
        }
    }
}

::rrr::ChannelError FiberChannel::send_frame(const ::rrr::ChannelFrame& frame) {
    rusty::Box<rrr::ChannelConnectionBase>& ch = this->ch_;
    return ch->send_frame(frame);
}
...
bool FiberChannel::is_closed() const {
    ...
    const rusty::Box<rrr::ChannelConnectionBase>& ch = this->ch_;
    return ch->is_closed();
}

::rrr::ChannelConnectionProxy& FiberChannel::channel_for_test() {
    return this->ch_;
}

FiberChannel::~FiberChannel() noexcept(true) {
    if (_rusty_forgotten) { return; }
    rusty::Box<rrr::ChannelConnectionBase>& ch = this->ch_;
    ch->set_on_frame(::rrr::OnFrameCallback::default_());
    ch->set_on_closed(::rrr::OnClosedCallback::default_());
    ch->set_on_error(::rrr::OnErrorCallback::default_());
}

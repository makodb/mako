...
// srpc-cpp-source: crates/srpc/src/rpc/any_message.rs
// srpc-cpp-source-sha256: 578b192ea5dbab6571a3fe883ff324c1e5024f9755eb6379f8ff5f2fecd44476
...
// srpc-cpp-unit-id: rrr.any_message.interface
// srpc-cpp-module: rrr.any_message
...
// srpc-cpp-cpp-module-index-source-sha256: 41a9e011a41365bf81e6a0709f97d6330a9f86f2d1b78023626390e073348fd3
// srpc-cpp-cpp-module-index-sha256: e66a84e521430b9c542e99029d5dc16902a5fc5eb0723e3e4b3ae059f1af0042
// srpc-cpp-output-sha256: 3ac62bb9453d6cd37b8ef6c6fb67dfcf2f1f7abc48b8a7bf2637218f4662164e
...
namespace any_message_registry {
    struct RegistryMap;
    export using Factory = rusty::Function<SerializableProxy()>;
    const ::rusty::Mutex<rusty::Option<RegistryMap>>& registry();
    RegistryMap& map_mut(rusty::MutexGuard<rusty::Option<RegistryMap>>& guard);
...
export template<typename T>
bool anymessage_is_a(const AnyMessage& message);
export template<typename T>
rusty::Option<rusty::Arc<T>> anymessage_unpack(const AnyMessage& message);
export template<typename T>
AnyMessage anymessage_pack_as(std::string name, rusty::Arc<T> value);
export template<typename T>
AnyMessage anymessage_pack(rusty::Arc<T> value);

// C++ module import (reserved cpp::): rrr::debugging
...
namespace any_message_registry {
    ...
    export rusty::Option<SerializableProxy> create(const std::string& name);
    export std::string name_for_type_owned(::std::type_index type_id);
    export bool is_registered_name(const std::string& name);
    export bool is_registered_type(::std::type_index type_id);
    export void clear_for_testing();
    ...
    struct RegistryMap {
        rusty::HashMap<std::string, Factory> by_name;
...
export template<typename T>
int32_t reg_any_message_as(std::string name) {
    rusty::Function<SerializableProxy()> factory = rusty::Function<SerializableProxy()>([&]() -> SerializableProxy {
auto pointer = rusty::Arc<T>::make();
auto holder = ::rrr::details::SerializableSharedPtrHolder<T>(std::move(pointer));
rusty::Arc<::rrr::details::SerializableSharedPtrHolder<T>> concrete = rusty::Arc<::rrr::details::SerializableSharedPtrHolder<T>>::make(std::move(holder));
return std::move(concrete);
});
    return any_message_registry::register_type(std::move(name), std::type_index(typeid(T)), std::move(factory));
}

export template<typename T>
bool anymessage_is_a(const AnyMessage& message) {
    const auto name = any_message_registry::name_for_type_owned(std::type_index(typeid(T)));
    if (rusty::is_empty(name)) {
        return false;
    }
    return rusty::detail::deref_if_pointer_like(message.type_name_) == rusty::detail::deref_if_pointer_like(name);
}

export template<typename T>
rusty::Option<rusty::Arc<T>> anymessage_unpack(const AnyMessage& message) {
    if (!::rrr::anymessage_is_a<T>(message) || message.payload_.is_none()) {
        return rusty::Option<rusty::Arc<T>>{rusty::None};
    }
    auto& payload = message.payload_.as_ref().unwrap();
    const auto base = payload.get();
    const auto holder = ::rrr::serializable_holder_of<T>(std::move(base));
    if ((holder == nullptr)) {
        return rusty::Option<rusty::Arc<T>>{rusty::None};
    }
...
export template<typename T>
AnyMessage anymessage_pack_as(std::string name, rusty::Arc<T> value) {
    auto holder = ::rrr::details::SerializableSharedPtrHolder<T>(std::move(value));
    rusty::Arc<::rrr::details::SerializableSharedPtrHolder<T>> concrete = rusty::Arc<::rrr::details::SerializableSharedPtrHolder<T>>::make(std::move(holder));
    SerializableProxy payload = concrete;
    return AnyMessage{.type_name_ = std::move(name), .payload_ = rusty::Option<SerializableProxy>(std::move(payload))};
}

export template<typename T>
AnyMessage anymessage_pack(rusty::Arc<T> value) {
    auto name = any_message_registry::name_for_type_owned(std::type_index(typeid(T)));
    // @unsafe
    {
        ::rrr::verify(rusty::detail::rust_not(rusty::is_empty(name)));
    }
    return ::rrr::anymessage_pack_as<T>(std::move(name), std::move(value));
}
...
namespace any_message_registry {
    ...
    const rusty::Mutex<rusty::Option<RegistryMap>>& registry() {
        static rusty::Mutex<rusty::Option<RegistryMap>> REGISTRY = rusty::Mutex<rusty::Option<RegistryMap>>::new_(rusty::Option<RegistryMap>{rusty::None});
        return REGISTRY;
    }

    RegistryMap& map_mut(rusty::MutexGuard<rusty::Option<RegistryMap>>& guard) {
        rusty::MutexGuard<rusty::Option<RegistryMap>>* guard_shadow1 = &guard;
        if ((*((*guard_shadow1))).is_none()) {
            rusty::detail::deref_if_pointer_like(*guard_shadow1) = rusty::Option<RegistryMap>(RegistryMap{.by_name = rusty::HashMap<std::string, Factory>(), .name_by_type_hash = rusty::HashMap<size_t, std::string>()});
        }
        return (*((*guard_shadow1))).as_mut().unwrap();
    }

    export int32_t register_type(std::string name, ::std::type_index type_id, ::rrr::any_message_registry::Factory factory) {
        auto guard = registry().lock().unwrap();
        const auto hash = (type_id).hash_code();
        RegistryMap& map = map_mut(guard);
        // @unsafe
        {
            ::rrr::verify(map.by_name.get(name).is_none());
        }
        if (map.name_by_type_hash.get(hash).is_none()) {
            map.name_by_type_hash.insert(std::move(hash), rusty::clone(name));
        }
        map.by_name.insert(std::move(name), std::move(factory));
    ...
    export rusty::Option<SerializableProxy> create(const std::string& name) {
        auto guard = registry().lock().unwrap();
        RegistryMap& map = map_mut(guard);
        return [&]() -> rusty::Option<SerializableProxy> { auto&& _m = map.by_name.get(name); if (rusty::detail::deref_if_pointer(_m).is_some()) { auto&& _mv0 = rusty::detail::deref_if_pointer(_m).unwrap(); auto&& factory = rusty::detail::deref_if_pointer(_mv0); return rusty::Option<SerializableProxy>(factory()); } if (rusty::detail::deref_if_pointer(_m).is_none()) { return rusty::Option<SerializableProxy>{rusty::None}; } return [&]() -> rusty::Option<SerializableProxy> { rusty::intrinsics::unreachable_panic(); }(); }();
    }
    ...
    export std::string name_for_type_owned(::std::type_index type_id) {
        ...
        const auto hash = (type_id).hash_code();
        RegistryMap& map = map_mut(guard);
        return [&]() -> std::string { auto&& _m = map.name_by_type_hash.get(hash); if (rusty::detail::deref_if_pointer(_m).is_some()) { auto&& _mv0 = rusty::detail::deref_if_pointer(_m).unwrap(); auto&& name = rusty::detail::deref_if_pointer(_mv0); return rusty::clone(name); } if (rusty::detail::deref_if_pointer(_m).is_none()) { return rusty::default_like<std::string>(); } return [&]() -> std::string { rusty::intrinsics::unreachable_panic(); }(); }();
    }

    export bool is_registered_name(const std::string& name) {
        auto guard = registry().lock().unwrap();
        return map_mut(guard).by_name.get(name).is_some();
    }

    export bool is_registered_type(::std::type_index type_id) {
        auto guard = registry().lock().unwrap();
        const auto hash = (type_id).hash_code();
        return map_mut(guard).name_by_type_hash.get(hash).is_some();
    }
...
void AnyMessage::save(::rrr::BinaryWriteArchive& archive) const {
    // @unsafe
    {
        ::rrr::Serialize_::serialize(this->type_name_, archive);
    }
    if (this->payload_.is_some()) {
        auto& payload = this->payload_.as_ref().unwrap();
        const auto base = payload.get();
        // @unsafe
        {
            ((rusty::detail::deref_if_pointer_like(base))).save(archive);
        }
    }
}

void AnyMessage::load(::rrr::BinaryReadArchive& archive) {
    // @unsafe
    {
        ::rrr::Deserialize_::deserialize(this->type_name_, archive);
    }
    auto proxy_option = any_message_registry::create(this->type_name_);
    // @unsafe
    {
        ::rrr::verify(proxy_option.is_some());
    }
    auto proxy = proxy_option.unwrap();
...
bool AnyMessage::is_a() const {
    return ::rrr::anymessage_is_a<T>((*this));
}

template<typename T>
rusty::Option<rusty::Arc<T>> AnyMessage::unpack() const {
    return ::rrr::anymessage_unpack<T>((*this));
}

template<typename T>
AnyMessage AnyMessage::pack_as(std::string name, rusty::Arc<T> value) {
    return ::rrr::anymessage_pack_as<T>(std::move(name), std::move(value));
}

template<typename T>
AnyMessage AnyMessage::pack(rusty::Arc<T> value) {
    return ::rrr::anymessage_pack<T>(std::move(value));
}

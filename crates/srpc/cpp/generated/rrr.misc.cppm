...
// srpc-cpp-source: crates/srpc/src/base/misc.rs
// srpc-cpp-source-sha256: 7a7ba37200421c51724a559967f1a3b05d8d5fe2909b847c2517c9a693ff28d9
...
// srpc-cpp-unit-id: rrr.misc.interface
// srpc-cpp-module: rrr.misc
...
// srpc-cpp-output-sha256: e9c6f18802eec11f0b9a7e6eea69560723e48e9c3642af36d7a23a29b391f202
...
export std::string format_thousands(double val) {
    const std::string formatted = std::format("{:.2f}", val);
    const auto bytes = rusty::as_bytes(formatted);
    auto dot = static_cast<size_t>(0);
    while (rusty::detail::deref_if_pointer_like(dot) < rusty::len(bytes)) {
        if (bytes[dot] == static_cast<uint8_t>(46)) {
            break;
        }
        dot += 1;
    }
    ...
    while (rusty::detail::deref_if_pointer_like(index) < rusty::detail::deref_if_pointer_like(dot)) {
        if ((((((rusty::detail::deref_if_pointer_like(dot) - rusty::detail::deref_if_pointer_like(index))) % static_cast<int32_t>(3)) == static_cast<size_t>(0)) && (rusty::detail::deref_if_pointer_like(index) != static_cast<size_t>(0))) && (bytes[rusty::detail::deref_if_pointer_like(index) - 1] != static_cast<uint8_t>(45))) {
            out = std::format("{}," , out);
        }
        out = std::format("{}{}", rusty::to_string(out), rusty::to_string(static_cast<char32_t>(bytes[index])));
        index += 1;
    }
    while (rusty::detail::deref_if_pointer_like(index) < rusty::len(bytes)) {
        out = std::format("{}{}", rusty::to_string(out), rusty::to_string(static_cast<char32_t>(bytes[index])));
        index += 1;
    }
    if (out == "-0.00") {

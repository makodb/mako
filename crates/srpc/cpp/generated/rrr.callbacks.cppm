...
// srpc-cpp-source: crates/srpc/src/rpc/callbacks.rs
// srpc-cpp-source-sha256: 260e3808886b68402864713576f64d362a44559bda34197fbbd8ffdbcabdac89
...
// srpc-cpp-unit-id: rrr.callbacks.interface
// srpc-cpp-module: rrr.callbacks
...
// srpc-cpp-output-sha256: d077759858970fb50105554c61c76eb426b70cb79770db4f948c2098f352b1b8
...
export using ErrorCallback = rusty::Arc<rusty::Function<void(::rrr::RpcError, const std::string&) const>>;
export using ReconnectCallback = rusty::Arc<rusty::Function<void(bool) const>>;
void invoke_connection_callback_safely(const ConnectionCallback& callback);
void invoke_reconnect_callback_safely(const ReconnectCallback& callback, bool success);
...
export struct CallbackManager {
    ...
    void add_on_connected(rusty::Function<void() const> callback) const;
    void add_on_disconnected(rusty::Function<void() const> callback) const;
    void add_on_error(rusty::Function<void(::rrr::RpcError, const std::string&) const> callback) const;
    void add_on_reconnecting(rusty::Function<void() const> callback) const;
    void add_on_reconnected(rusty::Function<void(bool) const> callback) const;
...
(rusty::detail::deref_if_pointer_like(callback))();
})));
}

void invoke_error_callback_safely(const ErrorCallback& callback, ::rrr::RpcError error, const std::string& message) {
    static_cast<void>(catch_unwind(AssertUnwindSafe([&]() {
(rusty::detail::deref_if_pointer_like(callback))(std::move(error), message);
})));
}

void invoke_reconnect_callback_safely(const ReconnectCallback& callback, bool success) {
    static_cast<void>(catch_unwind(AssertUnwindSafe([&]() {
(rusty::detail::deref_if_pointer_like(callback))(std::move(success));
})));
}
...
size_t ConnectionCallbacks::total_count() const {
    return (((rusty::len(this->on_connected) + rusty::len(this->on_disconnected)) + rusty::len(this->on_error)) + rusty::len(this->on_reconnecting)) + rusty::len(this->on_reconnected);
}
...
CallbackManager CallbackManager::new_() {
    return CallbackManager{.callbacks_field = rusty::Mutex<ConnectionCallbacks>::new_(ConnectionCallbacks::new_()), .inflight_field = rusty::Mutex<size_t>::new_(static_cast<size_t>(0)), .inflight_cv_field = rusty::Box<rusty::Condvar>::new_(Condvar::new_())};
}

void CallbackManager::inflight_enter() const {
    auto guard = this->inflight_field.lock().unwrap();
    *guard += 1;
}
...
void CallbackManager::add_on_connected(rusty::Function<void() const> callback) const {
    ConnectionCallback callback_shadow1 = Arc<rusty::Function<void() const>>::make(std::move(callback));
    auto guard = this->callbacks_field.lock().unwrap();
    (*guard).on_connected.push(std::move(callback_shadow1));
}

void CallbackManager::add_on_disconnected(rusty::Function<void() const> callback) const {
    ConnectionCallback callback_shadow1 = Arc<rusty::Function<void() const>>::make(std::move(callback));
    auto guard = this->callbacks_field.lock().unwrap();
    (*guard).on_disconnected.push(std::move(callback_shadow1));
}

void CallbackManager::add_on_error(rusty::Function<void(::rrr::RpcError, const std::string&) const> callback) const {
    ErrorCallback callback_shadow1 = Arc<rusty::Function<void(::rrr::RpcError, const std::string&) const>>::make(std::move(callback));
    auto guard = this->callbacks_field.lock().unwrap();
    (*guard).on_error.push(std::move(callback_shadow1));
}

void CallbackManager::add_on_reconnecting(rusty::Function<void() const> callback) const {
    ConnectionCallback callback_shadow1 = Arc<rusty::Function<void() const>>::make(std::move(callback));
    auto guard = this->callbacks_field.lock().unwrap();
    (*guard).on_reconnecting.push(std::move(callback_shadow1));
}

void CallbackManager::add_on_reconnected(rusty::Function<void(bool) const> callback) const {
    ReconnectCallback callback_shadow1 = Arc<rusty::Function<void(bool) const>>::make(std::move(callback));
    auto guard = this->callbacks_field.lock().unwrap();
    (*guard).on_reconnected.push(std::move(callback_shadow1));
}

void CallbackManager::invoke_on_connected() const {
    this->inflight_enter();
    const auto callbacks = [&]() { auto guard = this->callbacks_field.lock().unwrap();
return rusty::clone((*guard).on_connected); }();
    const auto count = rusty::len(callbacks);
    auto index = 0;
    while (rusty::detail::deref_if_pointer_like(index) < rusty::detail::deref_if_pointer_like(count)) {
        ::rrr::invoke_connection_callback_safely(callbacks[index]);
        rusty::detail::deref_if_pointer_like(index) += 1;
    }
    this->inflight_exit();
}
...
void CallbackManager::invoke_on_error(::rrr::RpcError error, const std::string& message) const {
    this->inflight_enter();
    const auto callbacks = [&]() { auto guard = this->callbacks_field.lock().unwrap();
return rusty::clone((*guard).on_error); }();
    ...
    auto index = 0;
    while (rusty::detail::deref_if_pointer_like(index) < rusty::detail::deref_if_pointer_like(count)) {
        ::rrr::invoke_error_callback_safely(callbacks[index], std::move(error), message);
        rusty::detail::deref_if_pointer_like(index) += 1;
    }
    this->inflight_exit();
}
...
return rusty::clone((*guard).on_reconnected); }();
    ...
    auto index = 0;
    while (rusty::detail::deref_if_pointer_like(index) < rusty::detail::deref_if_pointer_like(count)) {
        ::rrr::invoke_reconnect_callback_safely(callbacks[index], std::move(success));
        rusty::detail::deref_if_pointer_like(index) += 1;
    }
    this->inflight_exit();
}
...
void CallbackManager::clear_all() const {
    ...
    auto guard = this->inflight_field.lock().unwrap();
    const auto _guard = this->inflight_cv_field->wait_while(std::move(guard), [&](auto&& inflight) { return rusty::deref_mut(inflight) != 0; }).unwrap();
}

...
// srpc-cpp-source: crates/srpc/src/base/legacy_rand.rs
// srpc-cpp-source-sha256: a518fc4cfce0c0f939ebe1dd146476fba26b5e656fab455a9cd18cd37506786b
...
// srpc-cpp-unit-id: rrr.rand.interface
// srpc-cpp-module: rrr.rand
...
// srpc-cpp-cpp-module-index-source-sha256: c94e457b7e7ff31b87d22f26941b53347d0b47b2fc041e9a64654bbde707bb05
// srpc-cpp-cpp-module-index-sha256: a9dd93df727c19dba54f91f2e4fee2915b5d07c839c86131db88f60e2aa14a0f
// srpc-cpp-output-sha256: 5bcc794b69ab2e957174cc146fc3f27549d99bd98c39512f684807296f3dc1f9
...
export struct RandomGenerator {
    ...
    static uint32_t weighted_select(const ::std::vector<double>& weight_vector);
    static void destroy();
...
int32_t next_raw() {
    // @unsafe
    {
        return legacy_rand_ffi::srpc_rand_raw();
    }
}
...
int32_t RandomGenerator::rand(int32_t min, int32_t max) {
    ...
    const int32_t raw = ::rrr::next_raw();
    const int32_t width = rusty::wrapping_add(rusty::wrapping_sub(max, static_cast<std::remove_cvref_t<decltype(max)>>(std::move(min))), static_cast<std::remove_cvref_t<decltype(rusty::wrapping_sub(max, static_cast<std::remove_cvref_t<decltype(max)>>(std::move(min))))>>(static_cast<int32_t>(1)));
    return rusty::wrapping_add((rusty::detail::deref_if_pointer_like(raw) % rusty::detail::deref_if_pointer_like(width)), static_cast<std::remove_cvref_t<decltype((rusty::detail::deref_if_pointer_like(raw) % rusty::detail::deref_if_pointer_like(width)))>>(std::move(min)));
}
...
::std::string RandomGenerator::int2str_n(int32_t i, int32_t length) {
    if (!(rusty::detail::deref_if_pointer_like(length) >= static_cast<int32_t>(0))) { throw std::logic_error("int2str_n length must be nonnegative"); }
    ::std::string text = std::format("{}" , i);
    const int32_t text_len = static_cast<int32_t>(rusty::len(text));
    if (rusty::detail::deref_if_pointer_like(text_len) < rusty::detail::deref_if_pointer_like(length)) {
        ::std::string prefix = std::format("");
        int32_t remaining = rusty::detail::deref_if_pointer_like(length) - rusty::detail::deref_if_pointer_like(text_len);
        while (rusty::detail::deref_if_pointer_like(remaining) > static_cast<int32_t>(0)) {
            prefix = std::format("{}0" , prefix);
            remaining -= static_cast<int32_t>(1);
        }
        return std::format("{}{}" , prefix , text);
    }
    if (rusty::detail::deref_if_pointer_like(text_len) > rusty::detail::deref_if_pointer_like(length)) {
        const auto bytes = rusty::as_bytes(text);
        size_t index = rusty::len(text) - ((static_cast<size_t>(length)));
        ::std::string suffix = std::format("");
        while (rusty::detail::deref_if_pointer_like(index) < rusty::len(text)) {
            suffix = std::format("{}{}", rusty::to_string(suffix), rusty::to_string(static_cast<char32_t>(bytes[index])));
...
int32_t RandomGenerator::nu_rand(int32_t a, int32_t x, int32_t y) {
    const int32_t r1 = RandomGenerator::rand(static_cast<int32_t>(0), std::move(a));
    const int32_t r2 = RandomGenerator::rand(std::move(x), std::move(y));
    return ((((rusty::detail::deref_if_pointer_like(r1) | rusty::detail::deref_if_pointer_like(r2))) % ((((rusty::detail::deref_if_pointer_like(y) - rusty::detail::deref_if_pointer_like(x))) + static_cast<int32_t>(1))))) + rusty::detail::deref_if_pointer_like(x);
}

uint32_t RandomGenerator::weighted_select(const ::std::vector<double>& weight_vector) {
    double sum = 0.0;
    uint32_t index = static_cast<uint32_t>(0);
    while (((static_cast<size_t>(index))) < rusty::len(weight_vector)) {
        sum += weight_vector[static_cast<size_t>(index)];
        index = rusty::wrapping_add(index, static_cast<std::remove_cvref_t<decltype(index)>>(static_cast<uint32_t>(1)));
    }
    const double draw = RandomGenerator::rand_double(0.0, std::move(sum));
    double stage_sum = 0.0;
    uint32_t selected = static_cast<uint32_t>(0);
    while (((static_cast<size_t>(selected))) < rusty::len(weight_vector)) {
        stage_sum += weight_vector[static_cast<size_t>(selected)];
        if (rusty::detail::deref_if_pointer_like(draw) <= rusty::detail::deref_if_pointer_like(stage_sum)) {
            return std::move(selected);
        }
        selected = rusty::wrapping_add(selected, static_cast<std::remove_cvref_t<decltype(selected)>>(static_cast<uint32_t>(1)));
    }
    return rusty::wrapping_sub(selected, static_cast<std::remove_cvref_t<decltype(selected)>>(static_cast<uint32_t>(1)));
}

void RandomGenerator::destroy() {
    // @unsafe
    {
        legacy_rand_ffi::srpc_rand_destroy();
    }
}

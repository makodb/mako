...
// srpc-cpp-source: crates/srpc/src/rpc/request_queue.rs
// srpc-cpp-source-sha256: 2188784a706d02d8fc4f5b1368d56970e132b1eca898f3c4f29a2d72fe53ff61
...
// srpc-cpp-unit-id: rrr.request_queue.interface
// srpc-cpp-module: rrr.request_queue
...
// srpc-cpp-output-sha256: 2242843139d2b200f1a601229566e8838fa14e744dc2c61eb5d42f8d1b2ec559
...
export enum class OverflowStrategy : int32_t;
...
void rq_invoke_callback_safely(rusty::Function<void(int32_t)> callback, int32_t error_code);
...
export struct RequestQueueConfig {
    ...
    ::rrr::OverflowStrategy overflow_strategy;
    bool enabled;
...
export struct RequestQueue {
    ...
    RequestQueue();
    RequestQueue(::rrr::RequestQueueConfig config);
    bool enqueue(::rrr::QueuedRequest request) const;
    ...
    void update_config(::rrr::RequestQueueConfig config) const;
};

export std::string_view overflow_strategy_to_string(::rrr::OverflowStrategy strategy) {
    return [&]() -> std::string_view { auto&& _m = strategy; if (_m == OverflowStrategy::DROP_OLDEST) return std::string_view("DROP_OLDEST");
if (_m == OverflowStrategy::DROP_NEWEST) return std::string_view("DROP_NEWEST");
if (_m == OverflowStrategy::FAIL_FAST) return std::string_view("FAIL_FAST");
...
void rq_invoke_callback_safely(rusty::Function<void(int32_t)> callback, int32_t error_code) {
    if (auto& _iflet_callback_slot = callback; static_cast<bool>(_iflet_callback_slot)) {
        auto& callback = _iflet_callback_slot;
        static_cast<void>(catch_unwind(AssertUnwindSafe([&]() {
callback(std::move(error_code));
})));
    }
}
...
QueuedRequest QueuedRequest::new_() {
    return QueuedRequest{.xid = static_cast<int64_t>(0), .rpc_id = static_cast<int32_t>(0), .timestamp_us = ::rrr::queued_request_time_us(), .retry_count = static_cast<uint32_t>(0), .callback = rusty::Function<void(int32_t)>{}, .ttl_ms = static_cast<uint32_t>(30000)};
}

bool QueuedRequest::is_expired() const {
    const uint64_t now_us = ::rrr::queued_request_time_us();
    const uint64_t elapsed_us = rusty::wrapping_sub(now_us, static_cast<std::remove_cvref_t<decltype(now_us)>>(this->timestamp_us));
...
uint32_t QueuedRequest::age_ms() const {
    const uint64_t now_us = ::rrr::queued_request_time_us();
    return static_cast<uint32_t>((rusty::wrapping_sub(now_us, static_cast<std::remove_cvref_t<decltype(now_us)>>(this->timestamp_us)) / static_cast<uint64_t>(1000)));
}
...
RequestQueue::RequestQueue(::rrr::RequestQueueConfig config)
    : config_(rusty::Cell<RequestQueueConfig>::new_(std::move(config)))
    , queue_(rusty::Mutex<rusty::VecDeque<QueuedRequest>>::new_(rusty::VecDeque<QueuedRequest>::new_()))
{}

bool RequestQueue::enqueue(::rrr::QueuedRequest request) const {
    if (rusty::detail::rust_not(this->config_.get().enabled)) {
        ::rrr::rq_invoke_callback_safely(std::move(request.callback), std::move(kRequestQueueRejectedError));
        return false;
    }
    auto guard = this->queue_.lock().unwrap();
    const RequestQueueConfig config = this->config_.get();
    if (rusty::len(guard) >= rusty::detail::deref_if_pointer_like(config.max_size)) {
        switch (config.overflow_strategy) {
        case OverflowStrategy::DROP_OLDEST:
        {
            if (rusty::detail::rust_not(rusty::is_empty(guard))) {
                auto oldest = (*guard).pop_front().unwrap();
                ::rrr::rq_invoke_callback_safely(std::move([&](auto&& __r) -> decltype(auto) { if constexpr (requires { (__r.callback); }) { return (__r.callback); } else if constexpr (requires { (__r.callback_field); }) { return (__r.callback_field); } else if constexpr (requires { ((*__r).callback); }) { return ((*__r).callback); } else { return ((*__r).callback_field); } }(oldest)), std::move(kRequestQueueRejectedError));
            }
            break;
        }
    ...
    if (rusty::detail::deref_if_pointer_like(request.ttl_ms) == static_cast<uint32_t>(0)) {
        request.ttl_ms = std::move(config.default_ttl_ms);
    }
...
size_t RequestQueue::expire_stale() const {
    rusty::Vec<rusty::Function<void(int32_t)>> callbacks_to_invoke = rusty::Vec<rusty::Function<void(int32_t)>>::new_();
    size_t removed = static_cast<size_t>(0);
    {
        ...
        while (rusty::detail::deref_if_pointer_like(index) < rusty::len(guard)) {
            if (guard[index].is_expired()) {
                auto request = (*guard).remove(std::move(index)).unwrap();
                callbacks_to_invoke.push(std::move([&](auto&& __r) -> decltype(auto) { if constexpr (requires { (__r.callback); }) { return (__r.callback); } else if constexpr (requires { (__r.callback_field); }) { return (__r.callback_field); } else if constexpr (requires { ((*__r).callback); }) { return ((*__r).callback); } else { return ((*__r).callback_field); } }(request)));
                removed = rusty::wrapping_add(removed, static_cast<std::remove_cvref_t<decltype(removed)>>(static_cast<size_t>(1)));
            } else {
                index = rusty::wrapping_add(index, static_cast<std::remove_cvref_t<decltype(index)>>(static_cast<size_t>(1)));
            }
        }
    }
    for (auto&& callback : rusty::for_in(callbacks_to_invoke)) {
        ::rrr::rq_invoke_callback_safely(std::move(callback), std::move(kRequestQueueExpiredError));
    }
...
bool RequestQueue::empty() const {
    auto guard = this->queue_.lock().unwrap();
    return rusty::is_empty(guard);
}

bool RequestQueue::full() const {
    auto guard = this->queue_.lock().unwrap();
    return rusty::len(guard) >= rusty::detail::deref_if_pointer_like(this->config_.get().max_size);
}

size_t RequestQueue::remaining_capacity() const {
    auto guard = this->queue_.lock().unwrap();
    const size_t max_size = this->config_.get().max_size;
    if (rusty::detail::deref_if_pointer_like(max_size) > rusty::len(guard)) {
        return rusty::detail::deref_if_pointer_like(max_size) - rusty::len(guard);
    } else {
...
void RequestQueue::clear_all(int32_t error_code) const {
    ...
    {
        auto guard = this->queue_.lock().unwrap();
        while (rusty::detail::rust_not(rusty::is_empty(guard))) {
            auto request = (*guard).pop_front().unwrap();
            callbacks_to_invoke.push(std::move([&](auto&& __r) -> decltype(auto) { if constexpr (requires { (__r.callback); }) { return (__r.callback); } else if constexpr (requires { (__r.callback_field); }) { return (__r.callback_field); } else if constexpr (requires { ((*__r).callback); }) { return ((*__r).callback); } else { return ((*__r).callback_field); } }(request)));
        }
    }
    for (auto&& callback : rusty::for_in(callbacks_to_invoke)) {
        ::rrr::rq_invoke_callback_safely(std::move(callback), std::move(error_code));
    }
}
...
void RequestQueue::update_config(::rrr::RequestQueueConfig config) const {
    auto _guard = this->queue_.lock().unwrap();

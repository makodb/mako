...
_Static_assert(MT_RECORD_ID_NONE == UINT64_C(0),
               "zero is the absence sentinel");
...
_Static_assert(MT_FEATURE_SCOPED_POINT_READS == (UINT64_C(1) << 9),
               "scoped-read feature bit changed");
_Static_assert(MT_FEATURE_SCOPED_STRIDED_POINT_READS == (UINT64_C(1) << 10),
...
_Static_assert(offsetof(mt_read_scope, owner) == 0,
...
_Static_assert(sizeof(((mt_scan_result *)0)->reserved) == 2 * sizeof(uint64_t),
...
MT_ASSERT_FUNCTION(mt_runtime_config_init, mt_status (*)(mt_runtime_config *));
...
MT_ASSERT_FUNCTION(mt_read_scope_begin,
                   mt_status (*)(mt_tree *, mt_thread *, mt_read_scope *));
MT_ASSERT_FUNCTION(mt_read_scope_get,
                   mt_status (*)(const mt_read_scope *, const void *, size_t,
                                 mt_record_id *));
MT_ASSERT_FUNCTION(mt_read_scope_get_strided,
                   mt_status (*)(const mt_read_scope *, const void *, size_t,
                                 size_t, size_t, mt_record_id *));
...
MT_ASSERT_FUNCTION(mt_scan,
                   mt_status (*)(mt_tree *, mt_thread *, mt_scan_direction,
                                 const mt_scan_bound *, const mt_scan_bound *,
                                 mt_scan_entry *, size_t, void *, size_t,
                                 mt_scan_result *));

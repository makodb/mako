...
proc run_external_server_test {code overrides} {
    ...
    if {[catch {r flushall} err]} {
        if {![string match -nocase {*unknown command*flushall*} $err]} {
            error $err
        }
        set keys [r keys *]
        foreach chunk [lrange $keys 0 end] {
            lappend batch $chunk
            if {[llength $batch] >= 128} {
                r del {*}$batch
                set batch {}
            }
        }
        if {[info exists batch] && [llength $batch] > 0} {
            r del {*}$batch
        }
    }
    if {[catch {r function flush} err]} {
        if {![string match -nocase {*unknown command*function*} $err]} {
            error $err
        }
    }

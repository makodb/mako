#!/usr/bin/env bash
# Drift guard for the inline-Rust DSL in src/srpc.
#
...
# today: scripts/regen_storage_dsl.sh has zero src/srpc entries. Two
# blocks (client.1, server.1) drifted exactly this way.
#
...
mapfile -t FILES < <(grep -rl '#if RUSTYCPP_RUST' src/srpc --include='*.cpp' \

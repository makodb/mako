...
class CheckedInCanaryTests(unittest.TestCase):
    ...
    def test_source_boundary_census_tracks_the_remaining_scaffolding(self) -> None:
        ...
        # src/srpc/rpc/frame_codec.hpp compatibility shim. Two lines of std
    ...
    def test_checked_in_modules_are_canonical_rust_sources(self) -> None:
        ...
        # `#[cfg_attr(any(), cpp_import_namespace(srpc))]` marker lines were
        ...
        # 2563 -> 2677: +114, the canonical `src/srpc/src/debugging.rs` grafted
    ...
    def test_unsafe_allowances_are_confined_to_the_audited_c_boundaries(self) -> None:
        with (REPOSITORY / "src/srpc/Cargo.toml").open("rb") as stream:
        ...
        rand_path = REPOSITORY / "src/srpc/misc/rand.rs"
        circuit_path = REPOSITORY / "src/srpc/rpc/circuit_breaker.rs"
        basetypes_path = REPOSITORY / "src/srpc/base/basetypes.rs"
        utils_path = REPOSITORY / "src/srpc/rpc/utils.rs"
        frame_codec_path = REPOSITORY / "src/srpc/rpc/frame_codec.rs"
        debugging_path = REPOSITORY / "src/srpc/base/debugging.rs"
...
class ArchiverResolutionTests(unittest.TestCase):
    ...
    def setUp(self) -> None:
        temporary = tempfile.TemporaryDirectory(prefix="srpc-archiver-")
        self.addCleanup(temporary.cleanup)
...
class CrateModeGateTests(unittest.TestCase):
    def test_frame_codec_io_preamble_is_rejected_from_siblings(self) -> None:
        with tempfile.TemporaryDirectory(prefix="srpc-gate-preamble-") as temporary:
            output = Path(temporary)
            (output / "CMakeLists.txt").write_text(
                "# generated\n", encoding="utf-8"
            )
            (output / "srpc.frame_codec.cppm").write_text(
                "// generated\nmodule;\n"
                "#include <vector>\n"
            ...
            (output / "srpc.cppm").write_text(
                "export module srpc;\n"
                "namespace srpc {\n"
                "export import srpc.frame_codec;\n"
                "export import srpc.example;\n",
                encoding="utf-8",
            )
            modules = [
                mock.Mock(cpp_module="srpc.frame_codec"),
                mock.Mock(cpp_module="srpc.example"),
            ]
            specs = {
    ...
    def test_utils_preamble_is_rejected_from_sibling_children(self) -> None:
        with tempfile.TemporaryDirectory(prefix="srpc-gate-preamble-") as temporary:
            ...
            (output / "srpc.utils.cppm").write_text(
                "// generated\nmodule;\n"
                "#include <netdb.h>\n"
                "#include <cstdint>\n"
            ...
            (output / "srpc.cppm").write_text(
                "export module srpc;\n"
                "namespace srpc {\n"
                "export import srpc.utils;\n"
                "export import srpc.example;\n",
                encoding="utf-8",
            )
            modules = [
                mock.Mock(cpp_module="srpc.utils"),
                mock.Mock(cpp_module="srpc.example"),
            ]
            specs = {
    ...
    def test_placeholder_ratchet_checks_named_module_purview(self) -> None:
        with tempfile.TemporaryDirectory(prefix="srpc-gate-placeholder-") as temporary:
            generated = Path(temporary) / "srpc.example.cppm"
    ...
    def test_generated_children_have_only_their_direct_module_imports(self) -> None:
        ...
        with self.assertRaisesRegex(GATE.GateError, "exported=\\['srpc.rand'\\]"):
            GATE.require_exact_module_imports(
                "export module srpc.request_options;\n"
                "export import srpc.rand;\n",
                "srpc.request_options",
                ["srpc.rand"],
            )

    def test_executable_preserves_cxx_driver_symlink_spelling(self) -> None:
        with tempfile.TemporaryDirectory(prefix="srpc-gate-driver-test-") as temporary:
            root = Path(temporary)
    ...
    def test_symbol_owner_ignores_module_attachments_in_types(self) -> None:
        cases = {
            "srpc::f@srpc.errors(srpc::RpcError@srpc.errors)": "srpc.errors",
            "srpc::AvgStat@srpc.stat::avg() const": "srpc.stat",
            "srpc::value@srpc.internal_protocol": "srpc.internal_protocol",
            (
                "srpc::Facade<srpc::RpcError@srpc.errors>"
                "@srpc.client::call()"
            ): "srpc.client",
            "srpc::Facade<srpc::RpcError@srpc.errors>::call()": None,
            (
                "srpc::RpcError@srpc.errors "
                "rusty::clone<srpc::RpcError@srpc.errors>(int)"
            ): None,
            "srpc::Facade@srpc.types<int> srpc::make@srpc.errors<int>()": "srpc.errors",
            "srpc::operator<@srpc.errors(srpc::RpcError@srpc.errors, srpc::RpcError@srpc.errors)": "srpc.errors",
            "srpc::Widget@srpc.errors::operator bool() const": "srpc.errors",
            "srpc::Widget@srpc.errors::operator srpc::Facade@srpc.types<int>() const": "srpc.errors",
            "srpc::plain(int)": None,
        }
    ...
    def test_module_symbols_ratchets_only_strong_owned_definitions(self) -> None:
        output = "\n".join(
            [
                "0001 T srpc::f@srpc.errors(srpc::RpcError@srpc.errors)",
                "0002 W srpc::helper@srpc.errors()",
                "0003 t srpc::local@srpc.errors()",
                "0004 T srpc::other@srpc.client(srpc::RpcError@srpc.errors)",
                "0005 W srpc::RpcError@srpc.errors rusty::clone<int>(int)",
            ]
        )
        with mock.patch.object(GATE, "run", return_value=output):
            symbols = GATE.module_symbols(
                Path("/usr/bin/nm"),
                Path("/repository"),
                Path("/repository/libsrpc.a"),
                "srpc.errors",
            )
        self.assertEqual(
            symbols,
            {("T", "srpc::f@srpc.errors(srpc::RpcError@srpc.errors)")},
        )
    ...
    def test_symbol_census_uses_the_definition_owner_not_parameter_types(self) -> None:
        ...
        foreign = (
            ...
            "srpc::ConnectionMetrics@srpc.connection_metrics)"
        )
    ...
    def test_build_tree_output_is_reused_without_second_generation(self) -> None:
        with tempfile.TemporaryDirectory(prefix="srpc-gate-reuse-test-") as temporary:
            root = Path(temporary)
            ...
            modules = [mock.Mock(cpp_module="srpc.internal_protocol")]
            with mock.patch.object(
                GATE, "repository_root", return_value=root
            ), mock.patch.object(
                GATE,
    ...
    def test_standalone_generation_consumes_the_structured_preamble(self) -> None:
        with tempfile.TemporaryDirectory(prefix="srpc-gate-generate-test-") as temporary:
            ...
            self.assertEqual(
                command[command.index("--flat-import-namespace") + 1], "srpc"
            )
            self.assertEqual(
                command[-6:],
                [
            ...
            self.assertEqual(
                command[2], str((root / "src/srpc/Cargo.toml").resolve())
            )
    ...
    def test_cmake_crate_invocation_uses_an_absolute_manifest_variable(self) -> None:
        ...
        self.assertIn('--crate "${SRPC_GOAL0_CRATE_MANIFEST}"', cmake)
        self.assertNotIn(
            '--crate "${SRPC_SOURCE_DIR}/Cargo.toml"', cmake
        )

    def test_generated_gate_compiles_children_before_partial_root(self) -> None:
        modules = [
            mock.Mock(cpp_module="srpc.basetypes"),
            mock.Mock(cpp_module="srpc.callback_wrapper"),
            mock.Mock(cpp_module="srpc.internal_protocol"),
            mock.Mock(cpp_module="srpc.stat"),
            mock.Mock(cpp_module="srpc.errors"),
            mock.Mock(cpp_module="srpc.connection_metrics"),
            mock.Mock(cpp_module="srpc.completion_tracker"),
            mock.Mock(cpp_module="srpc.rand"),
            mock.Mock(cpp_module="srpc.request_options"),
            mock.Mock(cpp_module="srpc.reconnect_policy"),
            mock.Mock(cpp_module="srpc.circuit_breaker"),
            mock.Mock(cpp_module="srpc.connection_state"),
            mock.Mock(cpp_module="srpc.heartbeat"),
            mock.Mock(cpp_module="srpc.request_queue"),
            mock.Mock(cpp_module="srpc.load_balancer"),
            mock.Mock(cpp_module="srpc.utils"),
            mock.Mock(cpp_module="srpc.frame_codec"),
            mock.Mock(cpp_module="srpc.serializable"),
            mock.Mock(cpp_module="srpc.serializable_envelope"),
            mock.Mock(cpp_module="srpc.future"),
            mock.Mock(cpp_module="srpc.logging"),
            mock.Mock(cpp_module="srpc.idempotency"),
            mock.Mock(cpp_module="srpc.fiber"),
            mock.Mock(cpp_module="srpc.misc"),
            mock.Mock(cpp_module="srpc.channel"),
            mock.Mock(cpp_module="srpc.epoll_wrapper"),
            mock.Mock(cpp_module="srpc.pollable_proxy"),
            mock.Mock(cpp_module="srpc.callbacks"),
            mock.Mock(cpp_module="srpc.inmemory_channel"),
            mock.Mock(cpp_module="srpc.fiber_channel"),
            mock.Mock(cpp_module="srpc.threading"),
            mock.Mock(cpp_module="srpc.debugging"),
            mock.Mock(cpp_module="srpc.any_message"),
            mock.Mock(cpp_module="srpc.tcp_channel"),
            mock.Mock(cpp_module="srpc.reactor"),
            mock.Mock(cpp_module="srpc.server"),
            mock.Mock(cpp_module="srpc.client"),
        ]

        def symbols_for_module(
            _nm: Path, _root: Path, path: Path, module_name: str
        ) -> frozenset[tuple[str, str]]:
            # srpc.epoll_wrapper follows Rust std's sys-module pattern: its
            # platform implementation unit is compiled into libsrpc.a but is not
        ...
        completion_raw = list(GATE.ABI_SPECS["srpc.completion_tracker"].symbols)
        completion_raw.append(
            ("T", "initializer for module srpc.completion_tracker")
        )
        rand_raw = list(GATE.ABI_SPECS["srpc.rand"].symbols)
        rand_raw.append(("T", "initializer for module srpc.rand"))
        request_options_raw = list(
            GATE.ABI_SPECS["srpc.request_options"].symbols
        )
        request_options_raw.append(
            ("T", "initializer for module srpc.request_options")
        )
        reconnect_policy_raw = list(
            GATE.ABI_SPECS["srpc.reconnect_policy"].symbols
        )
        reconnect_policy_raw.append(
            ("T", "initializer for module srpc.reconnect_policy")
        )
        circuit_breaker_raw = list(
            GATE.ABI_SPECS["srpc.circuit_breaker"].symbols
        )
        circuit_breaker_raw.append(
            ("T", "initializer for module srpc.circuit_breaker")
        )
        basetypes_raw = list(GATE.ABI_SPECS["srpc.basetypes"].symbols)
        basetypes_raw.append(("T", "initializer for module srpc.basetypes"))
        request_queue_raw = list(GATE.ABI_SPECS["srpc.request_queue"].symbols)
        request_queue_raw.append(
            ("T", "initializer for module srpc.request_queue")
        )
        exact_raw = {
            name: [
        ...
        utils_raw = list(GATE.ABI_SPECS["srpc.utils"].symbols)
        for symbol in (
        ...
        utils_raw.append(("T", "initializer for module srpc.utils"))
        ...
        # reorders the schedule is modelled: `srpc.utils` imports `srpc.logging`,
        # and under this (srpc-owned) manifest order `srpc.logging` is declared
        # AFTER `srpc.utils`.
        crate_imports = {"srpc.utils": ["srpc.logging"]}
        with tempfile.TemporaryDirectory(prefix="srpc-gate-children-test-") as temporary:
            output = Path(temporary)
            for module in modules:
        ...
        # The srpc.logging forwarding fixture is retired: srpc.logging is a
        # canonical Rust module, so the production lane links libsrpc.a's own
        ...
        self.assertFalse(
            [
                ...
                if argument.endswith("/srpc.logging.probe.o")
                or argument.endswith("/srpc.logging.o")
            ]
        )
        generated_link = next(
            command
    ...
    def test_basetypes_cpp_oracle_pins_abort_and_atomic_concurrency(self) -> None:
        ...
        self.assertIn("std::abort();", GATE.ABI_SPECS["srpc.basetypes"].surface)
    ...
    def test_compile_module_passes_every_runtime_bmi_directory(self) -> None:
        ...
        self.assertEqual(result, Path("/work/srpc.completion_tracker.o"))
        self.assertEqual(len(run.call_args_list), 2)
        for call in run.call_args_list:
            command = call.args[0]
            self.assertIn("-std=gnu++23", command)
            self.assertIn("/repository/src/srpc", command)
    ...
    def test_gate_abi_ratchet_covers_every_manifest_module(self) -> None:
        ...
        with mock.patch.object(
            GATE.extraction,
            "load_manifest",
            return_value=[*modules, mock.Mock(cpp_module="srpc.orphan")],
        ):
            with self.assertRaisesRegex(

...
# t=1..11, then generate comparison plots.
#
...
#
# Usage:
#   nohup bash scripts/overnight_three_way.sh > overnight.log 2>&1 &
...
OUT="results/benchmarks/overnight_three_way_$STAMP"
mkdir -p "$OUT"

echo "===================================================================="
echo "  Overnight three-way sweep (Single-Raft 1:1, Multi-Raft, Paxos)"
echo "  Started: $(date)"
echo "  Output:  $OUT"
...
cp "$SRC_SINGLE/results.csv" "$OUT/single_1to1.csv"
cp -r "$SRC_SINGLE" "$OUT/single_1to1_sweep_dir"
echo "Single-Raft 1:1 done at $(date).  CSV: $OUT/single_1to1.csv"
...
cp "$SRC_MULTI/results.csv" "$OUT/multi.csv"
cp -r "$SRC_MULTI" "$OUT/multi_sweep_dir"
echo "Multi-Raft done at $(date).  CSV: $OUT/multi.csv"
...
cp "$SRC_PAXOS/results.csv" "$OUT/paxos.csv"
cp -r "$SRC_PAXOS" "$OUT/paxos_sweep_dir"
echo "Paxos done at $(date).  CSV: $OUT/paxos.csv"

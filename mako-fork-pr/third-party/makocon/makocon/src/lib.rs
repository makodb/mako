...
pub struct Server<T> {
    ...
    pub command: Option<fn(DecodedFrame<BytesFrame>, &mut Conn, &T)>,
            ...
            Ok(stream) => {
                ...
                thread::spawn(move || {
                    ...
                    loop {
                        ...
                        //     }
                        // }
                        match conn.reader.get_mut().read(&mut read_buf) {
                            Ok(0) => { conn.closed = true; break; }
                            Ok(n) => resp3.read_bytes(&read_buf[..n]),
                            Err(e) => { final_err = Some(Error::IoError(e)); conn.closed = true; break; }
                        }

                        while let Ok(opt) = resp3.next_frame() {
                            if let Some(frame) = opt {
                                if let Some(cmd_handler) = &command {
                                    cmd_handler(frame, &mut conn, &data);
                                }
                            } else {
                                break;
                            }
